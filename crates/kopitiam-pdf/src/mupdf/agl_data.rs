//! Generated Adobe Glyph List data, ported verbatim from MuPDF
//! `source/fitz/glyphlist.h` (commit 19f1284, AGPL-3.0, © Artifex Software,
//! Inc.), which `scripts/glyphdump.py` itself generated from Adobe's
//! `glyphlist.txt`. Translated to Rust for KOPITIAM (AGPL-3.0-only): the two
//! parallel C arrays `single_name_list[]` / `single_code_list[]` are fused into
//! one `(glyph name, Unicode)` table, kept in the C file's original order
//! (ASCII-sorted by name) so [`super::agl::unicode_from_glyph_name_strict`] can
//! binary-search it exactly as `fz_unicode_from_glyph_name_strict` does.
//!
//! Do not edit by hand: regenerate from `glyphlist.h` if the pin moves.

/// The Adobe Glyph List: `(glyph name, Unicode code point)`, ASCII-sorted by
/// name. Fused from MuPDF's `single_name_list` / `single_code_list`.
// MuPDF: single_name_list[] / single_code_list[] (glyphlist.h)
pub static AGL_SINGLE: [(&str, u16); 4455] = [
    ("A", 0x0041),
    ("AE", 0x00c6),
    ("AEacute", 0x01fc),
    ("AEmacron", 0x01e2),
