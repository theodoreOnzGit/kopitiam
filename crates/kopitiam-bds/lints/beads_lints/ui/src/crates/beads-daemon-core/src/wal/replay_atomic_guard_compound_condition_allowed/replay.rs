...
fn replay_index(mode: ReplayMode, index: &Index, extra_check: bool) {
    if mode != ReplayMode::CatchUp && extra_check {
        let mut txn = index.begin_txn();
        txn.write_rows();
        txn.commit();
    } else {
        // Compound predicates should not force else branch to CatchUp.
...
fn main() {
    replay_index(ReplayMode::CatchUp, &Index, false);
}
