...
use rustc_middle::ty::{
    self, AdtDef, AliasTy, AssocItem, AssocTag, Binder, BoundRegion, BoundVarIndexKind, FnSig,
    GenericArg, GenericArgKind, GenericArgsRef, IntTy, Region, RegionKind, TraitRef, Ty, TyCtxt,
    TypeSuperVisitable, TypeVisitable, TypeVisitableExt, TypeVisitor, UintTy, Upcast, VariantDef,
    VariantDiscr,
};
        ...
        ty::Dynamic(binder, _) => {
            for predicate in *binder {
                if let ty::ExistentialPredicate::Trait(ref trait_ref) = predicate.skip_binder()
                    && find_attr!(
                        cx.tcx.get_all_attrs(trait_ref.def_id),
                        AttributeKind::MustUse { .. }
                    )
                {
                    return true;
    ...
    match *ty.kind() {
        ty::Closure(id, subs) => {
            let decl = id.as_local().and_then(|id| {
                cx.tcx
                    .hir_fn_decl_by_hir_id(cx.tcx.local_def_id_to_hir_id(id))
            });
                        ...
                        || Some(bound.def_id()) == lang_items.fn_mut_trait() =>
                {
                    let output = bounds
                        ...
                        .find(|p| {
                            lang_items
                                .fn_once_output()
                                .is_some_and(|id| id == p.item_def_id())
                        })
...
fn assert_generic_args_match<'tcx>(tcx: TyCtxt<'tcx>, did: DefId, args: &[GenericArg<'tcx>]) {
    ...
    assert!(
        ...
        params
            .clone()
            .map(ty::GenericParamDefKind::descr)
            .format(", "),
    );
...
pub fn make_normalized_projection<'tcx>(
    ...
    fn helper<'tcx>(
        ...
        match tcx.try_normalize_erasing_regions(
            typing_env,
            Ty::new_projection_from_args(tcx, ty.def_id, ty.args),
        ) {
            Ok(ty) => Some(ty),
            Err(e) => {
...
impl<'tcx> InteriorMut<'tcx> {
    ...
    fn interior_mut_ty_chain_inner(
        ...
        let chain = match *ty.kind() {
            ty::RawPtr(inner_ty, _) if !self.ignore_pointers => {
                self.interior_mut_ty_chain_inner(cx, inner_ty, depth)
            }
            ty::Ref(_, inner_ty, _) | ty::Slice(inner_ty) => {
                self.interior_mut_ty_chain_inner(cx, inner_ty, depth)
            }
            ...
            ty::Adt(def, args) => {
                ...
                if is_std_collection || def.is_box() {
                    ...
                    def.all_fields().find_map(|f| {
                        self.interior_mut_ty_chain_inner(cx, f.ty(cx.tcx, args), depth)
                    })
                }
            }
            ty::Alias(ty::Projection, _) => {
                match cx.tcx.try_normalize_erasing_regions(cx.typing_env(), ty) {
                    Ok(normalized_ty) if ty != normalized_ty => {
                        self.interior_mut_ty_chain_inner(cx, normalized_ty, depth)
                    }
                    _ => None,
                }
            }
        ...
        match ty.kind() {
            ...
            ty::Adt(adt_def, args) => adt_def.all_fields().any(|field| {
                has_non_owning_mutable_access_inner(
                    cx,
                    phantoms,
                    normalize_ty(cx, field.ty(cx.tcx, args)),
                )
            }),
            ty::Array(elem_ty, _) | ty::Slice(elem_ty) => {
                has_non_owning_mutable_access_inner(cx, phantoms, *elem_ty)
            }
...
pub fn get_field_idx_by_name(ty: Ty<'_>, name: Symbol) -> Option<usize> {
    match *ty.kind() {
        ty::Adt(def, _) if def.is_union() || def.is_struct() => def
            .non_enum_variant()
            .fields
            .iter()
...
pub fn adjust_derefs_manually_drop<'tcx>(
    ...
    adjustments.iter().any(|a| {
        ...
        matches!(a.kind, Adjust::Deref(Some(op)) if op.mutbl == Mutability::Mut)
            && is_manually_drop(ty)
    })
}
