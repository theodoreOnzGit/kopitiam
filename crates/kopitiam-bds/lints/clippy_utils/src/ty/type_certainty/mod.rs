...
use rustc_hir::{
    self as hir, AmbigArg, Expr, ExprKind, GenericArgs, HirId, Node, Param, PathSegment, QPath,
    TyKind,
};
...
fn expr_type_certainty(cx: &LateContext<'_>, expr: &Expr<'_>, in_arg: bool) -> Certainty {
    let certainty = match &expr.kind {
        ...
        ExprKind::Array(exprs) => join(
            exprs
                .iter()
                .map(|expr| expr_type_certainty(cx, expr, in_arg)),
        ),
        ...
        ExprKind::Binary(_, lhs, rhs) => {
            ...
            match (
                ...
                (Certainty::Uncertain, Certainty::Certain(_))
                | (Certainty::Certain(_), Certainty::Uncertain) => Certainty::Certain(None),
                (l, r) => l.meet(r),
            }
        }
        ...
        ExprKind::If(_, if_expr, Some(else_expr)) => expr_type_certainty(cx, if_expr, in_arg)
            .join(expr_type_certainty(cx, else_expr, in_arg)),
...
fn param_certainty(cx: &LateContext<'_>, param: &Param<'_>) -> Certainty {
    ...
    let Some(fn_decl) = cx
        .tcx
        .hir_fn_decl_by_hir_id(cx.tcx.local_def_id_to_hir_id(owner_did))
    else {
    ...
    std::iter::zip(body_params, inputs)
        ...
        .map_or(Certainty::Uncertain, |(_, ty)| {
            type_certainty(cx, ty).clear_def_id()
        })
}
...
fn path_segment_certainty(
    ...
    let certainty = match update_res(cx, parent_certainty, path_segment, resolves_to_type)
        .unwrap_or(path_segment.res)
    {
        ...
        Res::Local(hir_id) => match cx.tcx.parent_hir_node(hir_id) {
            ...
            Node::LetStmt(local) => {
                ...
                let rhs = local.init.map_or(Certainty::Uncertain, |init| {
                    expr_type_certainty(cx, init, false)
                });
