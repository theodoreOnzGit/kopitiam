...
use rustc_hir::{
    AssocItemConstraint, BinOpKind, BindingMode, Block, BodyId, ByRef, Closure, ConstArg,
    ConstArgKind, ConstItemRhs, Expr, ExprField, ExprKind, FnDecl, FnRetTy, FnSig, GenericArg,
    GenericArgs, GenericBound, GenericBounds, GenericParam, GenericParamKind, GenericParamSource,
    Generics, HirId, HirIdMap, InlineAsmOperand, ItemId, ItemKind, LetExpr, Lifetime, LifetimeKind,
    LifetimeParamKind, Node, ParamName, Pat, PatExpr, PatExprKind, PatField, PatKind, Path,
    PathSegment, PreciseCapturingArgKind, PrimTy, QPath, Stmt, StmtKind, StructTailExpr,
    TraitBoundModifiers, Ty, TyKind, TyPat, TyPatKind, UseKind, WherePredicate, WherePredicateKind,
};
...
impl HirEqInterExpr<'_, '_, '_> {
    ...
    pub fn eq_item(&mut self, l: ItemId, r: ItemId) -> bool {
        ...
        let eq = match (left.kind, right.kind) {
            ...
            (
                ItemKind::Static(l_mut, l_ident, l_ty, l_body),
                ItemKind::Static(r_mut, r_ident, r_ty, r_body),
            ) => {
                l_mut == r_mut
                    && l_ident.name == r_ident.name
                    && self.eq_ty(l_ty, r_ty)
            ...
            (
                ItemKind::TyAlias(l_ident, l_generics, l_ty),
                ItemKind::TyAlias(r_ident, r_generics, r_ty),
            ) => {
                l_ident.name == r_ident.name
                    && self.eq_generics(l_generics, r_generics)
                    && self.eq_ty(l_ty, r_ty)
            ...
            (ItemKind::Use(l_path, l_kind), ItemKind::Use(r_path, r_kind)) => {
                self.eq_path_segments(l_path.segments, r_path.segments)
                    && match (l_kind, r_kind) {
                        (UseKind::Single(l_ident), UseKind::Single(r_ident)) => {
                            l_ident.name == r_ident.name
                        }
                        (UseKind::Glob, UseKind::Glob) | (UseKind::ListStem, UseKind::ListStem) => {
                            true
                        }
                        _ => false,
                    }
            ...
            (ItemKind::Mod(l_ident, l_mod), ItemKind::Mod(r_ident, r_mod)) => {
                l_ident.name == r_ident.name
                    && over(l_mod.item_ids, r_mod.item_ids, |l, r| self.eq_item(*l, *r))
            }
            _ => false,
        };
        if eq {
            self.local_items
                .insert(l.owner_id.to_def_id(), r.owner_id.to_def_id());
        }
        eq
    }
    ...
    fn eq_generics_predicate(
        &mut self,
        left: &[WherePredicate<'_>],
        right: &[WherePredicate<'_>],
    ) -> bool {
        over(left, right, |l, r| match (l.kind, r.kind) {
            (
                WherePredicateKind::BoundPredicate(l_bound),
                WherePredicateKind::BoundPredicate(r_bound),
            ) => {
                l_bound.origin == r_bound.origin
                    && self.eq_ty(l_bound.bounded_ty, r_bound.bounded_ty)
                    && self.eq_generics_param(
                        l_bound.bound_generic_params,
                        r_bound.bound_generic_params,
                    )
                    && self.eq_generics_bound(l_bound.bounds, r_bound.bounds)
            }
            (
                WherePredicateKind::RegionPredicate(l_region),
                WherePredicateKind::RegionPredicate(r_region),
            ) => {
        ...
        over(left, right, |l, r| match (l, r) {
            (GenericBound::Trait(l_trait), GenericBound::Trait(r_trait)) => {
                l_trait.modifiers == r_trait.modifiers
                    ...
                    && self.eq_generics_param(
                        l_trait.bound_generic_params,
                        r_trait.bound_generic_params,
                    )
            }
            ...
            (GenericBound::Use(l_capture, _), GenericBound::Use(r_capture, _)) => {
                over(l_capture, r_capture, |l, r| match (l, r) {
                    (
                        PreciseCapturingArgKind::Lifetime(l_lifetime),
                        PreciseCapturingArgKind::Lifetime(r_lifetime),
                    ) => Self::eq_lifetime(l_lifetime, r_lifetime),
                    (
                        PreciseCapturingArgKind::Param(l_param),
                        PreciseCapturingArgKind::Param(r_param),
    ...
    fn eq_generics_param(&mut self, left: &[GenericParam<'_>], right: &[GenericParam<'_>]) -> bool {
        over(left, right, |l, r| {
            (match (l.name, r.name) {
                ...
                | (ParamName::Error(l_ident), ParamName::Error(r_ident)) => {
                    l_ident.name == r_ident.name
                }
    ...
    fn eq_generics_param_kind(
        &mut self,
        left: &GenericParamKind<'_>,
        right: &GenericParamKind<'_>,
    ) -> bool {
        match (left, right) {
            (
                GenericParamKind::Lifetime { kind: l_kind },
                GenericParamKind::Lifetime { kind: r_kind },
            ) => match (l_kind, r_kind) {
                (LifetimeParamKind::Explicit, LifetimeParamKind::Explicit)
                | (LifetimeParamKind::Error, LifetimeParamKind::Error) => true,
                (
                    LifetimeParamKind::Elided(l_lifetime_kind),
                    LifetimeParamKind::Elided(r_lifetime_kind),
    ...
    fn eq_generic_arg(&mut self, left: &GenericArg<'_>, right: &GenericArg<'_>) -> bool {
        match (left, right) {
            (GenericArg::Const(l), GenericArg::Const(r)) => {
                self.eq_const_arg(l.as_unambig_ct(), r.as_unambig_ct())
            }
            (GenericArg::Lifetime(l_lt), GenericArg::Lifetime(r_lt)) => {
                Self::eq_lifetime(l_lt, r_lt)
            }
            (GenericArg::Type(l_ty), GenericArg::Type(r_ty)) => {
                self.eq_ty(l_ty.as_unambig_ty(), r_ty.as_unambig_ty())
            }
            ...
            (ConstArgKind::Tup(l_t), ConstArgKind::Tup(r_t)) => {
                l_t.len() == r_t.len()
                    && l_t
                        .iter()
                        .zip(*r_t)
                        .all(|(l_c, r_c)| self.eq_const_arg(l_c, r_c))
            }
            (ConstArgKind::Path(l_p), ConstArgKind::Path(r_p)) => self.eq_qpath(l_p, r_p),
            (ConstArgKind::Anon(l_an), ConstArgKind::Anon(r_an)) => {
                self.eq_body(l_an.body, r_an.body)
            }
    ...
    fn eq_pat(&mut self, left: &Pat<'_>, right: &Pat<'_>) -> bool {
        match (&left.kind, &right.kind) {
            ...
            (PatKind::Ref(le, lp, lm), PatKind::Ref(re, rp, rm)) => {
                lp == rp && lm == rm && self.eq_pat(le, re)
            }
    ...
    fn eq_qpath(&mut self, left: &QPath<'_>, right: &QPath<'_>) -> bool {
        match (left, right) {
            (QPath::Resolved(lty, lpath), QPath::Resolved(rty, rpath)) => {
                both(lty.as_ref(), rty.as_ref(), |l, r| self.eq_ty(l, r))
        ...
        match (left.res, right.res) {
            ...
            (Res::Def(l_kind, l), Res::Def(r_kind, r))
                if l_kind == r_kind
                    ...
                    | DefKind::Mod = l_kind =>
            {
                (l == r || self.local_items.get(&l) == Some(&r))
                    && self.eq_path_segments(left.segments, right.segments)
            }
    ...
    pub fn eq_ty(&mut self, left: &Ty<'_>, right: &Ty<'_>) -> bool {
        match (&left.kind, &right.kind) {
            ...
            (TyKind::Array(lt, ll), TyKind::Array(rt, rl)) => {
                self.eq_ty(lt, rt) && self.eq_const_arg(ll, rl)
            }
    ...
    /// type or const).
    fn eq_assoc_eq_constraint(
        &mut self,
        left: &AssocItemConstraint<'_>,
        right: &AssocItemConstraint<'_>,
    ) -> bool {
            ...
            ([], None)
                if block.span.check_source_text(cx, |src| {
                    tokenize(src, FrontmatterAllowed::No)
                        ...
                        .eq([TokenKind::OpenBrace, TokenKind::CloseBrace]
                            .iter()
                            .copied())
                }) =>
            {
...
impl<'a, 'tcx> SpanlessHash<'a, 'tcx> {
    ...
    pub fn hash_expr(&mut self, e: &Expr<'_>) {
        let simple_const = self.maybe_typeck_results.and_then(|typeck_results| {
            ConstEvalCtxt::with_env(self.cx.tcx, self.cx.typing_env(), typeck_results)
                .eval_local(e, e.span.ctxt())
        });
...
pub fn has_ambiguous_literal_in_expr(cx: &LateContext<'_>, expr: &Expr<'_>) -> bool {
    match expr.kind {
        ...
        ExprKind::Array(exprs) | ExprKind::Tup(exprs) => {
            exprs.iter().any(|e| has_ambiguous_literal_in_expr(cx, e))
        }
        ...
        ExprKind::MethodCall(_, receiver, args, _) | ExprKind::Call(receiver, args) => {
            has_ambiguous_literal_in_expr(cx, receiver)
                || args.iter().any(|e| has_ambiguous_literal_in_expr(cx, e))
        }
        ...
        ExprKind::Block(blk, _) => blk
            .expr
            .as_ref()
            .is_some_and(|e| has_ambiguous_literal_in_expr(cx, e)),
        ...
        ExprKind::Match(scrutinee, arms, _) => {
            has_ambiguous_literal_in_expr(cx, scrutinee)
                || arms
                    .iter()
                    .any(|arm| has_ambiguous_literal_in_expr(cx, arm.body))
        }
        ...
        ExprKind::Ret(opt_expr) | ExprKind::Break(_, opt_expr) => opt_expr
            .as_ref()
            .is_some_and(|e| has_ambiguous_literal_in_expr(cx, e)),
