...
impl Display for Sugg<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        match self {
            ...
            Sugg::UnOp(op, inner) => {
                write!(f, "{}{}", op.as_str(), inner.clone().maybe_inner_paren())
            }
        }
    }
}
...
impl<'a> Sugg<'a> {
    ...
    pub fn hir_opt(cx: &LateContext<'_>, expr: &hir::Expr<'_>) -> Option<Self> {
        ...
        let get_snippet =
            |span| snippet_with_context(cx, span, ctxt, "", &mut Applicability::Unspecified).0;
    ...
    pub fn as_ty<R: Display>(self, rhs: R) -> Sugg<'static> {
        make_assoc(
            AssocOp::Cast,
            &self,
            &Sugg::NonParen(rhs.to_string().into()),
        )
    }
    ...
    pub fn maybe_paren(self) -> Self {
        match self {
            ...
            Sugg::UnOp(op, inner) => {
                Sugg::NonParen(format!("({}{})", op.as_str(), inner.maybe_inner_paren()).into())
            }
        }
    }
...
pub fn make_assoc(op: AssocOp, lhs: &Sugg<'_>, rhs: &Sugg<'_>) -> Sugg<'static> {
    ...
    fn is_shift(op: AssocOp) -> bool {
        matches!(
            op,
            AssocOp::Binary(ast::BinOpKind::Shl | ast::BinOpKind::Shr)
        )
    }
...
mod test {
    ...
    fn binop_maybe_paren() {
        ...
        let sugg = Sugg::BinOp(
            AssocOp::Binary(ast::BinOpKind::Add),
            "(1 + 1)".into(),
            "(1 + 1)".into(),
        );
