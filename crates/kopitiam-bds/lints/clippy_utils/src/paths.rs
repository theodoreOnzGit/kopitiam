...
impl PathLookup {
    ...
    pub fn get<'tcx>(&self, tcx: &impl HasTyCtxt<'tcx>) -> &[DefId] {
        self.once
            .get_or_init(|| lookup_path(tcx.tcx(), self.ns, self.path))
    }
    ...
    pub fn matches_path<'tcx>(
        &self,
        cx: &LateContext<'_>,
        maybe_path: impl MaybeQPath<'tcx>,
    ) -> bool {
        maybe_path
            .res(cx)
            .opt_def_id()
    ...
    pub fn matches_ty<'tcx>(&self, tcx: &impl HasTyCtxt<'tcx>, ty: Ty<'_>) -> bool {
        ty.ty_adt_def()
            .is_some_and(|adt| self.matches(&tcx.tcx(), adt.did()))
    }
}
...
pub fn lookup_path(tcx: TyCtxt<'_>, ns: PathNS, path: &[Symbol]) -> Vec<DefId> {
    ...
    let mut out = Vec::new();
    for &base in find_crates(tcx, root)
        .iter()
        .chain(find_primitive_impls(tcx, root))
    {
...
fn non_local_item_child_by_name(
    tcx: TyCtxt<'_>,
    def_id: DefId,
    ns: PathNS,
    name: Symbol,
    ...
    match tcx.def_kind(def_id) {
        DefKind::Mod | DefKind::Enum | DefKind::Trait => {
            tcx.module_children(def_id).iter().find_map(|child| {
        ...
        DefKind::Impl { .. } => {
            tcx.associated_item_def_ids(def_id)
                ...
                .copied()
                .find(|assoc_def_id| {
                    tcx.item_name(*assoc_def_id) == name
                        && ns.matches(tcx.def_kind(assoc_def_id).ns())
                })
        }
        _ => None,
    }
