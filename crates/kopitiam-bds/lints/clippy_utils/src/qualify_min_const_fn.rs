...
use rustc_middle::mir::{
    Body, CastKind, NonDivergingIntrinsic, Operand, Place, ProjectionElem, Rvalue, Statement,
...
fn check_rvalue<'tcx>(
    ...
    match rvalue {
        ...
        Rvalue::Cast(CastKind::PointerExposeProvenance, _, _) => Err((
            span,
        ...
        match elem {
            ...
            ProjectionElem::Deref => match base.ty(body, cx.tcx).ty.kind() {
                ty::RawPtr(_, hir::Mutability::Mut) => {
                    return Err((
                        span,
                        "dereferencing raw mut pointer in const fn is unstable".into(),
                    ));
                }
                    ...
                    if !msrv.meets(cx, msrvs::CONST_RAW_PTR_DEREF) =>
                {
                    return Err((
                        span,
                        "dereferencing raw const pointer in const fn is unstable".into(),
                    ));
                }
                _ => (),
            },
...
fn check_terminator<'tcx>(
    ...
    match &terminator.kind {
        ...
        TerminatorKind::SwitchInt { discr, targets: _ } => {
            check_operand(cx, discr, span, body, msrv)
        }
        ...
        | TerminatorKind::TailCall {
            ...
            if let ty::FnDef(fn_def_id, fn_substs) = fn_ty.kind() {
                ...
                let fn_def_id =
                    match Instance::try_resolve(cx.tcx, cx.typing_env(), *fn_def_id, fn_substs) {
                        Ok(Some(fn_inst)) => fn_inst.def_id(),
                        Ok(None) => {
                            return Err((
                                span,
                                format!("cannot resolve instance for {func:?}").into(),
                            ));
                        }
                        Err(_) => {
                            return Err((
                                span,
                                format!("error during instance resolution of {func:?}").into(),
                            ));
                        }
                    };
...
fn is_ty_const_destruct<'tcx>(tcx: TyCtxt<'tcx>, ty: Ty<'tcx>, body: &Body<'tcx>) -> bool {
    ...
    fn is_ty_const_destruct_unused<'tcx>(
        ...
        let obligation = Obligation::new(
            ...
            TraitRef::new(
                tcx,
                tcx.require_lang_item(LangItem::Destruct, body.span),
                [ty],
            ),
        );
