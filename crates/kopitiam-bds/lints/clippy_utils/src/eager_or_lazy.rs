...
fn fn_eagerness(
    ...
    if (matches!(name, sym::is_empty | sym::len) || name.as_str().starts_with("as_"))
        && have_one_arg
    {
        if matches!(
    ...
    impl<'tcx> Visitor<'tcx> for V<'_, 'tcx> {
        fn visit_expr(&mut self, e: &'tcx Expr<'_>) {
            ...
            match e.kind {
                ...
                // arm pattern can't be merged due to `ref`, see rust#105778
                ...
                ExprKind::Unary(UnOp::Neg, right)
                    if ConstEvalCtxt::new(self.cx).eval(right).is_none() =>
                {
                    self.eagerness |= NoChange;
                }
                ...
                ExprKind::Binary(op, left, right)
                    if matches!(op.node, BinOpKind::Div | BinOpKind::Rem)
                        ...
                        && let right = ecx
                            .eval(right)
                            .and_then(|c| c.int_value(self.cx.tcx, right_ty))
                        && matches!(
                            (left, right),
                            // `1 / x`: x might be zero
                            (_, None)
