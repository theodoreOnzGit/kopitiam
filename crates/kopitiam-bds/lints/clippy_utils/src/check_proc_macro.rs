...
use rustc_ast::ast::{
    AttrKind, Attribute, GenericArgs, IntTy, LitIntType, LitKind, StrStyle, TraitObjectSyntax,
    UintTy,
};
use rustc_ast::token::CommentKind;
use rustc_hir::intravisit::FnKind;
use rustc_hir::{
    Block, BlockCheckMode, Body, Closure, Destination, Expr, ExprKind, FieldDef, FnHeader, FnRetTy,
    HirId, Impl, ImplItem, ImplItemImplKind, ImplItemKind, IsAuto, Item, ItemKind, Lit, LoopSource,
    MatchSource, MutTy, Node, Path, QPath, Safety, TraitImplHeader, TraitItem, TraitItemKind, Ty,
    TyKind, UnOp, UnsafeSource, Variant, VariantData, YieldSource,
};
    ...
    fn expr_search_pat_inner(tcx: TyCtxt<'_>, e: &Expr<'_>, outer_span: Span) -> (Pat, Pat) {
        ...
        match e.kind {
            ...
            ExprKind::Unary(UnOp::Deref, e) => {
                (Pat::Str("*"), expr_search_pat_inner(tcx, e, outer_span).1)
            }
            ...
            ExprKind::Tup([e]) | ExprKind::DropTemps(e) => {
                expr_search_pat_inner(tcx, e, outer_span)
            }
            ExprKind::Cast(e, _) | ExprKind::Type(e, _) => {
                (expr_search_pat_inner(tcx, e, outer_span).0, Pat::Str(""))
            }
            ExprKind::Let(let_expr) => (
                Pat::Str("let"),
                expr_search_pat_inner(tcx, let_expr.init, outer_span).1,
            ),
            ...
            | ExprKind::Yield(e, YieldSource::Await { .. }) => (
                expr_search_pat_inner(tcx, e, outer_span).0,
                Pat::Str("await"),
            ),
            ...
            ExprKind::Break(_, Some(e)) => (
                Pat::Str("break"),
                expr_search_pat_inner(tcx, e, outer_span).1,
            ),
            ...
            ExprKind::Yield(e, YieldSource::Yield) => (
                Pat::Str("yield"),
                expr_search_pat_inner(tcx, e, outer_span).1,
            ),
            _ => (Pat::Str(""), Pat::Str("")),
...
fn ty_search_pat(ty: &Ty<'_>) -> (Pat, Pat) {
    match ty.kind {
        ...
        TyKind::UnsafeBinder(binder_ty) => {
            (Pat::Str("unsafe"), ty_search_pat(binder_ty.inner_ty).1)
        }
