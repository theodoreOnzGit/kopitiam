...
use rustc_span::{
    BytePos, DUMMY_SP, DesugaringKind, Pos, RelativeBytePos, SourceFile, SourceFileAndLine, Span,
    SpanData, SyntaxContext, hygiene,
};
use std::borrow::Cow;
use std::fmt;
...
fn map_range(
        ...
        && let Some(range) = f(&src.sf, text, src.range.clone())
    {
        ...
        debug_assert!(
            ...
            "Range `{range:?}` has overlapping bounds"
        );
...
fn with_leading_whitespace_inner(
    lines: &[RelativeBytePos],
    src: &str,
    range: Range<usize>,
) -> Option<usize> {
...
pub fn expr_block(
    ...
    let (code, from_macro) =
        snippet_block_with_context(sess, expr.span, outer, default, indent_relative_to, app);
    if !from_macro
...
pub fn indent_of(sess: &impl HasSession, span: Span) -> Option<usize> {
    snippet_opt(sess, line_span(sess, span))
...
fn snippet_with_context_sess<'a>(
    ...
    if span.desugaring_kind() == Some(DesugaringKind::RangeExpr)
        && span.parent_callsite().unwrap().ctxt() == outer
    {
        return (
