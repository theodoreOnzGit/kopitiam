...
use rustc_hir::{
    self as hir, Expr, ExprKind, HirId, LangItem, Pat, PatExpr, PatExprKind, PatKind, Path,
...
pub trait MaybeTypeckRes<'tcx> {
    ...
    fn ty_based_def(&self, node: impl HasHirId) -> Option<DefRes> {
        ...
        self.typeck_res()
            .and_then(|typeck| f(typeck, node.hir_id()))
    }
}
impl<'tcx> MaybeTypeckRes<'tcx> for LateContext<'tcx> {
    ...
    fn typeck_res(&self) -> Option<&TypeckResults<'tcx>> {
        if let Some(typeck) = self.maybe_typeck_results() {
            ...
            debug_assert!(
                false,
                "attempted type-dependent lookup in a non-body context"
            );
            None
        }
    }
...
pub trait MaybeQPath<'a>: Copy {
    ...
    fn res_if_named<'tcx>(self, typeck: &impl MaybeTypeckRes<'tcx>, name: Symbol) -> Res {
        ...
        match self.opt_qpath() {
            Some((qpath, id)) if let Some(typeck) = typeck.typeck_res() => {
                f(qpath, id, typeck, name)
            }
            _ => Res::Err,
        }
    }
    ...
    fn ty_rel_def_if_named<'tcx>(
        ...
        match self.opt_qpath() {
            Some((&QPath::TypeRelative(_, seg), id)) if seg.ident.name == name => {
                typeck.ty_based_def(id)
            }
            _ => None,
        }
    }
    ...
    fn ty_rel_def_with_seg<'tcx>(
        self,
        typeck: &impl MaybeTypeckRes<'tcx>,
    ) -> Option<(DefRes, &'a PathSegment<'a>)> {
        match self.opt_qpath() {
            Some((QPath::TypeRelative(_, seg), id)) if let Some(def) = typeck.ty_based_def(id) => {
                Some((def, seg))
            }
            _ => None,
        }
...
pub trait MaybeDef: Copy {
    ...
    fn opt_diag_name<'tcx>(self, tcx: &impl HasTyCtxt<'tcx>) -> Option<Symbol> {
        self.opt_def_id()
            .and_then(|id| tcx.tcx().get_diagnostic_name(id))
    }
    ...
    fn assoc_parent<'tcx>(self, tcx: &impl HasTyCtxt<'tcx>) -> Option<DefId> {
        match self.opt_def(tcx) {
            Some((DefKind::AssocConst | DefKind::AssocFn | DefKind::AssocTy, id)) => {
                tcx.tcx().opt_parent(id)
            }
            _ => None,
        }
    }
