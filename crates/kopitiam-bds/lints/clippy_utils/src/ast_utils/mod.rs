...
pub fn unordered_over<X, Y>(
    left: &[X],
    right: &[Y],
    mut eq_fn: impl FnMut(&X, &Y) -> bool,
) -> bool {
...
pub fn eq_pat(l: &Pat, r: &Pat) -> bool {
    ...
    match (&l.kind, &r.kind) {
        ...
        (Ref(l, l_pin, l_mut), Ref(r, r_pin, r_mut)) => {
            l_pin == r_pin && l_mut == r_mut && eq_pat(l, r)
        }
        (Tuple(l), Tuple(r)) | (Slice(l), Slice(r)) => over(l, r, eq_pat),
        (Path(lq, lp), Path(rq, rp)) => {
            both(lq.as_deref(), rq.as_deref(), eq_qself) && eq_path(lp, rp)
        }
...
pub fn eq_path_seg(l: &PathSegment, r: &PathSegment) -> bool {
    eq_id(l.ident, r.ident)
        && both(l.args.as_ref(), r.args.as_ref(), |l, r| {
            eq_generic_args(l, r)
        })
}
...
pub fn eq_expr(l: &Expr, r: &Expr) -> bool {
    ...
    match (&l.kind, &r.kind) {
        ...
        (Loop(lt, ll, _), Loop(rt, rl, _)) => {
            eq_label(ll.as_ref(), rl.as_ref()) && eq_block(lt, rt)
        }
        (Block(lb, ll), Block(rb, rl)) => eq_label(ll.as_ref(), rl.as_ref()) && eq_block(lb, rb),
        (TryBlock(lb, lt), TryBlock(rb, rt)) => {
            eq_block(lb, rb) && both(lt.as_deref(), rt.as_deref(), eq_ty)
        }
        (Yield(l), Yield(r)) => {
            eq_expr_opt(l.expr().map(Box::as_ref), r.expr().map(Box::as_ref)) && l.same_kind(r)
        }
        ...
        (Match(ls, la, lkind), Match(rs, ra, rkind)) => {
            (lkind == rkind) && eq_expr(ls, rs) && over(la, ra, eq_arm)
        }
        (
            ...
            eq_generics(lg, rg)
                && both(lot.as_deref(), rot.as_deref(), |l, r| {
                    matches!(l.safety, Safety::Default) == matches!(r.safety, Safety::Default)
                        && matches!(l.polarity, ImplPolarity::Positive)
                            == matches!(r.polarity, ImplPolarity::Positive)
...
#[expect(
    clippy::ref_option,
    reason = "This is the type how it is stored in the AST"
)]
...
pub fn eq_use_tree_kind(l: &UseTreeKind, r: &UseTreeKind) -> bool {
    ...
    match (l, r) {
        ...
        (Nested { items: l, .. }, Nested { items: r, .. }) => {
            over(l, r, |(l, _), (r, _)| eq_use_tree(l, r))
        }
        _ => false,
    }
}
...
pub fn eq_ty(l: &Ty, r: &Ty) -> bool {
    ...
    match (&l.kind, &r.kind) {
        ...
        (Ref(ll, l), Ref(rl, r)) => {
            both(ll.as_ref(), rl.as_ref(), |l, r| eq_id(l.ident, r.ident))
                && l.mutbl == r.mutbl
                && eq_ty(&l.ty, &r.ty)
        }
...
pub fn eq_generic_param(l: &GenericParam, r: &GenericParam) -> bool {
    ...
    l.is_placeholder == r.is_placeholder
        ...
        && match (&l.kind, &r.kind) {
            ...
            (Type { default: l }, Type { default: r }) => {
                both(l.as_ref(), r.as_ref(), |l, r| eq_ty(l, r))
            }
            (
                Const {
                    ty: lt,
...
pub fn eq_precise_capture(l: &PreciseCapturingArg, r: &PreciseCapturingArg) -> bool {
    match (l, r) {
        ...
        (PreciseCapturingArg::Arg(l, _), PreciseCapturingArg::Arg(r, _)) => {
            l.segments[0].ident == r.segments[0].ident
        }
        _ => false,
    }
}
