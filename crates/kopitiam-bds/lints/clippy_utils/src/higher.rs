...
use rustc_hir::{
    Arm, Block, Expr, ExprKind, HirId, LoopSource, MatchSource, Node, Pat, QPath, StructTailExpr,
};
        ...
        match expr.kind {
            ExprKind::Call(path, [arg1, arg2])
                if let ExprKind::Path(qpath) = path.kind
                    && cx
                        .tcx
                        .qpath_is_lang_item(qpath, hir::LangItem::RangeInclusiveNew) =>
            {
                Some(Range {
                    start: Some(arg1),
                    end: Some(arg2),
            ...
            ExprKind::Struct(&qpath, fields, StructTailExpr::None) => {
                match (cx.tcx.qpath_lang_item(qpath)?, fields) {
                    ...
                    (hir::LangItem::RangeTo, [field]) if field.ident.name == sym::end => {
                        Some(Range {
                            start: None,
                            end: Some(field.expr),
                            limits: ast::RangeLimits::HalfOpen,
    ...
    if let ExprKind::Call(func, args) = expr.kind {
        match func.kind {
            ...
            ExprKind::Path(QPath::Resolved(_, path))
                if cx
                    .tcx
                    .is_diagnostic_item(sym::default_fn, path.res.opt_def_id()?)
