...
use rustc_hir::{
    BinOpKind, Block, ConstArgKind, ConstBlock, ConstItemRhs, Expr, ExprKind, HirId, PatExpr,
    PatExprKind, QPath, TyKind, UnOp,
};
...
impl Constant {
    ...
    pub fn new_numeric_max<'tcx>(tcx: TyCtxt<'tcx>, ty: Ty<'tcx>) -> Option<Self> {
        match *ty.kind() {
            ty::Uint(ty) => Some(Self::Int(
                match ty.normalize(tcx.sess.target.pointer_width) {
...
impl<'tcx> ConstEvalCtxt<'tcx> {
    ...
    fn expr(&self, e: &Expr<'_>) -> Option<Constant> {
        ...
        match e.kind {
            ExprKind::ConstBlock(ConstBlock { body, .. }) => {
                self.expr(self.tcx.hir_body(body).value)
            }
            ...
            ExprKind::Call(callee, []) => {
                    ...
                    && let Some(did) = self.typeck.qpath_res(qpath, callee.hir_id).opt_def_id()
                {
                    match self.tcx.get_diagnostic_name(did) {
                        ...
                        Some(sym::i128_legacy_fn_max_value) => {
                            Some(Constant::Int(i128::MAX as u128))
                        }
                        _ => None,
                    }
                } else {
    ...
    pub fn eval_is_empty(&self, e: &Expr<'_>) -> Option<bool> {
        match e.kind {
            ExprKind::ConstBlock(ConstBlock { body, .. }) => {
                self.eval_is_empty(self.tcx.hir_body(body).value)
            }
            ...
            QPath::Resolved(None, path)
                if path.span.ctxt() == self.ctxt.get()
                    && path
                        .segments
                        .iter()
                        .all(|s| self.ctxt.get() == s.ident.span.ctxt())
                ...
                if let TyKind::Path(QPath::Resolved(None, ty_path)) = ty.kind
                    ...
                    && (matches!(
                        ...
                        sym::i8
                            ...
                            | sym::f64
                            | sym::char
                    ) || (ty_name.ident.name == sym::usize
                        && const_name.ident.name == sym::MIN))
                    ...
                    && let Some((DefKind::AssocConst, did)) =
                        self.typeck.type_dependent_def(id) =>
            {
                did
            }
    ...
    fn binop(&self, op: BinOpKind, left: &Expr<'_>, right: &Expr<'_>) -> Option<Constant> {
        ...
        match (l, r) {
            (Constant::Int(l), Some(Constant::Int(r))) => {
                match *self.typeck.expr_ty_opt(left)?.kind() {
                    ty::Int(ity) => {
                        ...
                        match op {
                            ...
                            BinOpKind::Add => {
                                l.checked_add(r).and_then(|n| ity.ensure_fits(n)).map(zext)
                            }
                            BinOpKind::Sub => {
                                l.checked_sub(r).and_then(|n| ity.ensure_fits(n)).map(zext)
                            }
                            BinOpKind::Mul => {
                                l.checked_mul(r).and_then(|n| ity.ensure_fits(n)).map(zext)
                            }
                            ...
                            BinOpKind::Shr if r < bits && !r.is_negative() => {
                                l.checked_shr(r.try_into().ok()?).map(zext)
                            }
                            BinOpKind::Shl if r < bits && !r.is_negative() => {
                                l.checked_shl(r.try_into().ok()?).map(zext)
                            }
                    ...
                    ty::Uint(ity) => {
                        ...
                        match op {
                            ...
                            BinOpKind::Shr if r < bits => {
                                l.checked_shr(r.try_into().ok()?).map(Constant::Int)
                            }
                            BinOpKind::Shl if r < bits => {
                                l.checked_shl(r.try_into().ok()?).map(Constant::Int)
                            }
    ...
    match (val, ty.kind()) {
        ...
        (ConstValue::Indirect { alloc_id, offset }, ty::Array(sub_type, len)) => {
            ...
            for idx in 0..len {
                ...
                let val = alloc
                    .read_scalar(&tcx, range, /* read_provenance */ false)
                    .ok()?;
                res.push(match flt {
