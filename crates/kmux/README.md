...
`crates/*` and therefore sees exactly one member — `kmux` — while the
twelve forked sub-crates live under `crates/kmux/crates/` and are pulled
...
```sh
cargo build --release -p kmux      # produces target/release/kmux
cargo test  --release -p kmux
```
...
```sh
cargo check --release -p kmux --target aarch64-linux-android
```
