...
#[cfg(windows)]
use crate::proto::DaemonStatusResponse;
use crate::proto::{Response, RmuxError};
#[cfg(unix)]
use crate::sdk::bootstrap::startup_unix::{
    connect_or_start_with, StartupError, StartupOutcome, DEFAULT_STARTUP_DEADLINE,
    STARTUP_POLL_INTERVAL,
};
#[cfg(windows)]
use crate::sdk::bootstrap::startup_windows::{
    connect_or_start_blocking_with, StartupError, StartupOutcome, DEFAULT_STARTUP_DEADLINE,
    STARTUP_POLL_INTERVAL,
};

use crate::client::shell_quote::shell_quote_path;
#[cfg(any(all(test, unix), not(any(unix, windows))))]
use crate::client::ConnectResult;
use crate::client::{default_socket_path, upgrade, ClientError, Connection};
...
/// [`crate::client::connect`] or [`crate::client::connect_or_absent`] directly so they do not
...
fn probe_server_readiness_status(
    ...
    match response {
        ...
        other => Err(ClientError::Protocol(crate::proto::RmuxError::Server(
...
fn ensure_server_running_polling(
    ...
    if config.loads_startup_config() {
        return ensure_server_running_with_probe(
            ...
            AUTO_START_TIMEOUT,
            POLL_INTERVAL,
            || crate::client::connect_or_absent(socket_path),
    ...
    ProbeFn: FnMut(&mut Connection) -> Result<(), ClientError>,
{
    ...
    loop {
        match connect() {
            Ok(crate::client::ConnectResult::Connected(mut connection)) => match probe(&mut connection) {
                Ok(()) => return Ok(connection),
                Err(error) if is_transient_connect_error(&error) => {}
                Err(error) => return Err(AutoStartError::Client(error)),
            },
            Ok(crate::client::ConnectResult::Absent) => {}
            Err(error) if is_transient_connect_error(&error) => {}
            Err(error) => return Err(AutoStartError::Client(error)),
        }
...
#[cfg(windows)]
fn append_startup_ready_event(command: &mut Command, ready: &crate::os::daemon::StartupReadyEvent) {
...
fn hidden_daemon_command_preserving_fd(
    ...
    crate::os::daemon::configure_hidden_daemon_command_preserving_fds(
        &mut command,
        allow_job_breakaway,
        &[preserved_fd],
    );
