...
fn default_socket_path_matches_server_path() {
    ...
    let server_path = crate::server::default_socket_path().expect("server socket path");
    assert_eq!(
...
fn roundtrip_reads_partial_response_frames() {
    let request = Request::HasSession(HasSessionRequest {
        target: crate::proto::SessionName::new("alpha").expect("valid session"),
    });
...
fn legacy_wire_v1_request_writer_uses_v1_shutdown_envelope() {
    ...
    connection
        .write_legacy_wire_v1_request(&Request::KillServer(crate::proto::KillServerRequest))
