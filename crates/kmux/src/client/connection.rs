...
use crate::ipc::{connect_blocking, BlockingLocalStream, LocalEndpoint};
use crate::proto::{
...
pub fn socket_path_for_label(label: impl AsRef<OsStr>) -> Result<PathBuf, ClientError> {
    crate::ipc::endpoint_for_label(label)
        .map(LocalEndpoint::into_path)
        .map_err(ClientError::Io)
}
...
pub fn resolve_socket_path(
    ...
    crate::ipc::resolve_endpoint(socket_name, socket_path)
        .map(LocalEndpoint::into_path)
        .map_err(ClientError::Io)
}
...
pub fn resolve_tmux_compatible_socket_path(
    ...
    crate::ipc::resolve_tmux_compatible_endpoint(socket_name, socket_path)
        .map(LocalEndpoint::into_path)
        .map_err(ClientError::Io)
}
