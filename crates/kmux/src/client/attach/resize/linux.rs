...
use crate::proto::TerminalGeometry;
...
#[derive(Debug)]
pub(in crate::client::attach) struct SignalMaskGuard {
    previous: KernelSigSet,
}

impl SignalMaskGuard {
    pub(in crate::client::attach) fn block_winch() -> super::Result<Self> {
...
#[derive(Debug)]
pub(in crate::client::attach) struct ResizeWatcher {
    stop: Arc<AtomicBool>,
...
impl ResizeWatcher {
    pub(in crate::client::attach) fn spawn(
        terminal_fd: OwnedFd,
    ...
    #[cfg(test)]
    pub(in crate::client::attach) fn notify_for_test(&self) -> rustix::io::Result<()> {
