...
use crate::core::WINDOW_SILENCE;
use crate::proto::{types::OptionScopeSelector, SessionName, WindowTarget};

use super::super::RequestHandler;
use super::{monitor_silence_seconds, SilenceTimerState};

impl RequestHandler {
    pub(in crate::server::handler) async fn sync_session_silence_timers(&self, session_name: &SessionName) {
        let desired = {
    ...
    pub(in crate::server::handler) async fn sync_all_silence_timers(&self) {
        let desired = {
            let state = self.state.lock().await;
            state
                .sessions
    ...
    pub(in crate::server::handler) async fn cancel_session_silence_timers(
        &self,
    ...
    pub(in crate::server::handler) async fn sync_alert_timers_for_option_scope(
        &self,
