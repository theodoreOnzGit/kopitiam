...
use crate::proto::{PaneTarget, SessionName, Target, WindowTarget};

use super::super::{scripting_support::format_context_for_target, RequestHandler};
use crate::server::format_runtime::render_automatic_window_name;
use crate::server::pane_terminals::HandlerState;

impl RequestHandler {
    pub(in crate::server::handler) async fn refresh_automatic_window_name_for_pane_target(
        &self,
        target: &PaneTarget,
    ) -> bool {
    ...
    pub(in crate::server::handler) async fn sync_automatic_window_name_for_pane_target(
        &self,
        target: &PaneTarget,
    ) -> bool {
        !self
    ...
    pub(in crate::server::handler) async fn refresh_automatic_window_name_for_window_target(
        &self,
        target: &WindowTarget,
    ) -> bool {
