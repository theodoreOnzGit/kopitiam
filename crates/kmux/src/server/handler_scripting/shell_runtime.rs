...
use crate::core::{command_parser::ParsedCommands, formats::is_truthy, PaneId};
use crate::proto::{
...
use crate::server::hook_runtime::current_hook_formats;
use crate::server::terminal::{SessionBaseEnvironment, TerminalProfile};

impl RequestHandler {
    pub(in crate::server::handler) async fn handle_run_shell(
        &self,
        requester_pid: u32,
    ...
    pub(in crate::server::handler) async fn handle_run_shell_with_client_name(
        &self,
        requester_pid: u32,
        request: RunShellRequest,
        ...
        if request.background {
            ...
            let hook_context_active = crate::server::hook_runtime::hooks_disabled();
            if let Err(error) = spawn_background_async("rmux-run-shell", move || async move {
                ...
                if hook_context_active {
                    crate::server::hook_runtime::with_hook_execution(hook_formats, task).await;
                } else {
                    task.await;
                }
            }) {
    ...
    pub(in crate::server::handler) async fn handle_if_shell(
        &self,
        requester_pid: u32,
    ...
    pub(in crate::server::handler) async fn handle_if_shell_with_client_name(
        &self,
        requester_pid: u32,
        request: IfShellRequest,
        client_name: Option<String>,
