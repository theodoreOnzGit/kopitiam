...
use crate::proto::{RmuxError, DEFAULT_MAX_FRAME_LENGTH};
#[cfg(unix)]
...
impl ShellTaskProfile {
    ...
    fn shell_command(&self, command: &str) -> StdCommand {
        let mut child = crate::server::terminal::shell_std_command(&self.shell, &self.cwd, command);
...
#[cfg(windows)]
struct ShellTaskProcessGroup {
    job: Option<crate::os::process::ProcessJob>,
}
...
impl ShellTaskProcessGroup {
    fn from_child(child: &Child) -> Self {
        Self {
            job: child
                .raw_handle()
                .and_then(|handle| crate::os::process::ProcessJob::for_raw_handle(handle).ok()),
        }
    }
...
mod tests {
    ...
    use crate::proto::{OptionName, ScopeSelector, SetOptionMode};
    ...
    async fn timeout_kills_descendant_process_holding_stdout() {
        ...
        assert!(
            !crate::os::process::is_live(child_pid),
