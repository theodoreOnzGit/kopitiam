use crate::core::{SessionStore, TargetFindContext};
...
pub(super) fn parse_set_option_invocation(
    ...
    if let Err(error) = crate::core::resolve_option_name_typed(&option) {
...
fn should_defer_set_option_value_validation(option: &str, value: Option<&str>) -> bool {
    ...
    crate::core::option_name_by_name(option) == Some(OptionName::ExtendedKeys)
}
...
fn resolve_set_option_scope(
    ...
    crate::core::resolve_option_name(option)?;
    let is_user = option
        .split('[')
        .next()
        .is_some_and(|base| base.starts_with('@'));
    let supports_scope = |scope: &OptionScopeSelector| {
        crate::core::validate_option_name_mutation(option, scope, SetOptionMode::Replace, None, true)
            .is_ok()
    };
    ...
    if global {
        let scope = crate::core::default_global_scope_for_option_name(option)?;
        if !is_user && !supports_scope(&scope) {
            return Err(RmuxError::Server(
...
fn resolve_show_options_global_scope(
    ...
    match crate::core::default_global_scope_for_option_name(name) {
        Ok(scope) => Ok(scope),
