...
fn resolve_queue_placement_anchor_target(
    ...
    let target = if let Some(session_value) = session_value {
        ...
        let window_index = sessions
            ...
            .ok_or_else(|| crate::server::pane_terminals::session_not_found(&session_name))?
            .active_window_index();
...
pub(in crate::server::handler::scripting_support) fn bare_link_window_destination_candidate(
    value: &str,
) -> bool {
    !value.is_empty()
        && !value.contains([':', '.'])
...
pub(super) fn queue_target_find_context(
    ...
    crate::server::handler::with_visible_pane_bases(context, input.sessions, input.options)
}
...
pub(super) fn parse_layout_name(value: &str) -> Result<crate::proto::LayoutName, RmuxError> {
    value.parse()
}

pub(super) fn is_unsupported_named_layout(layout: crate::proto::LayoutName) -> bool {
    matches!(
        layout,
        crate::proto::LayoutName::MainHorizontalMirrored
            | crate::proto::LayoutName::MainVerticalMirrored
    )
}
    ...
    fn session_store_with_alpha_beta() -> SessionStore {
        ...
        for name in ["alpha", "beta"] {
            sessions
                .create_session(
                    session_name(name),
                    crate::proto::TerminalSize { cols: 80, rows: 24 },
                )
