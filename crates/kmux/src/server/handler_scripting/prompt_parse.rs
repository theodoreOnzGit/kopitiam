...
use crate::core::command_parser::{CommandArgument, ParsedCommand};
...
pub(in crate::server::handler) enum PromptHistoryAction {
    Show,
    Clear,
}

#[derive(Debug, Clone)]
pub(in crate::server::handler) struct ParsedPromptHistoryCommand {
    pub(in crate::server::handler) action: PromptHistoryAction,
    pub(in crate::server::handler) prompt_type: Option<PromptType>,
}
