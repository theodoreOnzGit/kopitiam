...
pub(in crate::server::handler::scripting_support) fn parse_set_hook(
...
pub(in crate::server::handler::scripting_support) fn parse_show_hooks(
    mut args: CommandTokens,
) -> Result<Request, RmuxError> {
    let mut global = false;
    let mut window = false;
...
fn resolve_hook_scope(
    ...
    crate::core::validate_hook_registration(hook, &scope)?;
    Ok(scope)
}
...
fn resolve_implicit_hook_target_scope(
    ...
    for scope in candidates {
        match crate::core::validate_hook_registration(hook, &scope) {
            Ok(()) => return Ok(scope),
