...
use crate::proto::{CommandOutput, ErrorResponse, Request, Response, RmuxError, Target};

use crate::server::mouse::AttachedMouseEvent;
...
#[derive(Debug, Clone)]
pub(in crate::server::handler) struct QueueExecutionContext {
...
impl QueueExecutionContext {
    pub(in crate::server::handler) fn new(caller_cwd: Option<PathBuf>) -> Self {
        Self {
            caller_cwd,
            source_file_depth: 0,
            current_file: None,
    ...
    pub(in crate::server::handler) fn without_caller_cwd() -> Self {
        Self {
            caller_cwd: None,
            source_file_depth: 0,
            current_file: None,
    ...
    pub(in crate::server::handler) fn for_sourced_commands(
        &self,
    ...
    pub(in crate::server::handler) fn with_current_target(
        mut self,
    ...
    pub(in crate::server::handler) fn with_implicit_current_target(
        mut self,
        current_target: Option<Target>,
    ) -> Self {
        self.current_target = current_target;
    ...
    pub(in crate::server::handler) fn uses_explicit_current_target(&self) -> bool {
        self.current_target_allows_canfail_fallback
    }

    pub(in crate::server::handler) fn with_client_name(mut self, client_name: Option<String>) -> Self {
        self.client_name = client_name;
        self
    }

    pub(in crate::server::handler) fn with_mouse_target(mut self, mouse_target: Option<Target>) -> Self {
        self.mouse_target = mouse_target;
        self
    }

    pub(in crate::server::handler) fn with_mouse_event(
        mut self,
    ...
    pub(in crate::server::handler) fn current_target(&self) -> Option<&Target> {
        self.current_target.as_ref()
    }

    pub(in crate::server::handler) fn canfail_fallback_target(&self) -> Option<&Target> {
        self.current_target_allows_canfail_fallback
...
#[derive(Debug, Clone)]
pub(in crate::server::handler) enum QueueCommandAction {
    Normal {
...
pub(super) enum QueueInvocation {
    ...
    MouseResizePane(crate::proto::PaneTarget),
