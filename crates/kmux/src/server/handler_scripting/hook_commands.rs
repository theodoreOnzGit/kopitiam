use crate::core::command_parser::ParsedCommands;
use crate::proto::{RmuxError, SessionName, Target};
...
use crate::server::terminal::{spawn_hook_command_with_profile, TerminalProfile};

impl RequestHandler {
    #[cfg_attr(not(test), allow(dead_code))]
    pub(in crate::server::handler) async fn execute_hook_command(
        &self,
        requester_pid: u32,
        command: &str,
    ) -> Result<(), RmuxError> {
    ...
    pub(in crate::server::handler) async fn execute_hook_command_with_context(
        &self,
        requester_pid: u32,
        command: &str,
