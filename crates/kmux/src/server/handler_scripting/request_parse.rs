...
use crate::proto::{Request, RmuxError};
...
fn parse_no_argument_request(args: CommandTokens, command: &str) -> Result<Request, RmuxError> {
    ...
    match command {
        "kill-server" => Ok(Request::KillServer(crate::proto::KillServerRequest)),
        "lock-server" => Ok(Request::LockServer(crate::proto::LockServerRequest)),
        other => Err(RmuxError::Server(format!(
            "unsupported command in queue: {other}"
        ))),
    }
