use crate::proto::{OptionName, RmuxError};
...
use crate::server::renderer::RenderedPrompt;

impl RequestHandler {
    pub(in crate::server::handler) async fn start_command_prompt(
        &self,
    ...
    pub(in crate::server::handler) async fn start_confirm_before(
        &self,
    ...
    #[allow(dead_code)]
    pub(in crate::server::handler) async fn attached_prompt_render(
        &self,
        attach_pid: u32,
    ...
    pub(in crate::server::handler) async fn prompt_active(&self, attach_pid: u32) -> bool {
        let active_attach = self.active_attach.lock().await;
        active_attach
            .by_pid
            .get(&attach_pid)
    ...
    pub(in crate::server::handler) async fn handle_prompt_event_deferred_refresh(
        &self,
        attach_pid: u32,
    ...
    pub(in crate::server::handler) async fn flush_attached_prompt_refresh(
        &self,
        attach_pid: u32,
    ) -> Result<(), RmuxError> {
    ...
    pub(in crate::server::handler) async fn clear_prompt_for_attach(&self, attach_pid: u32) {
        let finished = {
            let mut active_attach = self.active_attach.lock().await;
            active_attach
                .by_pid
    ...
    pub(in crate::server::handler) async fn execute_queued_prompt_history(
        &self,
        ...
        match command.action {
            PromptHistoryAction::Show => {
                ...
                Ok(QueueCommandAction::Normal {
                    output: Some(crate::proto::CommandOutput::from_stdout(body.into_bytes())),
                    error: None,
                    exit_status: None,
                })
            }
    ...
    async fn parse_prompt_commands(
        ...
        responses: &[String],
    ) -> Result<crate::core::command_parser::ParsedCommands, RmuxError> {
