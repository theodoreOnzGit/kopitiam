...
use crate::core::{encode_paste_bytes, GridRenderOptions, LifecycleEvent, ScreenCaptureRange};
use crate::proto::{
...
use crate::server::pane_terminals::{session_not_found, PaneCaptureRequest};
...
impl RequestHandler {
    pub(super) async fn handle_set_buffer(
        &self,
        requester_pid: u32,
        request: crate::proto::SetBufferRequest,
    ) -> Response {
    ...
    pub(super) async fn handle_show_buffer(
        &self,
        request: crate::proto::ShowBufferRequest,
    ) -> Response {
    ...
    pub(super) async fn handle_paste_buffer(
        &self,
        request: crate::proto::PasteBufferRequest,
    ) -> Response {
        ...
        let (buffer_name, content, buffer_order, master, bracketed_mode) = {
            ...
            let bracketed_mode = request.bracketed
                && state
                    ...
                    .is_some_and(|state| {
                        state.mode & crate::core::input::mode::MODE_BRACKETPASTE != 0
                    });
    ...
    pub(super) async fn handle_list_buffers(
        &self,
        request: crate::proto::ListBuffersRequest,
    ) -> Response {
        let state = self.state.lock().await;
    ...
    pub(super) async fn handle_delete_buffer(
        &self,
        request: crate::proto::DeleteBufferRequest,
    ) -> Response {
        let mut state = self.state.lock().await;
    ...
    pub(super) async fn handle_load_buffer(
        &self,
        requester_pid: u32,
        request: crate::proto::LoadBufferRequest,
    ) -> Response {
    ...
    pub(super) async fn handle_save_buffer(
        &self,
        request: crate::proto::SaveBufferRequest,
    ) -> Response {
    ...
    pub(super) async fn handle_capture_pane(
        &self,
        request: crate::proto::CapturePaneRequest,
    ) -> Response {
        let content = {
            let mut state = self.state.lock().await;
    ...
    pub(super) async fn handle_clear_history(
        &self,
        request: crate::proto::ClearHistoryRequest,
    ) -> Response {
        let mut state = self.state.lock().await;
...
fn capture_render_options(request: &crate::proto::CapturePaneRequest) -> GridRenderOptions {
...
fn parse_buffer_limit(state: &crate::server::pane_terminals::HandlerState) -> usize {
    state
        .options
...
fn render_paste_payload(content: &[u8], request: &crate::proto::PasteBufferRequest) -> Vec<u8> {
