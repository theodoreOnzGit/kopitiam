...
use crate::server::control::ControlCommandResult;
...
pub(in crate::server::handler) use self::parser_context::command_parser_from_state;
...
impl RequestHandler {
    ...
    pub(in crate::server::handler) async fn start_attached_prompt_binding_commands(
        &self,
        requester_pid: u32,
        commands: &ParsedCommands,
        context: &QueueExecutionContext,
    ...
    async fn execute_queued_command(
        ...
        let result = match invocation {
            ...
            QueueInvocation::Request(request) => {
                ...
                let request = crate::server::server_access::apply_access_policy(request, can_write)?;
    ...
    async fn execute_queued_list_panes_all(
        ...
        for session_name in session_names {
            let response = self
                .handle_list_panes(crate::proto::ListPanesRequest {
                    target: session_name,
...
impl RequestHandler {
    async fn control_queue_action_from_outcome(
        ...
        requester_pid: u32,
        request: Request,
        outcome: crate::server::pane_io::HandleOutcome,
    ) -> Result<QueueCommandAction, RmuxError> {
