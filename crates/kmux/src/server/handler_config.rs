...
impl RequestHandler {
    pub(super) async fn handle_set_environment(
        &self,
        request: crate::proto::SetEnvironmentRequest,
    ) -> Response {
        if !matches!(
            request.scope,
    ...
    pub(super) async fn handle_set_hook(&self, request: crate::proto::SetHookRequest) -> Response {
    ...
    async fn handle_set_hook_mutation_inner(&self, request: SetHookMutationRequest) -> Response {
        ...
        if request.run_immediately {
            ...
            self.queue_inline_hook(
                ...
                request.scope.clone(),
                current_target,
                crate::server::hook_runtime::PendingInlineHookFormat::HookOnly,
            );
    ...
    pub(super) async fn handle_show_options(
        &self,
        request: crate::proto::ShowOptionsRequest,
    ) -> Response {
        let state = self.state.lock().await;
        ...
        let mode = if request.include_inherited {
            crate::core::ShowOptionsMode::ResolvedWithInheritanceMarkers
        } else if matches!(
            request.scope,
            ...
            crate::core::ShowOptionsMode::Resolved
        } else {
            crate::core::ShowOptionsMode::Explicit
        };
    ...
    pub(super) async fn handle_show_environment(
        &self,
        request: crate::proto::ShowEnvironmentRequest,
    ) -> Response {
        if !matches!(
            request.scope,
            ScopeSelector::Global | ScopeSelector::Session(_)
    ...
    pub(super) async fn handle_show_hooks(
        &self,
        request: crate::proto::ShowHooksRequest,
    ) -> Response {
        let state = self.state.lock().await;
...
fn show_options_session_name(scope: &OptionScopeSelector) -> Option<&crate::proto::SessionName> {
    match scope {
...
fn render_show_environment_entry(
    entry: &crate::core::ShowEnvironmentEntry,
    shell_format: bool,
) -> Option<String> {
    if !shell_format {
...
enum ShowHooksSelection {
    GlobalSession,
    GlobalWindow,
    Session(crate::proto::SessionName),
    Window(WindowTarget),
    Pane(crate::proto::PaneTarget),
}
...
fn render_global_hook_lines(
    store: &HookStore,
    root: HookGlobalRoot,
    filter: Option<crate::proto::HookName>,
) -> Vec<String> {
