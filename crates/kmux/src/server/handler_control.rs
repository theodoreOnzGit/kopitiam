...
use crate::server::control::{ControlClientFlags, ControlModeUpgrade, ControlServerEvent};
use crate::server::control_notifications::{
...
pub(super) struct ActiveControl {
    pub(super) id: u64,
    pub(super) session_name: Option<crate::proto::SessionName>,
    pub(super) last_session: Option<crate::proto::SessionName>,
...
impl RequestHandler {
    ...
    pub(super) async fn attached_count(&self, session_name: &crate::proto::SessionName) -> usize {
        let attach_count = {
    ...
    pub(super) async fn resolve_managed_client(
        ...
        requester_pid: u32,
        command_name: &str,
    ) -> Result<ManagedClient, crate::proto::RmuxError> {
        {
            let active_attach = self.active_attach.lock().await;
        ...
        match attach_candidates.len() + control_candidates.len() {
            0 if command_name == "show-messages" => Err(crate::proto::RmuxError::Message(
    ...
    pub(super) async fn refresh_control_session(&self, session_name: &crate::proto::SessionName) {
        let mut active_control = self.active_control.lock().await;
    ...
    pub(crate) async fn control_session_panes(
        &self,
        session_name: &crate::proto::SessionName,
    ) -> Result<Vec<(u32, PaneOutputSender)>, crate::proto::RmuxError> {
        let state = self.state.lock().await;
    ...
    async fn take_startup_config_error_notifications(&self) -> Vec<String> {
        ...
        errors
            ...
            .flat_map(|error| match error {
                crate::proto::RmuxError::Server(message) => message
                    .lines()
...
impl ActiveControlState {
    pub(super) fn attached_count(&self, session_name: &crate::proto::SessionName) -> usize {
        self.by_pid
            .values()
    ...
    fn current_session_candidate(&self, requester_pid: u32) -> Option<crate::proto::SessionName> {
