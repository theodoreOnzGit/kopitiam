...
async fn session_lease_rejects_too_short_ttl() {
    ...
    let created_lease = handler
        .handle(Request::CreateSessionLease(
            crate::proto::CreateSessionLeaseRequest {
                session_name: alpha,
                ttl_millis: crate::proto::MIN_SESSION_LEASE_TTL_MILLIS - 1,
            },
        ))
        .await;
...
async fn session_lease_reaper_kills_unrenewed_session() {
    ...
    assert_eq!(
        exists,
        Response::HasSession(crate::proto::HasSessionResponse { exists: false })
    );
}
...
async fn session_lease_renew_and_release_preserves_session() {
    ...
    let renewed = handler
        .handle(Request::RenewSessionLease(
            crate::proto::RenewSessionLeaseRequest {
                session_name: alpha.clone(),
    ...
    assert_eq!(
        renewed,
        Response::RenewSessionLease(crate::proto::RenewSessionLeaseResponse { renewed: true })
    );

    let released = handler
        .handle(Request::ReleaseSessionLease(
            crate::proto::ReleaseSessionLeaseRequest {
                session_name: alpha.clone(),
                token: created_lease.token,
            },
        ))
    ...
    assert_eq!(
        released,
        Response::ReleaseSessionLease(crate::proto::ReleaseSessionLeaseResponse { released: true })
    );
    ...
    assert_eq!(
        exists,
        Response::HasSession(crate::proto::HasSessionResponse { exists: true })
    );
}
...
async fn session_destroyed_by_last_pane_exit_clears_stale_lease() {
    ...
    let respawned = handler
        .handle(Request::RespawnPane(Box::new(
            crate::proto::RespawnPaneRequest {
                target: PaneTarget::new(alpha.clone(), 0),
                kill: true,
                start_directory: None,
                environment: None,
                command: None,
                process_command: Some(crate::proto::ProcessCommand::Shell("exit 0".to_owned())),
            },
        )))
        .await;
...
async fn wait_for_session_state(
    ...
    loop {
        let exists = handler
            ...
            .await;
        if exists == Response::HasSession(crate::proto::HasSessionResponse { exists: expected }) {
            return;
        }
        assert!(
