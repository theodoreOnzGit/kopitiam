...
async fn windows_daemon_accepts_ipc_and_dispatches_runtime_requests() -> io::Result<()> {
    ...
    let response = tokio::task::spawn_blocking(move || {
        let mut stream = crate::ipc::connect_blocking(&endpoint, Duration::from_secs(5))?;
...
fn unique_endpoint() -> io::Result<crate::ipc::LocalEndpoint> {
    let unique_id = UNIQUE_ID.fetch_add(1, Ordering::Relaxed);
    crate::ipc::endpoint_for_label(format!("server-windows-{}-{unique_id}", std::process::id()))
}

fn roundtrip(endpoint: &crate::ipc::LocalEndpoint, request: Request) -> io::Result<Response> {
...
fn read_response(stream: &mut crate::ipc::BlockingLocalStream) -> io::Result<Response> {
