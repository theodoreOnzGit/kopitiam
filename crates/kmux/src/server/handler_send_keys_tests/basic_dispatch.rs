...
async fn send_keys_marks_attached_session_input_as_interactive() {
    ...
    tokio::time::timeout(Duration::from_secs(1), async {
        while let Some(control) = control_rx.recv().await {
            if matches!(control, crate::server::pane_io::AttachControl::InteractiveInput) {
                return;
            }
        }
...
async fn pane_input_ref_marks_attached_session_input_as_interactive() {
    ...
    let response = handler
        .handle(Request::PaneInput(crate::proto::PaneInputRequest {
...
async fn send_keys_reset_terminal_updates_transcript_without_writing_to_child() {
    ...
    let captured = state
        .capture_transcript(
            &target,
            crate::server::pane_terminals::PaneCaptureRequest {
