...
async fn live_attach_ctrl_d_uses_windows_console_key_path() {
    ...
    let mut pending_input = Vec::new();
    let keystroke = crate::proto::AttachedKeystroke::new(vec![0x04]).with_windows_console_key(
        crate::proto::AttachedWindowsConsoleKey::new(0x44, 0x20, 0x04, 0x0008, 1),
    );
...
async fn live_attach_unbound_ctrl_p_uses_windows_console_key_path() {
    ...
    let mut pending_input = Vec::new();
    let keystroke = crate::proto::AttachedKeystroke::new(vec![0x10]).with_windows_console_key(
        crate::proto::AttachedWindowsConsoleKey::new(0x50, 0x19, 0x10, 0x0008, 1),
    );
    let forwarded = handler
        .handle_attached_keystroke_input(requester_pid, &mut pending_input, &keystroke)
        .await
...
async fn live_attach_prefix_ctrl_b_is_not_forwarded_as_windows_console_key() {
    ...
    let mut pending_input = Vec::new();
    let keystroke = crate::proto::AttachedKeystroke::new(vec![0x02]).with_windows_console_key(
        crate::proto::AttachedWindowsConsoleKey::new(0x42, 0x30, 0x02, 0x0008, 1),
    );
    let forwarded = handler
        .handle_attached_keystroke_input(requester_pid, &mut pending_input, &keystroke)
        .await
...
async fn live_attach_windows_console_ctrl_semicolon_dispatches_root_binding() {
    ...
    let mut pending_input = Vec::new();
    let keystroke = crate::proto::AttachedKeystroke::new(b";".to_vec()).with_windows_console_key(
        crate::proto::AttachedWindowsConsoleKey::new(0xba, 0x27, b';' as u16, 0x0008, 1),
    );
    let forwarded = handler
        .handle_attached_keystroke_input(requester_pid, &mut pending_input, &keystroke)
        .await
...
async fn read_only_live_attach_drops_decoded_key_input() {
        ...
        .await;
    {
        ...
        active
            .flags
            .insert(crate::server::client_flags::ClientFlags::READONLY);
    }
