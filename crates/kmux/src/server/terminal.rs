...
use crate::core::{EnvironmentStore, OptionStore, PaneId};
use crate::proto::{AttachShellCommand, OptionName, ProcessCommand, RmuxError, SessionName};
use crate::pty::{ChildCommand, PtyChild, PtyMaster, TerminalSize as PtyTerminalSize};
...
impl TerminalProfile {
    ...
    pub(crate) fn initial_pane_title(&self) -> Option<String> {
        let host = crate::server::host_name::local_hostname()?;
...
pub(crate) fn spawn_pane_process(
    ...
    let spawned = command.spawn().map_err(|error| {
        RmuxError::spawn_failed(format!(
            "{} shell: {error}",
            crate::proto::SPAWN_FAILED_MESSAGE_PREFIX
        ))
    })?;
