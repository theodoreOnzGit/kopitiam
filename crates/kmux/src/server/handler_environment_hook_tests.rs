...
async fn global_environment_applies_to_initial_panes_created_after_mutation() {
    ...
    assert_eq!(
        handler
            ...
            .await,
        Response::SetEnvironment(crate::proto::SetEnvironmentResponse {
            scope: ScopeSelector::Global,
            name: variable_name.to_owned(),
        })
    );
...
async fn default_terminal_applies_to_initial_panes_and_yields_to_explicit_term() {
    ...
    assert_eq!(
        handler
            ...
            .await,
        Response::SetOption(crate::proto::SetOptionResponse {
            scope: ScopeSelector::Global,
            option: OptionName::DefaultTerminal,
            mode: SetOptionMode::Replace,
        })
    ...
    let split = handler
        .handle(Request::SplitWindow(crate::proto::SplitWindowRequest {
            target: crate::proto::SplitWindowTarget::Session(session_name("alpha")),
            direction: crate::proto::SplitDirection::Vertical,
            before: false,
            environment: None,
        }))
        .await;
...
async fn set_hook_updates_the_store_and_one_shot_hooks_are_consumed_on_attach() {
    ...
    assert_eq!(
        handler
            ...
            .await,
        Response::SetHook(crate::proto::SetHookResponse {
            scope: ScopeSelector::Session(session_name("alpha")),
            hook: HookName::ClientAttached,
            lifecycle: HookLifecycle::OneShot,
        })
    ...
    let outcome = handler
        .dispatch(
            std::process::id(),
            Request::AttachSession(crate::proto::AttachSessionRequest {
...
async fn session_closed_hooks_fire_before_session_scope_is_removed() {
    ...
    assert_eq!(
        handler
            .handle(Request::KillSession(crate::proto::KillSessionRequest {
                target: session_name("alpha"),
                kill_all_except_target: false,
                clear_alerts: false,
            }))
            .await,
        Response::KillSession(crate::proto::KillSessionResponse { existed: true })
    );
...
async fn kill_pane_does_not_synthesize_pane_exited_hook_like_tmux() {
    ...
    let pane_target = crate::proto::PaneTarget::with_window(session_name("alpha"), 0, 1);
    assert!(matches!(
        handler
            .handle(Request::SetHook(SetHookRequest {
    ...
    assert!(matches!(
        handler
            .handle(Request::KillPane(crate::proto::KillPaneRequest {
                target: pane_target,
...
async fn window_unlinked_hooks_keep_removed_window_name_and_id() {
    ...
    assert!(matches!(
        handler
            .handle(Request::KillWindow(crate::proto::KillWindowRequest {
    ...
    assert!(matches!(
        handler
            .handle(Request::ShowOptions(ShowOptionsRequest {
                scope: crate::proto::OptionScopeSelector::SessionGlobal,
                name: None,
                value_only: false,
...
async fn spawned_pane_environment_contains_pane_id_with_percent_prefix() {
    ...
    let pane_zero = state
        .pane_profile(&session_name("alpha"), 0)
        .expect("pane 0 profile exists");
    let rmux_pane = pane_zero.environment_value("KMUX_PANE");
    assert!(
        rmux_pane.is_some(),
        "KMUX_PANE must be set in spawned pane environment"
    );
    let rmux_pane = rmux_pane.expect("KMUX_PANE is set");
    assert_eq!(pane_zero.environment_value("TMUX_PANE"), Some(rmux_pane));
    assert!(
        rmux_pane.starts_with('%'),
        "KMUX_PANE must start with %: got {rmux_pane}"
    );
    let id_part = &rmux_pane[1..];
    assert!(
        id_part.parse::<u32>().is_ok(),
        "KMUX_PANE must be %<id>: got {rmux_pane}"
    );
}
...
async fn spawned_pane_environment_contains_mux_socket_pid_session_format() {
    ...
    let pane_zero = state
        .pane_profile(&session_name("alpha"), 0)
        .expect("pane 0 profile exists");
    let rmux_value = pane_zero.environment_value("KMUX").expect("KMUX is set");
...
async fn environment_override_layering_session_then_override_then_rmux_pane() {
    ...
    // KMUX_PANE must still be present despite -e.
    assert!(pane_zero.environment_value("KMUX_PANE").is_some());
    drop(state);
...
fn shell_quote_str(value: &str) -> String {
    crate::server::test_shell::command_quote(value)
}
...
fn append_x_command(path: &std::path::Path) -> String {
    crate::server::test_shell::powershell_encoded_command(&format!(
        ...
        crate::server::test_shell::powershell_quote_path(path)
    ))
}
