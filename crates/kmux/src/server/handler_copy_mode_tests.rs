...
fn platform_copy_mode_arg(arg: &str) -> String {
    match arg {
        "cat >/dev/null" => crate::server::test_shell::stdin_discard_command(),
        _ => arg.to_owned(),
    }
}
...
fn stdin_to_file_command(path: &Path) -> String {
    format!("cat > {}", crate::server::test_shell::sh_quote_path(path))
}

#[cfg(unix)]
fn stdin_to_relative_file_command(name: &str) -> String {
    format!("cat > {}", crate::server::test_shell::sh_quote(name))
}

#[cfg(windows)]
fn stdin_to_file_command(path: &Path) -> String {
    let quoted_path = crate::server::test_shell::powershell_quote_path(path);
...
async fn copy_mode_capture_uses_backing_screen_snapshot() {
    ...
    assert_eq!(
        response,
        Response::CopyMode(crate::proto::CopyModeResponse {
            target: target.clone(),
            active: true,
            view_mode: false,
        })
