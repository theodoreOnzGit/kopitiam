...
impl RequestHandler {
    pub(in crate::server::handler) async fn handle_daemon_status(
        &self,
    ...
    pub(in crate::server::handler) async fn handle_shutdown_if_idle(
        &self,
        requester_connection_id: u64,
    ) -> Response {
        let (session_count, client_count) = self
    ...
    pub(in crate::server::handler) async fn handle_kill_server(&self) -> Response {
        self.retained_exited_outputs
            .lock()
            .expect("retained exited output mutex must not be poisoned")
            .clear();
