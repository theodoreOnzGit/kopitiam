...
/// its sibling. Names come from [`crate::os::host`] rather than being spelled
...
fn public_rmux_binary() -> Option<PathBuf> {
    ...
    if file_stem == crate::os::host::PUBLIC_BINARY_NAME {
        return current.is_file().then_some(current);
    }
    if file_stem != crate::os::host::DAEMON_BINARY_NAME {
        return None;
    }
    let mut candidate = current.clone();
    candidate.set_file_name(crate::os::host::public_binary_file_name());
...
mod tests {
    ...
    fn disabled_shim_leaves_environment_untouched() {
        let _lock = crate::server::test_env::lock_blocking();
        let _guard = crate::server::test_env::EnvVarGuard::set("RMUX_DISABLE_TMUX_SHIM", Some("1"));
    ...
    fn shim_dir_prefers_xdg_runtime_dir() {
        ...
        let _guard = crate::server::test_env::EnvVarGuard::set("XDG_RUNTIME_DIR", Some(&root_value));
