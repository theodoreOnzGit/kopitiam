...
#[cfg(windows)]
use crate::ipc::connect_blocking;
...
#[cfg(unix)]
use crate::server::unix_socket::bind_unix_listener_at;
#[cfg(unix)]
use crate::server::unix_socket::real_user_id;
#[cfg(all(test, unix))]
use crate::server::unix_socket::{
...
pub fn default_socket_path() -> io::Result<PathBuf> {
    crate::ipc::default_endpoint().map(LocalEndpoint::into_path)
}
...
impl ServerDaemon {
    ...
    pub async fn bind(self) -> io::Result<ServerHandle> {
        #[cfg(unix)]
        {
            ...
            let signal_watcher = crate::server::signals::SignalWatcher::install()?;
...
fn signal_startup_ready_fd(_ready_fd: i32) {
    #[cfg(any(target_os = "linux", target_os = "android"))]
    {
        let _ = crate::os::daemon::signal_startup_ready_fd(_ready_fd);
    }
}

#[cfg(windows)]
fn signal_startup_ready_event(ready_event: &OsStr) {
    let _ = crate::os::daemon::signal_startup_ready_event(ready_event);
}
