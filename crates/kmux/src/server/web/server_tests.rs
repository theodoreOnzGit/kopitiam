...
use crate::server::web::protocol::{AUTH_FRAME_TIMEOUT, WEB_SHARE_PROTOCOL_VERSION};
use crate::server::web::SecretHashForCrypto;
use base64::Engine;
use crate::proto::{
...
use crate::web_crypto::{derive_client_session, generate_ephemeral, Message, Opener, Sealer};
...
async fn drive_handshake_through_auth(
    ...
    let ml_kem = crate::web_crypto::ml_kem::KeyPair::generate([0x21u8; 64]);
...
async fn write_client_hello(stream: &mut TcpStream, token_id: &str) {
    ...
    let ml_kem_ek = crate::web_crypto::ml_kem::KeyPair::generate([0x33u8; 64]).encapsulation_key();
    let hello = format!(
        r#"{{"type":"hello","protocol_version":{},"capabilities":["e2ee-token-auth","terminal-palette-v1"],"token_id":"{}","client_nonce":"{}","client_public":"{}","client_ml_kem_ek":"{}"}}"#,
        WEB_SHARE_PROTOCOL_VERSION,
        token_id,
...
fn decode_ml_kem_ct(value: &str) -> [u8; crate::web_crypto::ml_kem::CIPHERTEXT_LEN] {
    let bytes = base64::engine::general_purpose::URL_SAFE_NO_PAD
        .decode(value)
        .expect("ml-kem ciphertext is base64url");
    <[u8; crate::web_crypto::ml_kem::CIPHERTEXT_LEN]>::try_from(bytes.as_slice())
