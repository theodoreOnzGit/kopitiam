...
use crate::server::web::origin::validate_public_base_url;
use crate::server::web::secrets::{derive_spectator_token, random_token};
use crate::server::web::{WebShareRegistry, WebShareSettings};
...
async fn frontend_override_changes_browser_origin_without_changing_local_endpoint() {
    let registry = available_registry_with_settings(
        crate::server::web::WebShareSettings::from_options(
            9778,
...
fn local_web_share_requires_bound_listener_and_valid_port() {
    assert!(crate::server::web::WebShareSettings::from_options(0, None).is_err());

    let cold_registry = WebShareRegistry::default();
    assert!(cold_registry
        .config(crate::proto::WebShareConfigRequest)
...
async fn stopped_or_expired_share_rejects_previous_tokens() {
    ...
    assert!(
        registry
            .stop(crate::proto::StopWebShareRequest {
