...
use crate::proto::{RmuxError, SessionId, SessionName};
...
impl From<CreateWebShareRequest> for ResolvedCreateWebShareRequest {
    fn from(request: CreateWebShareRequest) -> Self {
        let target = match &request.scope {
            crate::proto::WebShareScope::Pane(target) => WebShareTarget::pane(target.clone()),
            crate::proto::WebShareScope::Session(name) => {
                WebShareTarget::session(name.clone(), crate::proto::SessionId::new(0))
            }
        };
        Self {
            request,
...
impl WebShareRegistry {
    ...
    pub(crate) fn handle(
        &self,
        request: crate::proto::WebShareRequest,
    ) -> Result<WebShareResponse, RmuxError> {
        match request {
            crate::proto::WebShareRequest::Create(_) => Err(RmuxError::Server(
                "web-share create requires a resolved server target".to_owned(),
            )),
            crate::proto::WebShareRequest::List(request) => {
                Ok(WebShareResponse::List(self.list(request)))
            }
            crate::proto::WebShareRequest::Stop(request) => {
                Ok(WebShareResponse::Stopped(self.stop(request)))
            }
            crate::proto::WebShareRequest::StopAll(request) => {
                Ok(WebShareResponse::StoppedAll(self.stop_all(request)))
            }
            crate::proto::WebShareRequest::Lookup(request) => {
                Ok(WebShareResponse::Lookup(self.lookup(request)))
            }
            crate::proto::WebShareRequest::Config(request) => {
