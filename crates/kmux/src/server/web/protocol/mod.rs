...
use crate::proto::{AttachedWindowsConsoleKey, ResizePaneAdjustment, SplitDirection, TerminalSize};
#[cfg(windows)]
use crate::pty::WindowsConsoleKeyEvent;

use crate::server::input_keys::{encode_key, ExtendedKeyFormat};
use crate::server::keys::parse_key_code;
use crate::server::web::crypto::E2EE_CAPABILITY;
...
fn encode_session_windows_console_key(token: &str) -> Option<(Vec<u8>, AttachedWindowsConsoleKey)> {
    ...
    #[cfg(windows)]
    {
        let key = crate::core::key_code_lookup_bits(parse_key_code(token)?);
...
fn session_windows_ctrl_letter_for_key(
    ...
    for letter in b'A'..=b'Z' {
        ...
        if Some(key)
            == crate::core::key_string_lookup_string(&name).map(crate::core::key_code_lookup_bits)
        {
