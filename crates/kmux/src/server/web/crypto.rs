...
use base64::Engine;
use crate::web_crypto::{Message, Opener, Sealer};
...
/// Thin newtype around the [`crate::web_crypto::Opener`] (server-to-client opener
/// from the caller's point of view: the server opens client-to-server frames).
pub(super) struct FrameOpener {
    opener: Opener,
}

/// Thin newtype around the [`crate::web_crypto::Sealer`].
...
pub(super) fn derive_server_crypto(
    ...
    let (sealer, opener) = crate::web_crypto::derive_server_session(
        psk,
        dh,
        ml_kem_secret,
        client_hello_bytes,
...
fn decode_ml_kem_ek(encoded: &str) -> Result<Vec<u8>, ()> {
    let decoded = base64::engine::general_purpose::URL_SAFE_NO_PAD
        .decode(encoded)
        .map_err(|_| ())?;
    if decoded.len() != crate::web_crypto::ml_kem::ENCAPSULATION_KEY_LEN {
        return Err(());
    }
    Ok(decoded)
}
...
type MlKemEncapsulation = (
    [u8; crate::web_crypto::ml_kem::CIPHERTEXT_LEN],
    Zeroizing<[u8; 32]>,
);
...
pub(super) fn encapsulate_ml_kem(
    ...
    let Ok(ek) =
        <&[u8; crate::web_crypto::ml_kem::ENCAPSULATION_KEY_LEN]>::try_from(client_ml_kem_ek)
    else {
        return Ok(None);
    };
    let mut randomness = Zeroizing::new([0u8; crate::web_crypto::ml_kem::ENCAPS_RANDOMNESS_LEN]);
    ...
    Ok(crate::web_crypto::ml_kem::encapsulate(ek, *randomness)
...
mod tests {
    ...
    use crate::server::web::secrets::SecretHash;
    use base64::Engine;
    use crate::web_crypto::{derive_client_session, generate_ephemeral};
    ...
    fn e2ee_round_trips_text_and_binary_in_order() {
        ...
        assert_eq!(
            ...
            crate::web_crypto::Message::Binary(vec![0x10, b'o', b'k'])
        );
        ...
        assert_eq!(
            client_open.open(&frame).expect("open"),
            crate::web_crypto::Message::Binary(vec![0x11, b'o', b'k'])
        );
    }
    ...
    fn client_hello_rejects_missing_e2ee_capability() {
        ...
        let ek = b64(&crate::web_crypto::ml_kem::KeyPair::generate([1u8; 64]).encapsulation_key());
        let text = format!(
