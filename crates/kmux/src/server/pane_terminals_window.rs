...
impl HandlerState {
    ...
    pub(crate) fn resize_window(
    ...
    ) -> Result<crate::proto::ResizeWindowResponse, RmuxError> {
        ...
        let response = self.mutate_session_and_resize_terminals(&session_name, |session| {
            ...
            if let Some(adjustment) = request.adjustment {
                use crate::proto::ResizeWindowAdjustment;
                match adjustment {
            ...
            session.resize_window(
                window_index,
                crate::proto::TerminalSize { cols: sx, rows: sy },
            )?;

            Ok(crate::proto::ResizeWindowResponse {
    ...
    pub(crate) fn respawn_window(
    ...
    ) -> Result<crate::proto::RespawnWindowResponse, RmuxError> {
        ...
        Ok(crate::proto::RespawnWindowResponse { target })
    }
...
fn request_target_string(target: &crate::proto::MoveWindowTarget) -> String {
    match target {
        crate::proto::MoveWindowTarget::Session(session_name) => session_name.to_string(),
        crate::proto::MoveWindowTarget::Window(target) => target.to_string(),
    }
}
