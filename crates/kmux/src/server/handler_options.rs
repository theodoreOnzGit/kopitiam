...
use crate::server::handler_support::{ensure_option_scope_exists, ensure_scope_session_exists};
...
impl RequestHandler {
    pub(super) async fn handle_set_option(
        &self,
        request: crate::proto::SetOptionRequest,
    ) -> Response {
        if let Err(error) = crate::core::validate_option_mutation(
            request.option,
            &request.scope,
            request.mode,
            &request.value,
        ...
        let response = {
            ...
            match state.options.set(
                ...
                Ok(outcome) => {
                    ...
                    if let crate::proto::ScopeSelector::Window(target) = &request.scope {
    ...
    pub(super) async fn handle_set_option_by_name(
        &self,
        request: crate::proto::SetOptionByNameRequest,
    ) -> Response {
    ...
    pub(super) async fn handle_set_option_by_name_with_client_name(
        ...
        let response = {
            ...
            if let Err(error) = crate::core::validate_option_name_mutation(
                &request.name,
                &request.scope,
                request.mode,
                value.as_deref(),
    ...
    async fn reconcile_attached_sizes_for_option_scope(&self, scope: &OptionScopeSelector) {
        ...
        for session_name in session_names {
            if let Ok(Some(target)) = self.reconcile_attached_session_size(&session_name).await {
                self.emit(crate::core::LifecycleEvent::WindowResized { target })
                    .await;
            }
        }
    }
...
fn should_expand_set_option_value(name: &str, explicit_format: bool, value: &str) -> bool {
    ...
    value.contains("#{") && crate::core::option_name_by_name(name) == Some(OptionName::ExtendedKeys)
}
...
fn format_option_value(
    ...
    target: Option<&crate::proto::Target>,
    socket_path: &Path,
    client_name: Option<&str>,
    value: &str,
) -> Result<String, crate::proto::RmuxError> {
...
fn legacy_scope_to_refresh_scope(scope: &crate::proto::ScopeSelector) -> OptionScopeSelector {
    match scope {
        crate::proto::ScopeSelector::Global => OptionScopeSelector::SessionGlobal,
        crate::proto::ScopeSelector::Session(session_name) => {
            OptionScopeSelector::Session(session_name.clone())
        }
        crate::proto::ScopeSelector::Window(target) => OptionScopeSelector::Window(target.clone()),
        crate::proto::ScopeSelector::Pane(target) => OptionScopeSelector::Pane(target.clone()),
    }
}

fn option_scope_to_legacy_scope(scope: &OptionScopeSelector) -> Option<crate::proto::ScopeSelector> {
    match scope {
        OptionScopeSelector::ServerGlobal => Some(crate::proto::ScopeSelector::Global),
        OptionScopeSelector::SessionGlobal => Some(crate::proto::ScopeSelector::Global),
        OptionScopeSelector::WindowGlobal => None,
        OptionScopeSelector::Session(session_name) => {
            Some(crate::proto::ScopeSelector::Session(session_name.clone()))
        }
        OptionScopeSelector::Window(target) => {
            Some(crate::proto::ScopeSelector::Window(target.clone()))
        }
        OptionScopeSelector::Pane(target) => Some(crate::proto::ScopeSelector::Pane(target.clone())),
    }
}

fn resize_terminals_for_option_change(
    state: &mut crate::server::pane_terminals::HandlerState,
    option: OptionName,
    scope: &ScopeSelector,
) -> Result<(), RmuxError> {
...
fn all_session_names(state: &crate::server::pane_terminals::HandlerState) -> Vec<SessionName> {
    state
        .sessions
        .iter()
        .map(|(session_name, _)| session_name.clone())
...
pub(super) fn option_value_u32(
    options: &crate::core::OptionStore,
    session_name: Option<&crate::proto::SessionName>,
    option: crate::proto::OptionName,
) -> u32 {
    options
