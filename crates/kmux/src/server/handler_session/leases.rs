...
impl RequestHandler {
    pub(in crate::server::handler) async fn handle_create_session_lease(
        &self,
        request: crate::proto::CreateSessionLeaseRequest,
    ) -> Response {
    ...
    pub(in crate::server::handler) async fn handle_renew_session_lease(
        &self,
        request: crate::proto::RenewSessionLeaseRequest,
    ) -> Response {
        let ttl = match duration_from_millis(request.ttl_millis) {
            Ok(ttl) => ttl,
            Err(error) => return Response::Error(ErrorResponse { error }),
    ...
    pub(in crate::server::handler) async fn handle_release_session_lease(
        &self,
        request: crate::proto::ReleaseSessionLeaseRequest,
    ) -> Response {
        let released = self
            .session_leases
            .lock()
    ...
    pub(in crate::server::handler) fn remove_session_leases(&self, session_names: &[SessionName]) {
        self.session_leases
            .lock()
            .expect("session lease mutex must not be poisoned")
...
fn duration_from_millis(ttl_millis: u64) -> Result<Duration, RmuxError> {
    ...
    if ttl_millis < crate::proto::MIN_SESSION_LEASE_TTL_MILLIS {
        return Err(RmuxError::Server(format!(
            "session lease ttl must be at least {}ms",
            crate::proto::MIN_SESSION_LEASE_TTL_MILLIS
        )));
    }
