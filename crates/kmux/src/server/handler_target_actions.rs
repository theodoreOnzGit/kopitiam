...
impl RequestHandler {
    pub(in crate::server::handler) async fn handle_split_window_target_action(
        &self,
        requester_pid: u32,
        request: crate::proto::SplitWindowTargetActionRequest,
    ) -> Response {
        let target = match self
            .resolve_target_for_requester(
                requester_pid,
                crate::proto::ResolveTargetRequest {
                    target: request.target,
                    target_type: crate::proto::ResolveTargetType::Pane,
                    window_index: false,
    ...
    pub(in crate::server::handler) async fn handle_resize_pane_target_action(
        &self,
        requester_pid: u32,
        request: crate::proto::ResizePaneTargetActionRequest,
    ) -> Response {
        let target = match self
            .resolve_target_for_requester(
                requester_pid,
    ...
    pub(in crate::server::handler) async fn handle_capture_pane_target_action(
        &self,
        requester_pid: u32,
        request: crate::proto::CapturePaneTargetActionRequest,
    ) -> Response {
        let target = match self
            .resolve_target_for_requester(
                requester_pid,
