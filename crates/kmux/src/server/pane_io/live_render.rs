...
use crate::server::renderer::{PaneRenderDelta, PaneRenderSnapshot};
use crate::core::{OptionStore, Pane, Session};
...
impl LivePaneRender {
    ...
    pub(crate) fn render_frame_from_transcript(&mut self, replaceable: bool) -> PaneRenderDelta {
        ...
        if replaceable {
            ...
            self.snapshot = next;
            return PaneRenderDelta::Incremental(crate::server::renderer::PaneRenderDeltaFrame::new(
                frame,
                cursor_style,
            ));
        }
...
mod tests {
    ...
    use crate::server::renderer::PaneRenderDelta;
    ...
    fn issue63_interactive_render_stays_small_for_split_panes() {
        ...
        session
            .split_active_pane_with_direction(crate::proto::SplitDirection::Vertical)
            .expect("split pane");
        let pane = session.window().active_pane().expect("active pane").clone();
