...
#[cfg(unix)]
use crate::server::pane_reader_runtime::PaneReaderRuntime;
...
fn maybe_trim_process_heap_after(bytes: usize) {
    ...
    drop(tokio::task::spawn_blocking(
        crate::os::memory::trim_process_heap,
    ));
}
...
struct PanePublishContext<'a> {
    session_name: &'a crate::proto::SessionName,
    pane_id: PaneId,
...
mod unix_tests {
    ...
    use crate::core::{GridRenderOptions, PaneId, ScreenCaptureRange};
    use crate::proto::{SessionName, TerminalSize};
    use crate::pty::{ChildCommand, TerminalSize as PtyTerminalSize};

    use super::{spawn_pane_output_reader, PaneOutputReaderTask};
    use crate::server::pane_io::pane_output_channel;
    use crate::server::pane_reader_runtime::PaneReaderRuntime;
    use crate::server::pane_transcript::PaneTranscript;
...
"#;
    ...
    async fn wait_for_transcript(
        transcript: &crate::server::pane_transcript::SharedPaneTranscript,
        needle: &str,
        timeout: Duration,
    ) -> String {
        let deadline = Instant::now() + timeout;
...
mod tests {
    ...
    fn windows_output_reader_publishes_eof_exit_event_after_child_exit(
        ...
        let callback: crate::server::pane_io::PaneExitCallback = Arc::new(move |event| {
