...
use crate::server::handler::ControlRegistration;
...
async fn register_read_only_control_client(handler: &RequestHandler, requester_pid: u32) {
    ...
    handler
        .register_control_with_access(
            requester_pid,
            ControlModeUpgrade {
                mode: crate::proto::ControlMode::Plain,
...
async fn parsed_queue_display_panes_t_reports_target_client_errors_like_cli() {
    ...
    for (command, expected) in [
        ...
        assert_eq!(error, crate::proto::RmuxError::Message(expected.to_owned()));
    }
}
...
async fn parsed_queue_refresh_client_r_is_unknown_flag_like_tmux() {
    ...
    assert_eq!(
        error,
        crate::proto::RmuxError::Server("command refresh-client: unknown flag -r".to_owned())
    );
}
