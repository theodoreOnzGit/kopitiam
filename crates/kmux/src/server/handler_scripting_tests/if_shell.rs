...
async fn if_shell_format_mode_dispatches_selected_rmux_command() {
    ...
    assert_eq!(
        response,
        Response::IfShell(crate::proto::IfShellResponse::no_output())
    );
...
async fn source_file_if_shell_true_executes_brace_command_list() {
    ...
    assert_eq!(
        response,
        Response::SourceFile(crate::proto::SourceFileResponse::no_output())
    );
    ...
    let output = handler
        .execute_parsed_commands(
            ...
            QueueExecutionContext::without_caller_cwd()
                ...
                .with_mouse_target(Some(Target::Window(crate::proto::WindowTarget::with_window(
                    alpha, 0,
                )))),
        )
        .await
