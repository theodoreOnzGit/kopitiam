use super::*;
use crate::proto::MoveWindowTarget;
...
async fn move_window_relative_collision_uses_tmux_error_shape() {
    ...
    assert_eq!(
        error,
        crate::proto::RmuxError::Server("index in use: 1".to_owned())
    );
}
...
async fn move_window_trailing_colon_target_uses_first_free_index() {
    ...
    {
        let state = handler.state.lock().await;
        let request = crate::server::handler::scripting_support::parse_request_from_parts(
...
fn pane_id_in(session: &crate::core::Session, window_index: u32) -> Option<crate::core::PaneId> {
    session
