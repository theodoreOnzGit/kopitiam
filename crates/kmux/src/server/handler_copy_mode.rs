...
use crate::server::mouse::{copy_mode_mouse_context, copy_mode_mouse_drag_start_context};
...
pub(in crate::server::handler) mod key_binding;
...
impl RequestHandler {
    ...
    pub(super) async fn execute_copy_mode_command(
        ...
        let repeat_count = crate::server::limits::clamp_repeat_count(repeat_count);
...
async fn attached_mouse_context(
    ...
    requester_pid: u32,
    target: &PaneTarget,
) -> Option<crate::server::copy_mode::CopyModeMouseContext> {
...
fn clone_screen_for_target(
    state: &HandlerState,
    target: &PaneTarget,
) -> Result<crate::core::Screen, RmuxError> {
...
fn copy_mode_context(
    state: &HandlerState,
    target: &PaneTarget,
    refresh_screen: Option<crate::core::Screen>,
    mouse: Option<crate::server::copy_mode::CopyModeMouseContext>,
...
fn file_url_host_is_local(host: &str) -> bool {
    ...
    let Some(local) = crate::server::host_name::local_hostname() else {
        return false;
    };
