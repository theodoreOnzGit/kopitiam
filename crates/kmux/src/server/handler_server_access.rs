...
use crate::proto::{CommandOutput, ErrorResponse, Response, RmuxError, ServerAccessResponse};

use super::{ClientFlags, RequestHandler};
use crate::server::pane_io::AttachControl;
use crate::server::server_access::{resolve_user, validate_server_access_request, AccessMode};

impl RequestHandler {
    pub(in crate::server::handler) async fn handle_server_access(
        &self,
        request: crate::proto::ServerAccessRequest,
    ) -> Response {
    ...
    pub(in crate::server::handler) async fn update_live_access_mode(&self, uid: u32, can_write: bool) {
    ...
    pub(in crate::server::handler) async fn disconnect_clients_by_uid(&self, uid: u32) {
        let attached = {
