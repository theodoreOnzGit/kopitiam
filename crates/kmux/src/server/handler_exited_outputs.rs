...
use crate::core::events::PaneOutputSubscriptionKey;
use crate::proto::PaneTarget;

use crate::server::pane_io::PaneOutputSender;

use super::RequestHandler;

/// How long an exited, removed pane keeps its output ring available for a
/// late `Oldest` SDK subscription.
pub(in crate::server::handler) const EXITED_PANE_OUTPUT_RETENTION_TTL: Duration = Duration::from_secs(5);

#[derive(Debug, Clone)]
pub(in crate::server::handler) struct RetainedExitedPaneOutput {
...
impl RetainedExitedPaneOutput {
    ...
    pub(in crate::server::handler) fn pane(&self) -> &PaneOutputSubscriptionKey {
        &self.pane
    }

    pub(in crate::server::handler) fn output(&self) -> &PaneOutputSender {
        &self.output
    }
...
pub(in crate::server::handler) struct RetainedExitedPaneOutputs {
...
impl RetainedExitedPaneOutputs {
    pub(in crate::server::handler) fn insert(
        &mut self,
        target: PaneTarget,
        pane: PaneOutputSubscriptionKey,
        output: PaneOutputSender,
    ...
    pub(in crate::server::handler) fn get(
        &mut self,
        target: &PaneTarget,
        now: Instant,
    ...
    pub(in crate::server::handler) fn get_by_pane(
        &mut self,
        pane: &PaneOutputSubscriptionKey,
        now: Instant,
    ...
    pub(in crate::server::handler) fn cleanup_pane_if_expired(
        &mut self,
        pane: &PaneOutputSubscriptionKey,
        now: Instant,
    ) {
    ...
    pub(in crate::server::handler) fn is_empty(&mut self, now: Instant) -> bool {
        self.cleanup_expired(now);
        self.by_target.is_empty()
    }

    pub(in crate::server::handler) fn clear(&mut self) {
...
impl RequestHandler {
    pub(in crate::server::handler) fn retain_exited_pane_output(
        &self,
        target: PaneTarget,
        pane: PaneOutputSubscriptionKey,
        output: PaneOutputSender,
    ...
    pub(in crate::server::handler) fn retained_exited_pane_output(
        &self,
        target: &PaneTarget,
        now: Instant,
    ) -> Option<RetainedExitedPaneOutput> {
    ...
    pub(in crate::server::handler) fn retained_exited_pane_output_by_pane(
        &self,
        pane: &PaneOutputSubscriptionKey,
        now: Instant,
    ) -> Option<(PaneTarget, RetainedExitedPaneOutput)> {
...
mod tests {
    use super::*;
    use crate::server::pane_io::pane_output_channel_with_limits;
    use crate::proto::{PaneId, SessionName};
