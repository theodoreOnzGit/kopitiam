...
use crate::core::formats::{is_known_format_variable_name, FormatVariable, FormatVariables};
use crate::core::input::mode;
use crate::core::{colour_to_string, GridRenderOptions, ScreenCaptureRange, Style, StyleCell};
use crate::core::{Session, Window, WINLINK_ACTIVITY, WINLINK_BELL, WINLINK_SILENCE};
use crate::proto::OptionName;

use crate::server::hook_runtime::current_hook_format_value;

use crate::server::host_name::local_hostname;
...
impl FormatVariables for RuntimeFormatContext<'_> {
    ...
    fn format_name_exists(&self, scope: Option<char>, name: &str) -> Option<bool> {
        match scope {
            Some('s') => Some(
                crate::proto::SessionName::new(name)
                    .ok()
    ...
    fn format_value_by_name(&self, name: &str) -> Option<String> {
        ...
        let value = match name {
            ...
            "uid" => Some(crate::server::server_access::current_owner_uid().to_string()),
