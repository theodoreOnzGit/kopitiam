...
use crate::proto::{ProcessCommand, RmuxError, TerminalPixels, TerminalSize};
use crate::pty::{
    PtyChild, PtyMaster, Signal, TerminalGeometry as PtyTerminalGeometry,
    TerminalSize as PtyTerminalSize,
};

use crate::server::terminal::{spawn_pane_process, TerminalProfile};
...
impl PaneTerminal {
    ...
    pub(crate) fn resize(&self, geometry: PtyTerminalGeometry) -> crate::pty::Result<()> {
    ...
    pub(crate) fn clone_master(&self) -> crate::pty::Result<PtyMaster> {
        self.master.try_clone()
    }

    pub(crate) fn clone_master_for_output_reader(&mut self) -> crate::pty::Result<PtyMaster> {
        self.master.try_clone_for_startup_reader()
    }

    #[cfg(windows)]
    pub(crate) fn clone_child_for_wait(&self) -> crate::pty::Result<PtyChild> {
    ...
    pub(crate) fn tty_path(&self) -> Option<PathBuf> {
        crate::os::process::fd_path(self.pid(), 0)
    }

    pub(crate) fn is_alive(&mut self) -> crate::pty::Result<bool> {
    ...
    pub(crate) fn exit_status(&mut self) -> crate::pty::Result<Option<ExitStatus>> {
        let _ = self.is_alive()?;
        Ok(self.exit_status)
    }

    #[cfg(unix)]
    pub(crate) fn continue_if_stopped(&self) -> crate::pty::Result<bool> {
...
mod tests {
    use crate::core::PaneGeometry;
    use crate::proto::{TerminalPixels, TerminalSize};
