...
use crate::core::{LifecycleEvent, PaneId};
use crate::proto::{
...
impl RequestHandler {
    ...
    pub(super) async fn handle_new_window(
        &self,
        requester_pid: u32,
        request: crate::proto::NewWindowRequest,
    ) -> Response {
    ...
    pub(super) async fn handle_kill_window(
        &self,
        request: crate::proto::KillWindowRequest,
    ) -> Response {
    ...
    pub(super) async fn handle_select_window(
        &self,
        request: crate::proto::SelectWindowRequest,
    ) -> Response {
        let session_name = request.target.session_name().clone();
        ...
        if matches!(response, Response::SelectWindow(_)) {
            ...
            self.queue_inline_hook(
                ...
                ScopeSelector::Session(session_name.clone()),
                Some(Target::Window(crate::proto::WindowTarget::with_window(
                    session_name.clone(),
                    target_window_index,
                ))),
                PendingInlineHookFormat::AfterCommand,
    ...
    pub(super) async fn handle_rename_window(
        &self,
        request: crate::proto::RenameWindowRequest,
    ) -> Response {
    ...
    pub(super) async fn handle_next_window(
        &self,
        request: crate::proto::NextWindowRequest,
    ) -> Response {
        let session_name = request.target;
    ...
    pub(super) async fn handle_previous_window(
        &self,
        request: crate::proto::PreviousWindowRequest,
    ) -> Response {
        let session_name = request.target;
        let (response, resize_sessions) = {
            let mut state = self.state.lock().await;
    ...
    pub(super) async fn handle_last_window(
        &self,
        request: crate::proto::LastWindowRequest,
    ) -> Response {
        let session_name = request.target;
        let (response, resize_sessions) = {
            let mut state = self.state.lock().await;
    ...
    pub(super) async fn handle_list_windows(
        &self,
        request: crate::proto::ListWindowsRequest,
    ) -> Response {
        let attached_count = {
    ...
    pub(super) async fn handle_link_window(
        &self,
        request: crate::proto::LinkWindowRequest,
    ) -> Response {
    ...
    pub(super) async fn handle_move_window(
        &self,
        request: crate::proto::MoveWindowRequest,
    ) -> Response {
    ...
    pub(super) async fn handle_unlink_window(
        &self,
        request: crate::proto::UnlinkWindowRequest,
    ) -> Response {
    ...
    pub(super) async fn handle_swap_window(
        &self,
        request: crate::proto::SwapWindowRequest,
    ) -> Response {
        let refresh_sessions =
            unique_sessions(request.source.session_name(), request.target.session_name());
        let response = {
    ...
    pub(super) async fn handle_rotate_window(
        &self,
        request: crate::proto::RotateWindowRequest,
    ) -> Response {
        let session_name = request.target.session_name().clone();
    ...
    pub(super) async fn handle_resize_window(
        &self,
        request: crate::proto::ResizeWindowRequest,
    ) -> Response {
    ...
    pub(super) async fn handle_respawn_window(
        &self,
        requester_pid: u32,
        mut request: crate::proto::RespawnWindowRequest,
    ) -> Response {
        let session_name = request.target.session_name().clone();
        let target = request.target.clone();
        let socket_path = self.socket_path();
        let process_command =
            crate::server::legacy_command::from_legacy_command(request.command.as_deref());
        let client_environment = client_environment_snapshot(requester_pid);
        let spawn_environment = client_spawn_environment(client_environment.as_ref());
        let attached_count = self.attached_count(&session_name).await;
        let response = {
    ...
    async fn resolve_resize_window_linked_session_size(
        &self,
        mut request: crate::proto::ResizeWindowRequest,
    ) -> Result<crate::proto::ResizeWindowRequest, crate::proto::RmuxError> {
        use crate::proto::ResizeWindowAdjustment::{LargestLinkedSession, SmallestLinkedSession};
        ...
        let (linked_sessions, fallback_size) = {
            ...
            let session = state
                ...
                .session(request.target.session_name())
                .ok_or_else(|| {
                    crate::server::pane_terminals::session_not_found(request.target.session_name())
                })?;
            let _window = session
                .window_at(request.target.window_index())
                .ok_or_else(|| {
                    crate::proto::RmuxError::invalid_target(
...
fn resize_window_size_rank(size: &crate::proto::TerminalSize) -> (u32, u16, u16) {
    (
...
fn move_window_refresh_sessions(
    request: &crate::proto::MoveWindowRequest,
) -> Vec<crate::proto::SessionName> {
    if request.renumber {
        return match &request.target {
            crate::proto::MoveWindowTarget::Session(session_name) => vec![session_name.clone()],
            crate::proto::MoveWindowTarget::Window(target) => vec![target.session_name().clone()],
        };
    }
    ...
    let crate::proto::MoveWindowTarget::Window(target) = &request.target else {
...
fn window_pane_ids(window: &crate::core::Window) -> Vec<PaneId> {
    window.panes().iter().map(|pane| pane.id()).collect()
}

fn unique_sessions(
    source_session: &crate::proto::SessionName,
    target_session: &crate::proto::SessionName,
) -> Vec<crate::proto::SessionName> {
