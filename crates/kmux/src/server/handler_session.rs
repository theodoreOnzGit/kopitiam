...
use crate::proto::request::NewSessionExtRequest;
use crate::proto::types::OptionScopeSelector;
use crate::proto::{
...
use crate::server::format_runtime::{render_runtime_template, RuntimeFormatContext};
use crate::server::pane_terminals::InitialPaneSpawnOptions;
#[cfg(windows)]
use crate::server::pane_terminals::{CompletedDeferredInitialPane, DeferredInitialPaneSpawn, HandlerState};
use crate::server::terminal::{parse_environment_assignments, validate_process_command};
...
impl RequestHandler {
    pub(in crate::server::handler) async fn destroy_unattached_sessions_for_option_scope(
        &self,
    ...
    pub(in crate::server::handler) async fn destroy_unattached_sessions(
        &self,
    ...
    pub(in crate::server::handler) async fn handle_new_session(
        &self,
        requester_pid: u32,
        request: crate::proto::NewSessionRequest,
    ) -> Response {
    ...
    pub(in crate::server::handler) async fn handle_new_session_ext(
        &self,
        requester_pid: u32,
        ...
        let requested_process_command = request
            .process_command
            .or_else(|| crate::server::legacy_command::from_legacy_command(command.as_deref()));
    ...
    async fn finish_deferred_initial_pane_spawn(&self, completed: CompletedDeferredInitialPane) {
        ...
        let mut pending = completed.input_writer.map(|input_writer| {
            crate::server::pane_terminals::DeferredInitialPaneInputFlush {
                input_writer,
    ...
    async fn flush_deferred_initial_pane_input(
        flush: crate::server::pane_terminals::DeferredInitialPaneInputFlush,
    ) -> Result<(), RmuxError> {
        if flush.queued_input.is_empty() {
            return Ok(());
        }
        tokio::task::spawn_blocking(move || {
            let pane_pid = crate::pty::ProcessId::new(flush.pane_pid)
                .map_err(|error| std::io::Error::other(error.to_string()))?;
            for input in flush.queued_input {
                match input {
                    crate::server::pane_terminals::DeferredInitialPaneInput::Bytes(bytes) => {
                        flush.input_writer.write_all(&bytes)?;
                    }
                    crate::server::pane_terminals::DeferredInitialPaneInput::Console { action, .. } => {
    ...
    fn write_deferred_initial_console_input(
        pane_pid: crate::pty::ProcessId,
        action: crate::server::pane_terminals::DeferredInitialPaneConsoleInputAction,
    ) -> std::io::Result<()> {
        for attempt in 0..=DEFERRED_INITIAL_PANE_CONSOLE_INPUT_RETRIES {
            let result = match action {
                crate::server::pane_terminals::DeferredInitialPaneConsoleInputAction::Key(key) => {
                    crate::pty::write_windows_console_key(pane_pid, key)
                }
                crate::server::pane_terminals::DeferredInitialPaneConsoleInputAction::KeyThenInterrupt(
                    key,
                ) => crate::pty::write_windows_console_key_then_interrupt_if_processed(pane_pid, key),
                crate::server::pane_terminals::DeferredInitialPaneConsoleInputAction::Interrupt => {
                    crate::pty::send_windows_console_interrupt(pane_pid)
                }
                crate::server::pane_terminals::DeferredInitialPaneConsoleInputAction::Noop => Ok(()),
            };
            match result {
                Ok(()) => return Ok(()),
                Err(error)
    ...
    pub(in crate::server::handler) async fn handle_kill_session(
        &self,
        request: crate::proto::KillSessionRequest,
    ) -> Response {
        let session_name = {
            let state = self.state.lock().await;
    ...
    pub(in crate::server::handler) async fn handle_rename_session(
        &self,
        request: crate::proto::RenameSessionRequest,
    ) -> Response {
        let session_name = {
            let state = self.state.lock().await;
        ...
        if session_name == new_name {
            return Response::RenameSession(crate::proto::RenameSessionResponse { session_name });
        }
        ...
        let response = {
            ...
            match state.rename_session(&session_name, &new_name) {
                Ok(()) => {
                    ...
                    drop(active_attach);
                    renamed = true;
                    Response::RenameSession(crate::proto::RenameSessionResponse {
    ...
    pub(in crate::server::handler) async fn handle_list_sessions(
        &self,
        request: crate::proto::ListSessionsRequest,
    ) -> Response {
    ...
    pub(in crate::server::handler) async fn request_shutdown_if_server_empty(&self) -> bool {
    ...
    pub(in crate::server::handler) async fn queue_shutdown_if_server_empty(&self) -> bool {
        let should_shutdown = {
            let state = self.state.lock().await;
...
#[cfg(windows)]
fn deferred_initial_pane_ready_item(item: &crate::core::events::OutputCursorItem) -> bool {
    matches!(item, crate::core::events::OutputCursorItem::Event(event) if !event.bytes().is_empty())
}

fn destroy_unattached_candidate_sessions(
    state: &crate::server::pane_terminals::HandlerState,
    scope: &OptionScopeSelector,
) -> Vec<SessionName> {
    match scope {
...
fn session_pane_ids(session: &crate::core::Session) -> Vec<PaneId> {
    session
        .windows()
        .values()
