...
use crate::proto::{AttachFrameDecoder, AttachMessage};
#[cfg(any(unix, windows))]
...
#[cfg(any(unix, windows))]
use crate::server::renderer::{PaneRenderDelta, PaneRenderDeltaFrame};
#[cfg(any(unix, windows))]
        ...
        loop {
            ...
            tokio::select! {
                ...
                result = recv_pane_output_optional(current_target.pane_output.as_mut()), if !pane_refresh.is_pending() => {
                    ...
                    #[cfg(unix)]
                    if let crate::core::events::OutputCursorItem::Event(event) = &item {
                    ...
                    {
                        match item {
                            crate::core::events::OutputCursorItem::Event(event)
                                if !event.is_empty()
                                    && event.byte_len() <= 512
