...
impl HandlerState {
    ...
    pub(in crate::server::pane_terminals) fn session_pane_terminal_geometries_by_runtime(
        &self,
        session_name: &SessionName,
    ) -> Result<HashMap<SessionName, Vec<crate::server::pane_terminal_lookup::SessionPane>>, RmuxError>
    {
        let session = self
            .sessions
            .session(session_name)
        ...
        for (window_index, window) in session.windows() {
            ...
            panes.extend(window.panes().iter().map(|pane| {
                crate::server::pane_terminal_lookup::SessionPane {
                    id: pane.id(),
    ...
    pub(in crate::server::pane_terminals) fn remove_pane_terminal_from_runtime(
        &mut self,
        runtime_session_name: &SessionName,
        pane_id: PaneId,
    ) -> bool {
