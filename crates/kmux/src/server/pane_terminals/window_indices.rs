use crate::proto::{OptionName, RmuxError, SessionName};

use super::{session_not_found, HandlerState};

impl HandlerState {
    pub(in crate::server::pane_terminals) fn session_base_index(&self, session_name: &SessionName) -> u32 {
        self.options
    ...
    pub(in crate::server::pane_terminals) fn renumber_windows_if_enabled(
        &mut self,
    ...
    pub(in crate::server::pane_terminals) fn reindex_windows_from_base(
        &mut self,
        session_name: &SessionName,
    ) -> Result<(), RmuxError> {
    ...
    pub(in crate::server::pane_terminals) fn remap_reindexed_window_metadata(
        &mut self,
        session_name: &SessionName,
