...
use crate::core::{PaneGeometry, PaneId};
use crate::proto::{SessionId, SessionName, TerminalSize, WindowId};

use super::{HandlerState, PaneExitMetadata};

#[derive(Clone, PartialEq, Eq, Default)]
pub(in crate::server::pane_terminals) struct PrivatePaneEnvironment(Vec<String>);
...
pub(in crate::server::pane_terminals) struct PaneLifecycleSpawn {
    pub(in crate::server::pane_terminals) session_id: SessionId,
    pub(in crate::server::pane_terminals) window_id: WindowId,
    pub(in crate::server::pane_terminals) pane_id: PaneId,
    pub(in crate::server::pane_terminals) command: Option<Vec<String>>,
    pub(in crate::server::pane_terminals) working_directory: Option<PathBuf>,
    pub(in crate::server::pane_terminals) private_environment: Option<Vec<String>>,
    pub(in crate::server::pane_terminals) dimensions: TerminalSize,
    pub(in crate::server::pane_terminals) pid: Option<u32>,
}
...
impl HandlerState {
    ...
    #[cfg(windows)]
    pub(in crate::server::pane_terminals) fn record_pane_lifecycle_starting(
        &mut self,
    ...
    pub(in crate::server::pane_terminals) fn record_pane_lifecycle_spawn(
        &mut self,
        spawn: PaneLifecycleSpawn,
    ) {
        let pid = spawn.pid;
    ...
    #[cfg(windows)]
    pub(in crate::server::pane_terminals) fn mark_pane_lifecycle_running(
        &mut self,
        pane_id: PaneId,
        pid: Option<u32>,
    ) {
    ...
    pub(in crate::server::pane_terminals) fn update_pane_lifecycle_output_sequence(
        &mut self,
        pane_id: PaneId,
        output_sequence: u64,
    ) {
    ...
    pub(in crate::server::pane_terminals) fn mark_pane_lifecycle_exited(
        &mut self,
        pane_id: PaneId,
    ...
    pub(in crate::server::pane_terminals) fn remove_pane_lifecycle(&mut self, pane_id: PaneId) {
        let _ = self.pane_lifecycle.remove(&pane_id);
    }

    pub(in crate::server::pane_terminals) fn remove_pane_lifecycles<'a>(
        &mut self,
    ...
    pub(in crate::server::pane_terminals) fn sync_pane_lifecycle_dimensions_for_session(
        &mut self,
