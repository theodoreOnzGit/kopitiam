use crate::core::{input::mode, GridRenderOptions, PaneId, Screen, ScreenCaptureRange, Utf8Config};
use crate::proto::{
    OptionName, OptionScopeSelector, PaneTarget, RmuxError, ScopeSelector, SessionName,
};

use crate::server::pane_screen_state::PaneScreenState;
...
impl HandlerState {
    ...
    pub(crate) fn pane_copy_mode_summary(
        ...
        session_name: &SessionName,
        pane_id: PaneId,
    ) -> Option<crate::server::copy_mode::CopyModeSummary> {
        let window_index = self
            .sessions
            .session(session_name)?
            .window_index_for_pane_id(pane_id)?;
    ...
    pub(in crate::server::pane_terminals) fn history_limit_for_session(
        &self,
        session_name: &SessionName,
    ) -> usize {
        self.options
    ...
    pub(in crate::server::pane_terminals) fn refresh_transcript_limits_for_session(
        &mut self,
        session_name: &SessionName,
    ) {
    ...
    pub(in crate::server::pane_terminals) fn alternate_screen_enabled_for_pane_id(
        &self,
        session_name: &SessionName,
        pane_id: PaneId,
    ) -> bool {
    ...
    pub(in crate::server::pane_terminals) fn title_rename_enabled_for_pane_id(
        &self,
        session_name: &SessionName,
        pane_id: PaneId,
    ) -> bool {
    ...
    pub(in crate::server::pane_terminals) fn input_buffer_limit(&self) -> usize {
        self.options
    ...
    pub(in crate::server::pane_terminals) fn resize_transcripts(
        &mut self,
        session_name: &SessionName,
        pane_geometries: &[crate::server::pane_terminal_lookup::SessionPane],
    ) {
        ...
        for pane in pane_geometries {
            ...
            transcript
                .lock()
                .expect("pane transcript mutex must not be poisoned")
                .resize(crate::proto::TerminalSize {
