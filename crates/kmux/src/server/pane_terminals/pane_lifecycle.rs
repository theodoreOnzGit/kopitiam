...
use crate::server::pane_io::{PaneAlertCallback, PaneExitCallback};
use crate::server::pane_terminal_lookup::initial_pane;
use crate::server::pane_terminal_process::{open_pane_terminal, PaneTerminal};
use crate::server::terminal::{validate_process_command, SessionBaseEnvironment, TerminalProfile};
...
impl HandlerState {
    ...
    pub(in crate::server::pane_terminals) fn reset_window_terminal_with_base_environment(
        &mut self,
        session_name: &SessionName,
        window_index: u32,
        spawn: WindowSpawnOptions<'_>,
    ...
    pub(crate) fn kill_pane_with_options(
        ...
        if committed_outcome.window_destroyed() {
            let _ = self
                .options
                .remove_window(&crate::proto::WindowTarget::with_window(
                    session_name.clone(),
                    target.window_index(),
                ));
        } else {
...
fn terminate_removed_terminals(
    terminals: &mut std::collections::HashMap<PaneId, crate::server::pane_terminal_process::PaneTerminal>,
) {
...
pub(in crate::server::pane_terminals) fn clone_terminal_for_output_reader(
...
#[cfg(windows)]
pub(in crate::server::pane_terminals) fn clone_terminal_for_exit_watcher(
    terminal: &PaneTerminal,
    session_name: &SessionName,
    pane_id: PaneId,
) -> Result<crate::pty::PtyChild, RmuxError> {
