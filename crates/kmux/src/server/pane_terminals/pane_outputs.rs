...
use crate::core::{events::PaneOutputSubscriptionKey, PaneId};
use crate::proto::{PaneTarget, RmuxError, SessionName};

use crate::server::pane_io::PaneOutputSender;
use crate::server::pane_terminal_lookup::{missing_pane_terminal, pane_id_for_target};
use crate::server::pane_transcript::SharedPaneTranscript;
...
pub(in crate::server::pane_terminals) use self::spawn::PaneOutputSpawn;
...
pub(super) struct RemovedPaneOutputs {
    ...
    #[cfg(unix)]
    pane_output_readers: HashMap<PaneId, crate::server::pane_io::PaneOutputReaderTask>,
...
impl RemovedPaneOutputs {
    pub(in crate::server::pane_terminals) fn abort_output_readers(&mut self) {
        #[cfg(unix)]
...
impl HandlerState {
    ...
    pub(crate) fn subscribe_runtime_pane_output(
        ...
        runtime_session_name: &SessionName,
        pane_id: PaneId,
    ) -> Option<crate::server::pane_io::PaneOutputReceiver> {
        self.pane_outputs
            .get(runtime_session_name)
            .and_then(|panes| panes.get(&pane_id))
    ...
    pub(in crate::server::pane_terminals) fn remove_session_pane_outputs(
        &mut self,
        session_name: &SessionName,
    ) -> RemovedPaneOutputs {
    ...
    pub(in crate::server::pane_terminals) fn remove_pane_output(
        &mut self,
        session_name: &SessionName,
        pane_id: PaneId,
    ...
    pub(in crate::server::pane_terminals) fn remove_pane_outputs(
        &mut self,
        session_name: &SessionName,
        pane_ids: &[PaneId],
    ) -> RemovedPaneOutputs {
    ...
    pub(in crate::server::pane_terminals) fn insert_existing_pane_outputs(
        &mut self,
        session_name: &SessionName,
    ...
    pub(in crate::server::pane_terminals) fn pane_output_generation(
        &self,
        session_name: &SessionName,
        pane_id: PaneId,
    ) -> u64 {
