...
use crate::proto::{ProcessCommand, RmuxError, SessionName};

use crate::server::pane_io::PaneExitEvent;
use crate::server::pane_terminal_lookup::initial_pane;
use crate::server::pane_terminal_process::{open_pane_terminal, pty_geometry_from_layout, PaneTerminal};
...
impl HandlerState {
    pub(crate) fn prepare_deferred_initial_session_terminal(
        ...
        let profile = crate::server::terminal::TerminalProfile::for_initial_session_pane(
            &self.environment,
            &self.options,
            session_name,
            session_id.as_u32(),
        ...
        self.terminals.insert_pending_session(
            ...
            crate::server::terminal::SessionBaseEnvironment::from_profile(&profile),
        )?;
    ...
    pub(crate) fn starting_pane_profile_in_window(
        ...
        window_index: u32,
        pane_index: u32,
    ) -> Option<&crate::server::terminal::TerminalProfile> {
        self.starting_pane_in_window(session_name, window_index, pane_index)
    ...
    pub(crate) fn starting_pane_base_environment(
        ...
        runtime_session_name: &SessionName,
        pane_id: PaneId,
    ) -> Option<crate::server::terminal::SessionBaseEnvironment> {
        self.starting_panes
            .get(runtime_session_name)?
            .get(&pane_id)
            .map(|pane| crate::server::terminal::SessionBaseEnvironment::from_profile(&pane.profile))
    }
