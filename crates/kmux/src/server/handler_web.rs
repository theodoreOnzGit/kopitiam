...
use crate::server::pane_io::{self, AttachControl, LiveAttachInputContext, PaneOutputReceiver};
use crate::server::pane_terminal_lookup::pane_id_for_target;
use crate::server::server_access::current_owner_uid;
use crate::server::web::{ResolvedCreateWebShareRequest, WebSessionTarget, WebShareAccess, WebShareTarget};
use crate::core::{input::mode, PaneId};
...
impl RequestHandler {
    ...
    pub(crate) async fn open_web_share(
        ...
        let token_id = crate::server::web::SecretHashForCrypto::from_secret(token).token_id();
        self.open_web_share_token_id(&token_id, pin).await
    }

    pub(crate) fn web_settings(&self) -> crate::server::web::WebShareSettings {
    ...
    pub(crate) async fn ensure_web_share_listener_running(&self) -> Result<(), RmuxError> {
        ...
        if let Err(error) = crate::server::web::spawn(Arc::new(self.clone())).await {
    ...
    pub(in crate::server::handler) async fn handle_web_share(&self, request: WebShareRequest) -> Response {
            ...
            WebShareRequest::Create(request) => {
                ...
                match self.web_shares.create(request) {
                    Ok(created) => {
                        ...
                        Ok(crate::proto::WebShareResponse::Created(created))
                    }
                    Err(error) => Err(error),
                }
            }
    ...
    pub(crate) fn web_share_pre_auth_token(
        ...
        token_id: &str,
        origin: &str,
    ) -> Option<(crate::server::web::SecretHashForCrypto, bool)> {
        self.web_shares.pre_auth_token(token_id, origin)
    }

    pub(in crate::server::handler) fn prune_web_session(&self, removed: Option<(SessionName, SessionId)>) {
    ...
    pub(crate) async fn web_session_snapshot_with_scrolls(
        ...
        let terminal_context = OuterTerminalContext::default();
        let state = self.state.lock().await;
        let _lock_span = crate::server::perf_instrument::span("state_lock_hold")
        ...
        let _render_span = crate::server::perf_instrument::span("render_compose")
            .with_str("site", "web_session_snapshot")
    ...
    async fn wait_for_web_share_expiry(
        ...
        share_id: &str,
        expires_at_unix: u64,
    ) -> Option<crate::server::web::ExpiredWebShare> {
    ...
    ) -> Result<ResolvedCreateWebShareRequest, crate::proto::RmuxError> {
        let state = self.state.lock().await;
    ...
    async fn start_web_share_tunnel(
        ...
        let tunnel = crate::server::web::start_tunnel_provider(&provider, &settings).await?;
