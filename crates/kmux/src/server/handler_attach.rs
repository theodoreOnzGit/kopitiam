...
use crate::server::outer_terminal::{CursorScope, OuterTerminal, OuterTerminalContext};
use crate::server::pane_io::{AttachControl, AttachTarget, LivePaneRender, OverlayFrame};
use crate::server::pane_terminals::{session_not_found, HandlerState};
use crate::server::renderer;
use crate::server::terminal::TerminalProfile;
...
pub(crate) use crate::server::client_flags::ClientFlags;
pub(in crate::server::handler) use resize_policy::AttachedWindowSizePolicy;
...
impl RequestHandler {
    pub(crate) async fn handle_attached_keystroke(
        ...
        consumed: bool,
    ) -> Result<KeyDispatched, crate::proto::RmuxError> {
        let active_attach = self.active_attach.lock().await;
        if !active_attach.by_pid.contains_key(&attach_pid) {
            return Err(crate::proto::RmuxError::Server(
                "attached client disappeared".to_owned(),
            ));
        }
        let byte_len = u32::try_from(keystroke.bytes().len()).map_err(|_| {
            crate::proto::RmuxError::Server("attached keystroke length overflow".to_owned())
        })?;
        if consumed {
    ...
    pub(super) async fn terminal_context_and_size_for_attached_client(
        ...
        TerminalSize,
        Option<crate::proto::TerminalPixels>,
        bool,
        ClientFlags,
    )> {
        let active_attach = self.active_attach.lock().await;
    ...
    pub(super) async fn attached_session_name_for_command(
        ...
        attach_pid: u32,
        command_name: &str,
    ) -> Result<crate::proto::SessionName, crate::proto::RmuxError> {
        let active_attach = self.active_attach.lock().await;
        active_attach
            .by_pid
            .get(&attach_pid)
    ...
    pub(super) async fn attach_shell_command_for_session(
        ...
        command: String,
    ) -> Result<AttachShellCommand, crate::proto::RmuxError> {
    ...
    pub(super) async fn send_attach_control(
        ...
        command_name: &str,
        next_session_name: Option<crate::proto::SessionName>,
    ) -> Result<crate::proto::SessionName, crate::proto::RmuxError> {
    ...
    pub(super) async fn exit_attached_session(&self, session_name: &crate::proto::SessionName) {
...
pub(super) fn attach_target_for_session(
    ...
    socket_path: &Path,
) -> Result<AttachTarget, crate::proto::RmuxError> {
...
fn attach_target_for_session_with_prompt(
    ...
    let active_pane_geometry = active_pane.as_ref().map_or_else(
        || crate::core::PaneGeometry::new(0, 0, 0, 0),
        |pane| {
            renderer::visible_pane_terminal_geometry(session, &state.options, pane)
                .unwrap_or_else(|| crate::core::PaneGeometry::new(0, 0, 0, 0))
        },
    );
    ...
    let input_target = PaneTarget::with_window(
        session_name.clone(),
        session.active_window_index(),
        active_pane.as_ref().map_or(0, crate::core::Pane::index),
    );
...
pub(super) fn option_affects_attached_rendering(option: crate::proto::OptionName) -> bool {
    matches!(
        option,
        crate::proto::OptionName::ExtendedKeys
            | crate::proto::OptionName::AllowPassthrough
            | crate::proto::OptionName::FocusEvents
            | crate::proto::OptionName::Mouse
            | crate::proto::OptionName::SetClipboard
            | crate::proto::OptionName::TerminalFeatures
            | crate::proto::OptionName::TerminalOverrides
    ) || crate::core::option_affects_rendering(option)
}
...
mod tests {
    ...
    use crate::proto::{SessionName, TerminalSize};
    use tokio::sync::mpsc;
