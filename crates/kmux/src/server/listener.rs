...
use crate::ipc::{wait_for_peer_close, LocalListener, LocalStream, PeerIdentity};
use crate::proto::{
...
use crate::server::control::{self, ControlLifecycle, ControlServerEvent};
use crate::server::daemon::ShutdownHandle;
use crate::server::handler::{
    attach_support::AttachRegistration, ControlRegistration, DetachedRequestGuard, PreparedSdkWait,
    RequestHandler,
};
use crate::server::listener_options::ServeOptions;
use crate::server::listener_signals::handle_server_signal;
use crate::server::listener_signals::poll_server_signal;
use crate::server::listener_signals::wait_server_signal;
use crate::server::pane_io;
use crate::server::server_access::apply_access_policy;
use crate::server::socket_cleanup::SocketCleanup;
...
pub(crate) async fn serve(
    ...
    let handler = Arc::new(
        RequestHandler::with_owner_uid_subscription_limits_and_web_settings(
            ...
            crate::server::web::WebShareSettings::from_options_with_port_explicit(
                options.web_port,
...
async fn serve_connection(
    ...
    let Some(_) = handler.access_mode_for_peer(&requester) else {
        ...
        conn.write_response(&Response::Error(ErrorResponse {
            error: crate::proto::RmuxError::Server("access not allowed".to_owned()),
        }))
        .await?;
        return Ok(());
    };
...
mod tests {
    use super::*;
    use crate::server::server_access::AccessMode;
    use crate::proto::{
    ...
    async fn persistent_connection_reevaluates_server_access_per_request() -> io::Result<()> {
        let peer_uid = crate::os::identity::real_user_id().saturating_add(10_000);
        let peer = PeerIdentity {
            pid: std::process::id(),
            uid: peer_uid,
            user: crate::os::identity::UserIdentity::Uid(peer_uid),
        };
        let handler = Arc::new(RequestHandler::new());
        handler
    ...
    async fn read_request_sends_framed_error_for_decode_mismatch() -> io::Result<()> {
        ...
        let mut frame = vec![crate::proto::RMUX_FRAME_MAGIC, RMUX_WIRE_VERSION as u8];
    ...
    fn spawn_test_connection(
        ...
        spawn_test_connection_with_peer(
            ...
            PeerIdentity {
                pid: std::process::id(),
                uid: crate::os::identity::real_user_id(),
                user: crate::os::identity::UserIdentity::Uid(crate::os::identity::real_user_id()),
            },
        )
    }
...
mod windows_tests {
    ...
    use crate::proto::KillServerRequest;

    #[tokio::test]
    async fn kill_server_peer_disconnect_still_requests_shutdown() -> io::Result<()> {
        let endpoint = crate::ipc::endpoint_for_label(format!(
            "listener-kill-disconnect-{}",
            std::process::id()
        ))?;
        let listener = crate::ipc::LocalListener::bind(&endpoint)?;
        let handler = Arc::new(RequestHandler::new());
        ...
        tokio::task::spawn_blocking(move || -> io::Result<()> {
            let mut client =
                crate::ipc::connect_blocking(&endpoint_for_client, Duration::from_secs(2))?;
        ...
        match connection_task.await.expect("connection task") {
            Ok(()) => Ok(()),
            Err(error) if crate::ipc::is_peer_disconnect(&error) => Ok(()),
            Err(error) => Err(error),
        }
    }
}
