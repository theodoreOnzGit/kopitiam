...
use crate::core::{WINLINK_ACTIVITY, WINLINK_BELL, WINLINK_SILENCE};
#[cfg(unix)]
use crate::proto::SendKeysRequest;
use crate::proto::{
...
async fn pane_alert_event_sets_bell_and_activity_flags_and_emits_alert_hooks() {
    ...
    handler.pane_alert_callback()(crate::server::pane_io::PaneAlertEvent {
        session_name: session.clone(),
        pane_id,
        bell_count: 1,
        title_changed: false,
...
async fn pane_alert_callback_can_be_invoked_from_reader_thread() {
    ...
    std::thread::spawn(move || {
        callback(crate::server::pane_io::PaneAlertEvent {
    ...
    handler
        .register_attach_with_access(
            ...
            AttachRegistration {
                ...
                render_stream: false,
                uid,
                user: crate::os::identity::UserIdentity::Uid(uid),
                can_write: true,
...
async fn pane_exit_callback_can_be_invoked_from_reader_thread() {
    ...
    std::thread::spawn(move || {
        callback(crate::server::pane_io::PaneExitEvent::eof_published(
...
async fn pane_alert_event_respects_automatic_rename_off() {
    ...
    handler
        .handle_pane_alert_event(crate::server::pane_io::PaneAlertEvent {
            session_name: session.clone(),
            pane_id,
            bell_count: 0,
            title_changed: false,
...
async fn visual_bell_modes_dispatch_overlay_write_and_action_gating() {
    ...
    .await;
    handler
        .alerts_queue_window(current_window.clone(), crate::core::WINDOW_BELL)
        .await;
    ...
    .await;
    handler
        .alerts_queue_window(current_window, crate::core::WINDOW_BELL)
        .await;
    ...
    .await;
    handler
        .alerts_queue_window(WindowTarget::new(session.clone()), crate::core::WINDOW_BELL)
        .await;
    assert_no_visual_bell_delivery(&mut control_rx).await;

    handler
        .alerts_queue_window(other_window.clone(), crate::core::WINDOW_BELL)
        .await;
...
async fn format_variables_focus_clearing_and_alert_navigation_follow_winlink_flags() {
    ...
    assert_eq!(
        next,
        Response::NextWindow(crate::proto::NextWindowResponse {
    ...
    assert_eq!(
        previous,
        Response::PreviousWindow(crate::proto::PreviousWindowResponse {
    ...
    assert_eq!(
        wrapped_previous,
        Response::PreviousWindow(crate::proto::PreviousWindowResponse { target: window_two })
    );
}
...
async fn activity_deduplication_skips_second_alert_on_same_winlink() {
    ...
    handler
        .alerts_queue_window(window.clone(), crate::core::WINDOW_ACTIVITY)
        .await;
    ...
    handler
        .alerts_queue_window(window.clone(), crate::core::WINDOW_BELL)
        .await;
...
async fn silence_deduplication_skips_second_silence_on_same_winlink() {
    ...
    handler
        .alerts_queue_window(window.clone(), crate::core::WINDOW_SILENCE)
        .await;
...
async fn show_messages_message_log_resolves_target_client() {
    ...
    assert_eq!(
        response,
        Response::Error(crate::proto::ErrorResponse {
            error: crate::proto::RmuxError::Server("can't find client: 999999".to_owned())
        })
    );
}
