use crate::core::{BoxLines, PaneGeometry, Style};
use crate::proto::{OptionName, Target, TerminalSize};

use crate::server::format_runtime::{render_runtime_template, RuntimeFormatContext};
use crate::server::mouse::{AttachedMouseEvent, StatusLineLayout, StatusRangeType};
...
pub(super) fn menu_width(title: &str, items: &[MenuOverlayItem]) -> u16 {
    let utf8 = crate::core::Utf8Config::default();
    let title_width = crate::core::text_width(title, &utf8);
    let item_width = items
        .iter()
        .filter(|item| !item.separator)
        .map(|item| {
            crate::core::text_width(&item.label, &utf8)
                + item
                    .shortcut_label
                    .as_ref()
                    .map(|shortcut| crate::core::text_width(shortcut, &utf8) + 1)
                    .unwrap_or_default()
        })
        .max()
        .unwrap_or_default();
...
fn overlay_status_lines(session: &crate::core::Session, options: &crate::core::OptionStore) -> u16 {
    if matches!(
        options.resolve(Some(session.name()), OptionName::Status),
        Some("off")
    ) {
...
fn pane_geometry_for_target(session: &crate::core::Session, target: &Target) -> Option<PaneGeometry> {
    match target {
        Target::Pane(target) => session
            .window_at(target.window_index())
            .and_then(|window| window.pane(target.pane_index()))
            .map(crate::core::Pane::geometry),
        Target::Window(target) => session
            .window_at(target.window_index())
            .and_then(crate::core::Window::active_pane)
            .map(crate::core::Pane::geometry),
