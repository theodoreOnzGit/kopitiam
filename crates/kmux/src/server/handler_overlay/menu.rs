...
use crate::server::input_keys::MouseForwardEvent;
use crate::server::pane_terminals::HandlerState;
use crate::server::renderer::{render_menu_overlay, MenuRenderItem, MenuRenderSpec, OverlayRect};
...
#[derive(Debug, Clone)]
pub(in crate::server::handler) struct MenuOverlayState {
    ...
    pub(in crate::server::handler) selected_style: Style,
    pub(in crate::server::handler) border_style: Style,
    pub(in crate::server::handler) border_lines: BoxLines,
    pub(in crate::server::handler) flags: u8,
    pub(in crate::server::handler) choice: Option<usize>,
    pub(in crate::server::handler) items: Vec<MenuOverlayItem>,
}
...
#[derive(Debug, Clone)]
pub(in crate::server::handler) struct MenuOverlayItem {
    pub(in crate::server::handler) label: String,
    pub(in crate::server::handler) shortcut_label: Option<String>,
    pub(in crate::server::handler) shortcut: Option<PromptInputEvent>,
    pub(in crate::server::handler) separator: bool,
    pub(in crate::server::handler) action: Option<OverlayMenuAction>,
}

#[derive(Debug, Clone)]
pub(in crate::server::handler) enum OverlayMenuAction {
    Command(String),
    Popup(PopupMenuAction),
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::server::handler) enum PopupMenuAction {
    Close,
    Paste,
    FillSpace,
    Centre,
