...
use crate::core::{EnvironmentStore, OptionStore};
use crate::proto::{OptionName, ProcessCommand, ScopeSelector, SessionName, SetOptionMode};
#[cfg(windows)]
use crate::pty::TerminalSize as PtyTerminalSize;
...
fn terminal_profile_initial_pane_title_uses_host_short() {
    ...
    let host = crate::server::host_name::local_hostname().expect("host name");
...
fn terminal_profile_runtime_window_name_tracks_spawned_command_shape() {
    ...
    assert_eq!(
        profile
            .runtime_window_name(Some(&crate::proto::ProcessCommand::Shell(
                "printf hi".to_owned(),
            )))
            .as_deref(),
        Some("printf")
    ...
    assert_eq!(
        profile
            .runtime_window_name(Some(&crate::proto::ProcessCommand::Argv(vec![
    ...
    assert_eq!(
        profile
            .automatic_window_name(Some(&crate::proto::ProcessCommand::Shell(
                "sleep 30".to_owned(),
            )))
            .as_deref(),
        Some("sleep")
...
#[cfg(windows)]
fn reap_windows_test_child(child: &mut crate::pty::PtyChild) -> Result<(), Box<dyn Error>> {
