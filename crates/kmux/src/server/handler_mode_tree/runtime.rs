...
use crate::proto::{OptionName, PaneTarget, RmuxError, SessionName, Target};
...
impl RequestHandler {
    pub(in crate::server::handler) async fn execute_queued_mode_tree(
        &self,
        requester_pid: u32,
    ...
    pub(in crate::server::handler) async fn mode_tree_active(&self, attach_pid: u32) -> bool {
        let active_attach = self.active_attach.lock().await;
        active_attach
            .by_pid
            .get(&attach_pid)
    ...
    pub(in crate::server::handler) async fn refresh_mode_tree_overlay_if_active(
        &self,
        attach_pid: u32,
    ) -> Result<(), RmuxError> {
    ...
    pub(super) async fn show_mode_tree_help(&self, attach_pid: u32) -> Result<(), RmuxError> {
        ...
        let (overlay_frame, clear_frame, duration) = {
            ...
            let session = state
                ...
                .ok_or_else(|| session_not_found(&session_name))?;
            (
                crate::server::renderer::render_status_message(session, &state.options, MODE_TREE_HELP),
                crate::server::renderer::render_status_message(session, &state.options, ""),
