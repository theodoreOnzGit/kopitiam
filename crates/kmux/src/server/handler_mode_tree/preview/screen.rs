...
use crate::server::pane_terminals::{HandlerState, PaneCaptureRequest};

use super::super::mode_tree_render::{pad_visible_width, sanitize_overlay_text};

pub(in crate::server::handler::mode_tree_support) fn preview_lines_for_target(
    state: &HandlerState,
    target: &PaneTarget,
    limit: usize,
    width: usize,
...
pub(in crate::server::handler::mode_tree_support) fn preview_lines_for_screen(
...
fn preview_line_for_screen_view(
    line: &crate::core::ScreenLineView,
    start_x: usize,
    width: usize,
    row: usize,
    cursor: (usize, usize),
...
fn preview_cell_style(cell: &crate::core::ScreenCellView) -> Style {
    Style {
        cell: StyleCell {
            fg: cell.fg(),
            bg: cell.bg(),
...
pub(in crate::server::handler::mode_tree_support) fn preview_vertical_offset(
    screen: &crate::core::Screen,
    height: usize,
) -> usize {
    let rows = usize::from(screen.size().rows.max(1));
...
pub(in crate::server::handler::mode_tree_support) fn preview_horizontal_offset(
    screen: &crate::core::Screen,
    width: usize,
) -> usize {
