...
use crate::server::mouse::{AttachedMouseEvent, MouseLocation};
...
use crate::core::{input::InputParser, Screen};
...
fn take_switch_target(control: AttachControl) -> crate::server::pane_io::AttachTarget {
    match control {
...
async fn recv_switch_target(
    control_rx: &mut mpsc::UnboundedReceiver<AttachControl>,
    context: &str,
) -> crate::server::pane_io::AttachTarget {
...
async fn display_target_format(
    ...
    let response = handler
        .handle(Request::DisplayMessage(crate::proto::DisplayMessageRequest {
            target: Some(crate::proto::Target::Pane(target)),
            print: true,
