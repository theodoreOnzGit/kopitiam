use crate::core::command_parser::{CommandArgument, ParsedCommands};
use crate::core::key_code_lookup_bits;
use crate::proto::{PaneTarget, RmuxError, Target};
...
use crate::server::limits::clamp_repeat_count;

pub(in crate::server::handler) struct DirectCopyModeCommand {
    pub(in crate::server::handler) command: String,
    pub(in crate::server::handler) args: Vec<String>,
    pub(in crate::server::handler) repeat_count: usize,
}

pub(in crate::server::handler) fn direct_copy_mode_command(
...
impl RequestHandler {
    pub(in crate::server::handler) async fn handle_detached_copy_mode_key_code(
        &self,
        requester_pid: u32,
        target: PaneTarget,
...
mod tests {
    ...
    fn direct_copy_mode_command_clamps_repeat_count() {
        ...
        assert_eq!(
            command.repeat_count,
            crate::server::limits::MAX_COMMAND_REPEAT_COUNT
        );
    }
}
