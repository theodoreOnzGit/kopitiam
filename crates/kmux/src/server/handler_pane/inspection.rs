...
use crate::server::control_notifications::format_control_message_line;
...
impl RequestHandler {
    pub(in crate::server::handler) async fn handle_display_message(
        &self,
        requester_pid: u32,
        request: crate::proto::DisplayMessageRequest,
    ) -> Response {
    ...
    pub(in crate::server::handler) async fn handle_display_message_ext(
        &self,
        requester_pid: u32,
        request: crate::proto::DisplayMessageExtRequest,
    ) -> Response {
        self.handle_display_message_inner(
            requester_pid,
            request.target,
    ...
    pub(in crate::server::handler) async fn handle_list_panes(
        &self,
        request: crate::proto::ListPanesRequest,
    ) -> Response {
        let attached_count = {
            let active_attach = self.active_attach.lock().await;
...
#[cfg(windows)]
impl RequestHandler {
    pub(in crate::server::handler) async fn wait_for_windows_deferred_list_pane_pids(
        &self,
    ...
    pub(in crate::server::handler) async fn wait_for_windows_deferred_list_session_pane_pids(&self) {
        self.wait_for_windows_deferred_pane_pids_until(|| async {
            let state = self.state.lock().await;
    ...
    pub(in crate::server::handler) async fn wait_for_windows_deferred_all_pane_pids(&self) {
        self.wait_for_windows_deferred_pane_pids_until(|| async {
            let state = self.state.lock().await;
...
#[cfg(windows)]
pub(in crate::server::handler) fn format_references_pane_pid(format: Option<&str>) -> bool {
...
pub(in crate::server::handler) fn display_message_context<'a>(
    state: &'a HandlerState,
    target: &Target,
    attached_count: usize,
) -> Result<(&'a crate::core::Session, RuntimeFormatContext<'a>), RmuxError> {
...
fn lone_session_name(sessions: &crate::core::SessionStore) -> Option<crate::proto::SessionName> {
    (sessions.len() == 1)
        .then(|| {
            sessions
                .iter()
...
pub(in crate::server::handler) fn display_time(
    options: &crate::core::OptionStore,
    session_name: &crate::proto::SessionName,
) -> std::time::Duration {
    std::time::Duration::from_millis(
        options
            .resolve(Some(session_name), crate::proto::OptionName::DisplayTime)
...
pub(in crate::server::handler) fn attached_status_message_for_error(error: &RmuxError) -> String {
