...
pub(in crate::server::handler) struct PaneSnapshotInputs {
    pane_id: PaneId,
...
impl RequestHandler {
    pub(in crate::server::handler) async fn handle_pane_snapshot(
        &self,
    ...
    pub(in crate::server::handler) fn resolve_pane_snapshot_inputs(
        &self,
        state: &HandlerState,
    ...
    pub(in crate::server::handler) fn handle_pane_snapshot_inputs(
        &self,
        inputs: PaneSnapshotInputs,
    ) -> Response {
        let pane_id = inputs.pane_id;
        let _snapshot_span = crate::server::perf_instrument::span("snapshot")
...
pub(in crate::server::handler) struct PaneSnapshotRevisionRegistry {
...
mod tests {
    ...
    use crate::core::{Screen, TerminalScreen};
