...
use crate::core::{key_code_lookup_bits, key_code_to_bytes, key_string_lookup_string};
use crate::proto::{OptionName, PaneTarget, RmuxError, Target, DEFAULT_MAX_FRAME_LENGTH};
#[cfg(windows)]
...
use crate::server::key_table::{decode_attached_key, AttachedKeyDecode};
use crate::server::mouse::{classify_mouse_events, layout_for_session, ClassifiedMouseEvent};
use crate::server::pane_io::{AttachControl, OverlayFrame};
...
pub(in crate::server::handler) fn retain_partial_attached_control_input(
    context: &str,
...
impl RequestHandler {
    ...
    async fn handle_attached_live_mouse(
        &self,
        attach_pid: u32,
        raw: crate::server::input_keys::MouseForwardEvent,
    ) -> io::Result<()> {
        if self.attached_client_input_is_read_only(attach_pid).await? {
            return Ok(());
        }
    ...
    pub(in crate::server::handler) async fn attached_input_target(
        &self,
        attach_pid: u32,
    ...
    pub(in crate::server::handler) async fn attached_last_mouse_event(
        &self,
        attach_pid: u32,
    ) -> Option<crate::server::input_keys::MouseForwardEvent> {
        let active_attach = self.active_attach.lock().await;
        active_attach
            .by_pid
            .get(&attach_pid)
...
fn is_enter_key(key: crate::core::KeyCode) -> bool {
