...
use crate::core::{input::mode, key_code_lookup_bits};
#[cfg(windows)]
use crate::core::{key_string_lookup_string, KEYC_CTRL, KEYC_IMPLIED_META, KEYC_META, KEYC_SHIFT};
use crate::proto::{AttachedKeystroke, PaneTarget, Response};
#[cfg(unix)]
use crate::pty::PtyMaster;
#[cfg(windows)]
use crate::pty::WindowsConsoleKeyEvent;
...
use crate::server::input_keys::{decode_extended_key, decode_mouse, ExtendedKeyDecode, MouseDecode};
use crate::server::key_table::{
...
impl RequestHandler {
    ...
    async fn handle_attached_live_input_inner_with_windows_console_key(
        ...
        pending_input: &mut Vec<u8>,
        bytes: &[u8],
        windows_console_key: Option<crate::proto::AttachedWindowsConsoleKey>,
    ) -> io::Result<bool> {
        #[cfg(not(windows))]
    ...
    pub(crate) async fn try_forward_plain_attached_bytes_to_current_pane_fast(
        ...
        let clear_alerts_changed = {
            ...
            if state.options.resolve_for_window(
                target.session_name(),
                target.window_index(),
                crate::proto::OptionName::SynchronizePanes,
            ) == Some("on")
            {
                return Ok(None);
            }
            let pane_id = state
                ...
                .map(crate::core::Pane::id)
                .ok_or_else(|| {
                    io_other(crate::proto::RmuxError::invalid_target(
                        target.to_string(),
    ...
    async fn fast_path_attached_session(
        &self,
        attach_pid: u32,
    ) -> io::Result<Option<crate::proto::SessionName>> {
        let active_attach = self.active_attach.lock().await;
        let active = active_attach.by_pid.get(&attach_pid).ok_or_else(|| {
            io_other(crate::proto::RmuxError::Server(
                ...
                Some(session_name) => {
                    let window_index = {
                        ...
                        state
                            ...
                            .map(crate::core::Session::active_window_index)
                    };
    ...
    async fn dispatch_immediate_prefix_detach(
        ...
        let is_bare_detach_binding = {
            ...
            let prefix = session_option_key(
                &state,
                target.session_name(),
                crate::proto::OptionName::Prefix,
            );
            let prefix2 = session_option_key(
                &state,
                target.session_name(),
                crate::proto::OptionName::Prefix2,
            );
...
fn first_byte_matches_prefix(
    ...
    let prefix = session_option_key(state, target.session_name(), crate::proto::OptionName::Prefix);
    let prefix2 = session_option_key(
        state,
        target.session_name(),
        crate::proto::OptionName::Prefix2,
...
#[cfg(windows)]
fn windows_console_key_event(key: crate::proto::AttachedWindowsConsoleKey) -> WindowsConsoleKeyEvent {
...
fn windows_console_binding_override_key(
    ...
    Some(character.to_ascii_lowercase() as crate::core::KeyCode | KEYC_CTRL | preserved_modifiers)
}

#[cfg(windows)]
fn key_matches_name(key: crate::core::KeyCode, name: &str) -> bool {
    windows_key_code_named(name).is_some_and(|expected| expected == key)
}

#[cfg(windows)]
fn windows_key_code_named(name: &str) -> Option<crate::core::KeyCode> {
    key_string_lookup_string(name).map(key_code_lookup_bits)
}

#[cfg(all(test, windows))]
mod windows_console_binding_tests {
    use crate::core::{KEYC_CTRL, KEYC_IMPLIED_META, KEYC_META, KEYC_SHIFT};
    use crate::pty::WindowsConsoleKeyEvent;
