use crate::proto::{ErrorResponse, HookName, Response, ScopeSelector, Target};
...
impl RequestHandler {
    pub(in crate::server::handler) async fn handle_pipe_pane(
        &self,
        _requester_pid: u32,
        request: crate::proto::PipePaneRequest,
    ) -> Response {
    ...
    pub(in crate::server::handler) async fn handle_respawn_pane(
        &self,
        requester_pid: u32,
        mut request: crate::proto::RespawnPaneRequest,
    ) -> Response {
        let session_name = request.target.session_name().clone();
        let target = request.target.clone();
