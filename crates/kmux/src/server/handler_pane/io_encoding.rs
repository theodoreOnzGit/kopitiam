#[cfg(windows)]
use crate::core::key_string_lookup_string;
use crate::core::{key_code_lookup_bits, key_string_lookup_key};
use crate::proto::{
    ErrorResponse, OptionName, PaneTarget, Response, RmuxError, SendKeysResponse, SessionName,
};
use crate::pty::PtyMaster;
#[cfg(windows)]
use crate::pty::{ProcessId, WindowsConsoleKeyEvent};

use crate::server::input_keys::{encode_key, encode_mouse_event, ExtendedKeyFormat};
use crate::server::keys::parse_key_code;
#[cfg(windows)]
use crate::server::pane_terminals::DeferredInitialPaneConsoleInputAction;
...
pub(super) fn windows_console_input_for_attached_key(
    state: &HandlerState,
    target: &PaneTarget,
    decoded_key: crate::core::KeyCode,
...
fn target_uses_windows_cmd_shell(state: &HandlerState, target: &PaneTarget) -> bool {
    ...
    state
        ...
        .ok()
        .and_then(crate::os::process::command_name)
        .as_deref()
        .is_some_and(is_windows_cmd_name)
}
...
fn target_has_wsl_descendant_process(state: &HandlerState, target: &PaneTarget) -> bool {
    ...
    let descendant_matches = crate::os::process::descendant_command_names(pane_pid)
        .iter()
        ...
        PaneConsoleInputSink::ConsolePid(pid) => {
            ...
            tokio::task::spawn_blocking(move || match action {
                WindowsConsoleInputAction::Key(key) => {
                    crate::pty::write_windows_console_key(pid, key)
                }
                WindowsConsoleInputAction::KeyThenInterrupt(key) => {
                    crate::pty::write_windows_console_key_then_interrupt_if_processed(pid, key)
                }
                WindowsConsoleInputAction::Interrupt => {
                    crate::pty::send_windows_console_interrupt(pid)
                }
...
pub(super) fn pane_id_for_input_target(
    state: &HandlerState,
    target: &PaneTarget,
) -> Result<crate::core::PaneId, RmuxError> {
    let session_name = target.session_name();
    let window_index = target.window_index();
    let pane_index = target.pane_index();
    let session = state
...
pub(in crate::server::handler) async fn write_bracketed_pane_payload(
    master: PtyMaster,
    payload: Vec<u8>,
    bracketed: bool,
) -> std::io::Result<()> {
...
pub(super) fn encode_mouse_for_target(
    state: &HandlerState,
    target: &PaneTarget,
    event: &crate::server::mouse::AttachedMouseEvent,
) -> Result<Vec<u8>, RmuxError> {
    let session = state
        .sessions
        .session(target.session_name())
