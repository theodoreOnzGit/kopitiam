...
impl RequestHandler {
    pub(in crate::server::handler) async fn handle_last_pane(
        &self,
        request: crate::proto::LastPaneRequest,
    ) -> Response {
    ...
    pub(in crate::server::handler) async fn handle_select_pane(
        &self,
        request: crate::proto::SelectPaneRequest,
    ) -> Response {
        let session_name = request.target.session_name().clone();
        ...
        let (response, pane_changed) = {
            ...
            match (|| -> Result<SelectPaneResponse, RmuxError> {
                if let Some(style) = style.as_deref() {
                    ...
                    crate::core::validate_option_name_mutation(
                        "window-style",
                        &scope,
    ...
    pub(in crate::server::handler) async fn handle_select_pane_adjacent(
        &self,
        request: crate::proto::SelectPaneAdjacentRequest,
    ) -> Response {
        let session_name = request.target.session_name().clone();
        let window_index = request.target.window_index();
    ...
    pub(in crate::server::handler) async fn handle_select_pane_mark(
        &self,
        request: crate::proto::SelectPaneMarkRequest,
    ) -> Response {
        let session_name = request.target.session_name().clone();
        let window_index = request.target.window_index();
        let response = {
