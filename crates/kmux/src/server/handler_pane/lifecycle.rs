...
use crate::core::events::{OutputCursorItem, PaneOutputSubscriptionKey};
use crate::core::LifecycleEvent;
use crate::proto::{OptionName, PaneTarget, RmuxError, SessionId, Target, WindowTarget};
...
use crate::server::pane_io::{PaneExitCallback, PaneExitEvent, PaneOutputReceiver, PaneOutputSender};
use crate::server::pane_terminal_lookup::missing_pane_terminal;
use crate::server::pane_terminals::{session_not_found, HandlerState, PaneExitMetadata};
use tracing::warn;
...
enum PaneExitPlan {
    Ignore,
    KeepDead {
        runtime_session_name: crate::proto::SessionName,
        target: PaneTarget,
        prepare_dead: bool,
    ...
    RemovePane {
        ...
        removed_pane_ids: Vec<crate::core::PaneId>,
        pane_event: super::super::QueuedLifecycleEvent,
...
impl RequestHandler {
    pub(in crate::server::handler) fn pane_exit_callback(&self) -> PaneExitCallback {
...
fn pane_lifecycle_identifiers(
    state: &HandlerState,
    target: &PaneTarget,
    fallback_pane_id: crate::core::PaneId,
...
fn apply_dead_pane_automatic_window_name(
    state: &mut HandlerState,
    target: &PaneTarget,
) -> Result<Vec<crate::proto::SessionName>, RmuxError> {
    let rendered = state
    ...
    let should_update = {
        ...
        window.name() != Some(rendered.as_str())
            && crate::server::automatic_rename::window_allows_automatic_rename(
                &state.options,
                target.session_name(),
                target.window_index(),
                window,
...
mod tests {
    ...
    async fn ensure_eof_skips_timeout_when_exit_event_already_published_eof() {
        let sender = crate::server::pane_io::pane_output_channel();
        let generation = None;
        sender
