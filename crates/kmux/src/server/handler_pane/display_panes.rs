...
use crate::core::{key_code_lookup_bits, key_code_to_bytes};
use crate::proto::{
...
impl RequestHandler {
    pub(in crate::server::handler) async fn handle_display_panes(
        &self,
        request: crate::proto::DisplayPanesRequest,
    ) -> Response {
        ...
        if let Response::DisplayPanes(success) = &response {
            ...
            if let Some(active_pane) = active_pane {
                self.emit(crate::core::LifecycleEvent::PaneModeChanged {
    ...
    async fn display_panes_prefix_input(
        ...
        let (prefix_key, prefix2_key, prefix_bytes, prefix2_bytes, backspace) = {
            ...
            let backspace = state
                ...
                .and_then(crate::core::key_string_lookup_string)
...
pub(super) fn display_panes_time(
    ...
    Duration::from_millis(
        options
            .resolve(Some(session_name), crate::proto::OptionName::DisplayPanesTime)
