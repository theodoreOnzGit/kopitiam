use crate::core::{key_code_lookup_bits, key_string_lookup_key, key_string_lookup_string};
use crate::proto::{
...
use crate::server::keys::{parse_key_code, resolve_hex_key};
use crate::server::limits::bounded_repeat_count;

impl RequestHandler {
    pub(in crate::server::handler) async fn handle_send_keys(
        &self,
        request: crate::proto::SendKeysRequest,
    ) -> Response {
    ...
    pub(in crate::server::handler) async fn handle_send_keys_ext(
        &self,
        requester_pid: u32,
        request: crate::proto::SendKeysExtRequest,
    ) -> Response {
        self.handle_send_keys_ext_inner(requester_pid, request, None)
            .await
    }

    pub(in crate::server::handler) async fn handle_send_keys_ext2(
        &self,
        requester_pid: u32,
        request: crate::proto::SendKeysExt2Request,
    ) -> Response {
        let target_client = request.target_client;
        let request = crate::proto::SendKeysExtRequest {
    ...
    pub(in crate::server::handler) async fn handle_send_prefix(
        &self,
        requester_pid: u32,
        request: crate::proto::SendPrefixRequest,
    ) -> Response {
        let attached_session = {
            let active_attach = self.active_attach.lock().await;
...
fn input_write_sessions(writes: &[PaneInputWrite]) -> Vec<crate::proto::SessionName> {
...
#[cfg(windows)]
fn console_input_write_sessions(writes: &[PaneConsoleInputWrite]) -> Vec<crate::proto::SessionName> {
    let mut sessions = Vec::new();
    for write in writes {
        let session_name = write.session_name();
        if !sessions.iter().any(|existing| existing == session_name) {
