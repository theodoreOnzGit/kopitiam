...
use crate::core::{BoxLines, OptionStore, Session, Style};
use crate::proto::{OptionName, ScopeSelector, SessionName, SetOptionMode, TerminalSize};
...
fn overlay_position_resolves_tmux_shorthand_positions() {
    ...
    let pane = session
        ...
        .geometry();
    let runtime =
        RuntimeFormatContext::new(crate::core::formats::FormatContext::from_session(&session))
...
fn status_layout_marks_left_window_and_right_ranges() {
    ...
    assert!(layout
        .ranges
        .iter()
        .any(|range| matches!(range.kind, crate::server::status_ranges::StatusRangeType::Left)));
    assert!(layout
        .ranges
        .iter()
        .any(|range| matches!(range.kind, crate::server::status_ranges::StatusRangeType::Window(_))));
    assert!(layout
        .ranges
        .iter()
        .any(|range| matches!(range.kind, crate::server::status_ranges::StatusRangeType::Right)));
}
...
fn status_layout_tracks_inline_range_changes_inside_status_left() {
    ...
    assert!(layout.ranges.iter().any(|range| matches!(
        range.kind,
        crate::server::status_ranges::StatusRangeType::Left
    ) && range.x == (0..=0)));
    assert!(layout.ranges.iter().any(|range| matches!(
        range.kind,
        crate::server::status_ranges::StatusRangeType::Control(7)
    ) && range.x == (1..=1)));
}
