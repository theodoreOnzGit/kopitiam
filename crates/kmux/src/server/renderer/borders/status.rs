...
use crate::core::{OptionStore, Pane, Session, Style, Utf8Config};
...
use crate::server::pane_visible_geometry::{
...
pub(in crate::server::renderer) fn render_pane_border_status_lines(
    session: &Session,
    options: &OptionStore,
...
#[cfg(test)]
mod tests {
    use crate::core::{OptionStore, Session};
    use crate::proto::{OptionName, ScopeSelector, SetOptionMode, SplitDirection, TerminalSize};
    ...
    fn pane_border_status_top_renders_edge_and_border_lines() {
        ...
        options
            .set(
                ScopeSelector::Window(crate::proto::WindowTarget::with_window(
                    session.name().clone(),
                    session.active_window_index(),
                )),
    ...
    fn pane_border_status_uses_configured_border_line_fill() {
        let session = two_pane_session();
        let mut options = OptionStore::new();
        let scope = ScopeSelector::Window(crate::proto::WindowTarget::with_window(
            session.name().clone(),
            session.active_window_index(),
        ));
        options
