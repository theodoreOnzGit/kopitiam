...
use crate::core::{input::mode, GridRenderOptions, OptionStore, Pane, Screen, Session};
    ...
    if screen.has_selected_cells() && pane_selection_style_is_set(session, options, pane) {
        return styled_pane_screen(session, options, pane, screen)
            .capture_transcript_lines_independent(
                crate::core::ScreenCaptureRange::default(),
                render_options,
            )
            .into_iter()
...
mod tests {
    ...
    use crate::core::{input::InputParser, OptionStore, Screen, Session};
    use crate::proto::{
