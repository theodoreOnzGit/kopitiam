...
use crate::server::format_runtime::{
...
fn status_bar_line_with_pane_title(
    ...
    if !left.is_empty() {
        expanded.push_str(&format!(
            ...
            crate::core::style_tostring(&left_style),
            left
        ));
    }
    ...
    if !right.is_empty() {
        expanded.push_str(&format!(
            ...
            crate::core::style_tostring(&right_style),
            right
        ));
    }
...
pub(super) fn render_status_template_jobs_with_profile<V>(
    template: &str,
    variables: &V,
    profile: Option<&crate::server::terminal::TerminalProfile>,
    cache_ttl: Duration,
) -> String
where
    V: crate::core::formats::FormatVariables + ?Sized,
{
...
fn status_job_cache_ttl(options: &OptionStore, session_name: &crate::proto::SessionName) -> Duration {
