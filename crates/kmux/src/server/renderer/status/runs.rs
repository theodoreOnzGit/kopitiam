...
pub(in crate::server::renderer::status) fn render_status_runs(row: u16, runs: &[StatusRun]) -> Vec<u8> {
    if runs.is_empty() {
        return Vec::new();
    }
...
pub(in crate::server::renderer::status) fn truncate_status_runs(
    runs: &[StatusRun],
    width: usize,
...
pub(in crate::server::renderer::status) fn push_spaces(
...
pub(in crate::server::renderer::status) fn push_status_run(
    runs: &mut Vec<StatusRun>,
    text: String,
    style: StatusStyle,
) {
...
pub(in crate::server::renderer) fn sanitize_status_text(text: String) -> String {
    text.chars()
        .map(|character| {
...
pub(in crate::server::renderer) fn status_runs_width(
    runs: &[StatusRun],
    utf8_config: &Utf8Config,
) -> usize {
    runs.iter()
...
pub(in crate::server::renderer) struct StatusRun {
    pub(in crate::server::renderer) text: String,
    pub(in crate::server::renderer) style: StatusStyle,
}

pub(in crate::server::renderer) type StatusStyle = Style;
