...
use crate::server::format_runtime::RuntimeFormatContext;
use crate::server::status_ranges::{StatusLineLayout, StatusRange, StatusRangeType};
...
pub(crate) fn resolve_overlay_rect(
    ...
    let resolved_x = crate::server::format_runtime::render_runtime_template(x_template, &runtime, false);
    let resolved_y = crate::server::format_runtime::render_runtime_template(y_template, &runtime, false);
...
pub(crate) fn status_line_layout(
    ...
    if let Some(prompt) = prompt {
        let utf8 = crate::core::Utf8Config::from_options(options);
...
pub(crate) fn render_menu_overlay(spec: &MenuRenderSpec) -> Vec<u8> {
    ...
    for (index, item) in spec.items.iter().enumerate() {
        ...
        let label_width = shortcut_text
            ...
            .map(|shortcut| {
                row_text_width.saturating_sub(
                    super::tmux_text_width(shortcut, &crate::core::Utf8Config::default()) + 1,
                )
            })
        ...
        if let Some(shortcut) = shortcut_text.as_ref() {
            let shortcut_width =
                super::tmux_text_width(shortcut, &crate::core::Utf8Config::default());
