...
use crate::core::style::{parse_colour, Style};
use crate::core::{
...
use crate::server::copy_mode::CopyModeSummary;
...
pub(crate) fn render_copy_mode_position(
    ...
    let expanded = format!(
        "#[align=right {}]{}",
        crate::core::style_tostring(&style),
...
fn push_range(
    ranges: &mut Vec<crate::server::status_ranges::StatusRange>,
    start: usize,
    end: usize,
    kind: crate::server::status_ranges::StatusRangeType,
) {
    if start > end {
        return;
    }
    ...
    ranges.push(crate::server::status_ranges::StatusRange {
        x: start..=end,
        kind,
    });
}
