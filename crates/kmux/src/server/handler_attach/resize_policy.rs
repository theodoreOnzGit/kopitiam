...
use crate::proto::{OptionName, RmuxError, SessionName, TerminalSize, WindowTarget};
...
pub(in crate::server::handler) enum AttachedWindowSizePolicy {
    Latest,
    Largest,
    Smallest,
    Manual,
...
impl RequestHandler {
    pub(in crate::server::handler) async fn attached_window_size_policy_for_session(
        &self,
        session_name: &SessionName,
    ) -> Result<AttachedWindowSizePolicy, RmuxError> {
        let state = self.state.lock().await;
        let Some(session) = state.sessions.session(session_name) else {
            return Err(crate::server::pane_terminals::session_not_found(session_name));
        };
    ...
    pub(in crate::server::handler) async fn reconcile_attached_session_size(
        &self,
        session_name: &SessionName,
    ...
    pub(in crate::server::handler) async fn reconcile_attached_session_size_and_emit(
        &self,
        session_name: &SessionName,
    ) -> Result<(), RmuxError> {
    ...
    pub(in crate::server::handler) async fn selected_attached_session_size_for_new_client(
        &self,
        session_name: &SessionName,
    ...
    pub(in crate::server::handler) async fn prune_stale_attached_clients_for_session(
        &self,
        session_name: &SessionName,
    ) -> Vec<u32> {
        let stale_pids = {
    ...
    pub(in crate::server::handler) async fn remove_attached_clients_for_session(
        &self,
        session_name: &SessionName,
        attach_pids: Vec<u32>,
    ) -> Vec<u32> {
