...
impl RequestHandler {
    pub(crate) async fn refresh_attached_session(&self, session_name: &crate::proto::SessionName) {
        let _refresh_span = crate::server::perf_instrument::span("attach_refresh")
    ...
    pub(in crate::server::handler) async fn refresh_all_attached_sessions(&self) {
        let session_names = {
            let active_attach = self.active_attach.lock().await;
    ...
    pub(in crate::server::handler) async fn refresh_persistent_overlays_for_session(
        &self,
        session_name: &crate::proto::SessionName,
    ) {
