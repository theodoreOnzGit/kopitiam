...
#[cfg(unix)]
use crate::server::unix_socket::SocketFileIdentity;
...
impl SocketCleanup {
    pub(crate) fn new(socket_path: PathBuf, socket_identity: Option<SocketFileIdentity>) -> Self {
        let socket_identity =
            socket_identity.or_else(|| crate::server::unix_socket::socket_file_identity(&socket_path).ok());
        Self {
            socket_path,
            socket_identity,
        }
...
fn cleanup_socket_artifacts(socket_path: &Path, socket_identity: Option<SocketFileIdentity>) {
    if let Some(socket_identity) = socket_identity {
        let _ = crate::server::unix_socket::remove_socket_file_if_identity_matches(
            socket_path,
            socket_identity,
        );
    }
    ...
    crate::server::tmux_shim::cleanup_tmux_shim(socket_path);
}
...
mod tests {
    ...
    async fn drop_preserves_recreated_foreign_socket() {
        ...
        let bound = crate::server::unix_socket::bind_unix_listener_at(&socket_path).expect("bind socket");
