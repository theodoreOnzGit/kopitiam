...
use crate::server::control::{ControlModeUpgrade, ControlServerEvent};
use crate::proto::{
...
async fn display_message_for_control_client_uses_message_notification() {
    ...
    assert_eq!(
        response,
        Response::DisplayMessage(crate::proto::DisplayMessageResponse::no_output())
    );
    assert_eq!(
        drain_control_notifications(&mut control_rx),
...
async fn startup_config_errors_are_queued_as_percent_config_error_notifications() {
    ...
    handler
        ...
        .lock()
        .await
        .push(crate::proto::RmuxError::Server(
...
async fn hook_commands_do_not_emit_nested_control_notifications() {
    ...
    let has_beta = handler
        .handle(Request::HasSession(crate::proto::HasSessionRequest {
