...
use crate::core::events::{PaneSnapshotCoalescerRegistry, SubscriptionLimits};
use crate::ipc::PeerIdentity;
use crate::proto::{RmuxError, TerminalSize, WindowTarget};
...
use crate::server::server_access::{current_owner_uid, AccessMode, ServerAccessStore};
use crate::server::wait_for::WaitForStore;
#[cfg(all(any(unix, windows), feature = "web"))]
use crate::server::web::WebShareRegistry;
use attach_support::{ActiveAttachState, ClientFlags};
pub(in crate::server::handler) use client_environment_support::{
    client_spawn_environment, initial_session_spawn_environment,
};
pub(in crate::server::handler) use client_runtime_support::{
...
#[cfg(test)]
pub(in crate::server::handler) use lifecycle_support::after_hook_format_values;
pub(in crate::server::handler) use lifecycle_support::prepare_lifecycle_event;
...
pub(in crate::server::handler) use target_support::{
...
pub(in crate::server::handler) enum PendingShutdownReason {
    ExitEmpty,
    KillServer,
    SeamlessUpgradeIdle,
}
...
pub(in crate::server::handler) struct DetachedRequesterAccess {
    write_scopes: usize,
    read_only_scopes: usize,
}

impl DetachedRequesterAccess {
    pub(in crate::server::handler) fn can_write(&self) -> bool {
        self.write_scopes > 0
    }

    pub(in crate::server::handler) fn is_empty(&self) -> bool {
...
impl RequestHandler {
    ...
    pub(crate) fn with_owner_uid_subscription_limits_and_web_settings(
        owner_uid: u32,
        subscription_limits: SubscriptionLimits,
        web_settings: crate::server::web::WebShareSettings,
    ) -> Self {
    ...
    fn with_owner_uid_and_environment_and_display(
        ...
        #[cfg(unix)]
        if let Some(runtime) = crate::server::pane_reader_runtime::PaneReaderRuntime::current() {
