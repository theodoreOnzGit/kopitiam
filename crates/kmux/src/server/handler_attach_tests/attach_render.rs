...
async fn session_target_refreshes_follow_the_current_active_window() {
    ...
    let split = handler
        .handle(Request::SplitWindow(SplitWindowRequest {
            ...
            direction: crate::proto::SplitDirection::Horizontal,
            before: false,
            environment: None,
        }))
        .await;
    assert_eq!(
        split,
        Response::SplitWindow(crate::proto::SplitWindowResponse {
...
async fn attach_session_upgrade_renders_only_the_active_window() {
    ...
    let quiet_command = crate::proto::ProcessCommand::Argv(quiet_ready_command(&ready_marker));
    let mut state = handler.state.lock().await;
    let pane_id = state.sessions.allocate_pane_id();
    ...
    let outcome = handler
        .dispatch(
            std::process::id(),
            Request::AttachSession(crate::proto::AttachSessionRequest { target: alpha }),
        )
        .await;
    ...
    #[cfg(windows)]
    {
        let host_short = crate::server::host_name::local_hostname()
            .and_then(|host| {
                host.split('.')
                    .next()
