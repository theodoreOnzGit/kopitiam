...
async fn lock_client_emits_lock_control_before_refresh() {
    ...
    let response = handler
        .handle(Request::LockClient(crate::proto::LockClientRequest {
            target_client: "=".to_owned(),
        }))
        .await;
    assert_eq!(
        response,
        Response::LockClient(crate::proto::LockClientResponse {
            target_client: "=".to_owned(),
        })
    );
...
async fn lock_server_skips_already_suspended_clients() {
    ...
    let first_lock = handler
        .handle(Request::LockServer(crate::proto::LockServerRequest))
        .await;
...
async fn lock_session_for_nonexistent_session_returns_error() {
    ...
    let response = handler
        .handle(Request::LockSession(crate::proto::LockSessionRequest {
...
async fn kill_server_sets_shutdown_flag() {
    ...
    let response = handler
        .handle(Request::KillServer(crate::proto::KillServerRequest))
        .await;
...
async fn daemon_status_reports_version_and_activity_counts() {
    ...
    let response = handler
        .handle(Request::DaemonStatus(crate::proto::DaemonStatusRequest))
        .await;
    ...
    assert_eq!(status.wire_version, crate::proto::RMUX_WIRE_VERSION);
...
async fn shutdown_if_idle_queues_shutdown_only_when_empty() {
    ...
    let response = handler
        .handle(Request::ShutdownIfIdle(crate::proto::ShutdownIfIdleRequest))
        .await;
    assert_eq!(
        response,
        Response::ShutdownIfIdle(crate::proto::ShutdownIfIdleResponse {
            shutdown: true,
            session_count: 0,
            client_count: 0,
        })
...
async fn server_access_protects_owner_uid() {
    ...
    let response = handler
        .handle(Request::ServerAccess(crate::proto::ServerAccessRequest {
            add: true,
            deny: false,
            list: false,
            read_only: false,
...
async fn server_access_user_mutations_are_rejected_before_user_resolution_windows() {
    ...
    for request in [
        crate::proto::ServerAccessRequest {
            add: true,
            deny: true,
            list: false,
            read_only: false,
