...
async fn attached_resize_emits_client_resized_hook_with_client_context() {
    ...
    let hooked = handler
        .handle(Request::SetHookMutation(
            crate::proto::SetHookMutationRequest {
                scope: ScopeSelector::Global,
                hook: crate::proto::HookName::ClientResized,
                command: Some(
                ...
                lifecycle: crate::proto::HookLifecycle::Persistent,
                append: false,
                unset: false,
...
async fn different_requester_pids_reject_ambiguous_cross_process_attach_control() {
    ...
    assert_eq!(
        ...
        Response::Error(crate::proto::ErrorResponse {
            error: crate::proto::RmuxError::Server(
    ...
    let detached = handler
        .dispatch(
            intruder_pid,
            Request::DetachClient(crate::proto::DetachClientRequest),
        )
        .await
        .response;
    assert_eq!(
...
async fn switch_client_last_session_recalls_the_previous_session() {
    ...
    assert_eq!(
        switched,
        Response::SwitchClient(crate::proto::SwitchClientResponse {
...
async fn wait_for_client_detached_event(
    ...
    tokio::time::timeout(ATTACH_LIFECYCLE_TIMEOUT, async {
        loop {
            ...
            if matches!(
                event.event,
                crate::core::LifecycleEvent::ClientDetached { client_name: Some(ref name), .. }
                    if name == client_name
            ) {
                return;
            }
