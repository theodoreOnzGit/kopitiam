...
async fn lock_client_accepts_tty_path_targets() {
    ...
    let tty_path = crate::os::process::fd_path(child.id(), 0).expect("tty path");
...
async fn suspend_client_marks_client_as_suspended() {
    use crate::proto::request::SuspendClientRequest;
...
async fn refresh_client_flags_merge_incrementally() {
    use crate::proto::request::RefreshClientRequest;
