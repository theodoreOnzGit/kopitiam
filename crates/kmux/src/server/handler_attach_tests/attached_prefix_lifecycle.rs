...
async fn attached_command_prompt_can_chain_choose_tree_overlay() {
    ...
    let response = handler
        .handle(Request::BindKey(Box::new(crate::proto::BindKeyRequest {
...
async fn attached_binding_run_shell_expands_client_name() {
    ...
    wait_for_file_contents(
        &output_path,
        &crate::server::handler::attached_client_name(requester_pid),
    )
    .await;
}
...
async fn attached_binding_new_window_shell_command_expands_client_name() {
    ...
    let expected_client = crate::server::handler::attached_client_name(requester_pid);
    #[cfg(unix)]
...
async fn attached_binding_two_clients_get_distinct_client_names() {
    ...
    let first_name = crate::server::handler::attached_client_name(first_pid);
    let second_name = crate::server::handler::attached_client_name(second_pid);
...
async fn attached_session_status_updates_after_external_rename() {
    ...
    let renamed = handler
        .handle(Request::RenameSession(crate::proto::RenameSessionRequest {
            target: alpha.clone(),
            new_name: beta.clone(),
        }))
        .await;
...
async fn attached_kill_last_pane_exits_the_session() {
    ...
    assert_eq!(
        killed,
        Response::KillPane(crate::proto::KillPaneResponse {
            target: PaneTarget::new(alpha.clone(), 0),
...
async fn wait_for_buffer_contents(handler: &RequestHandler, name: &str, expected: &[u8]) {
    ...
    loop {
        let response = handler
            .handle(Request::ShowBuffer(crate::proto::ShowBufferRequest {
...
async fn wait_for_global_option_value(handler: &RequestHandler, name: &str, expected: &str) {
    ...
    loop {
        let response = handler
            .handle(Request::ShowOptions(crate::proto::ShowOptionsRequest {
