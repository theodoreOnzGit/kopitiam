...
use crate::server::handler::scripting_support::QueueExecutionContext;
use crate::server::hook_runtime::with_hook_execution;
use crate::core::command_parser::CommandParser;
use crate::core::TargetFindContext;
use crate::proto::{
...
fn command_quote(command: &str) -> String {
    crate::server::test_shell::command_quote(command)
}
...
fn shell_print_command(text: &str) -> String {
    format!(
        "[Console]::Out.Write({})",
        crate::server::test_shell::powershell_quote(text)
    )
}
...
#[cfg(windows)]
fn shell_success_command() -> String {
    crate::server::test_shell::powershell_encoded_command("exit 0")
}
