...
use crate::server::pane_terminals::PaneLifecycleProcessState;
use crate::proto::{
...
use crate::proto::{HookLifecycle, HookName};
use tokio::sync::mpsc;
...
fn shell_quote(path: &Path) -> String {
    crate::server::test_shell::sh_quote_path(path)
}

fn pipe_discard_command() -> String {
    crate::server::test_shell::stdin_discard_command()
}
...
#[cfg(windows)]
fn respawn_probe_command(output: &Path) -> String {
    crate::server::test_shell::powershell_encoded_command(&format!(
        "[System.IO.File]::WriteAllText({}, ((Get-Location).Path + ':' + $env:RMUX_RESPAWN))",
        crate::server::test_shell::powershell_quote_path(output)
    ))
}
...
async fn create_session(handler: &RequestHandler, session_name: &SessionName) {
    let created = handler
        ...
        .await;
    assert!(matches!(created, crate::proto::Response::NewSession(_)));
}
...
async fn sticky_lifecycle_state_is_id_keyed_and_redacts_spawn_env() {
    ...
    let split_target = match split {
        crate::proto::Response::SplitWindow(response) => response.pane,
    ...
    let list_stdout = match listed {
        crate::proto::Response::ListPanes(response) => {
    ...
    let windows_stdout = match windows {
        crate::proto::Response::ListWindows(response) => {
    ...
    let killed = handler
        ...
        .await;
    assert!(matches!(killed, crate::proto::Response::KillPane(_)));
    {
        let state = handler.state.lock().await;
        assert!(
    ...
    assert!(matches!(
        handler
            ...
            .await,
        crate::proto::Response::SetOption(_)
    ));
    let dead_respawn = handler
        ...
        .await;
    assert!(matches!(dead_respawn, crate::proto::Response::RespawnPane(_)));
    ...
    let respawned = handler
        ...
        .await;
    assert!(matches!(respawned, crate::proto::Response::RespawnPane(_)));
    {
        let state = handler.state.lock().await;
        let session = state.sessions.session(&alpha).expect("session exists");
        let pane = session
...
async fn detached_split_with_zoom_keeps_original_pane_zoomed() {
    ...
    assert!(matches!(
        handler
            ...
            .await,
        crate::proto::Response::NewSession(_)
    ));
...
async fn detached_split_targeting_inactive_pane_preserves_active_pane() {
    ...
    assert!(matches!(
        handler
            ...
            .await,
        crate::proto::Response::SplitWindow(_)
    ));
    assert!(matches!(
        handler
            ...
            .await,
        crate::proto::Response::SelectPane(_)
    ));
    assert!(matches!(
        handler
            .handle(Request::SplitWindowExt(Box::new(SplitWindowExtRequest {
...
async fn split_window_rolls_back_session_when_spawn_fails() {
    ...
    assert!(
        matches!(&response, crate::proto::Response::Error(error) if error.error.to_string().contains("failed to spawn pane shell")),
...
async fn move_pane_routes_through_join_semantics() {
    ...
    ));
    {
        ...
        state
            .insert_window_terminal(
                &alpha,
                1,
                crate::server::pane_terminals::WindowSpawnOptions {
                    start_directory: None,
                    command: None,
    ...
    let response = handler
        .handle(Request::MovePane(MovePaneRequest {
            ...
            before: true,
            full_size: false,
            size: Some(crate::proto::PaneSplitSize::Absolute(12)),
        }))
        .await;

    assert_eq!(
        response,
        crate::proto::Response::MovePane(crate::proto::MovePaneResponse {
            target: PaneTarget::with_window(alpha.clone(), 1, 0),
        })
    );
...
async fn join_pane_exits_attached_source_session_when_source_is_removed() {
    ...
    let response = handler
        .handle(Request::JoinPane(crate::proto::JoinPaneRequest {
...
async fn break_pane_print_target_uses_custom_format() {
    ...
    let crate::proto::Response::BreakPane(success) = response else {
...
async fn pipe_pane_rejects_dead_panes() {
    ...
    assert!(
        matches!(&response, crate::proto::Response::Error(error) if error.error.to_string().contains("target pane has exited")),
...
async fn respawn_pane_rejects_active_pane_without_kill_flag() {
    ...
    assert!(
        matches!(&response, crate::proto::Response::Error(error) if error.error.to_string().contains("still active")),
...
async fn respawn_pane_with_kill_flag_applies_directory_environment_and_command() {
    ...
    assert_eq!(
        response,
        crate::proto::Response::RespawnPane(crate::proto::RespawnPaneResponse {
            target: PaneTarget::with_window(alpha, 0, 0),
        })
    );
    let expected_cwd = expected_spawn_cwd(&cwd);
...
async fn display_panes_uses_the_default_select_pane_template() {
    ...
    let response = handler
        ...
        .await;
    assert!(matches!(response, crate::proto::Response::DisplayPanes(_)));
...
async fn display_panes_default_template_runs_select_pane_hooks() {
    ...
    assert!(matches!(
        handler
            ...
            .await,
        crate::proto::Response::SetHook(_)
    ));
...
async fn join_pane_rejects_same_source_and_target() {
    ...
    assert!(
        matches!(&response, crate::proto::Response::Error(error) if error.error.to_string().contains("must be different")),
...
async fn swap_pane_self_swap_is_a_no_op() {
    ...
    let response = handler
        .handle(Request::SwapPane(crate::proto::SwapPaneRequest {
            source: PaneTarget::with_window(alpha.clone(), 0, 0),
            target: PaneTarget::with_window(alpha.clone(), 0, 0),
            direction: None,
            detached: false,
    ...
    assert!(
        matches!(response, crate::proto::Response::SwapPane(_)),
...
async fn respawn_pane_dead_pane_succeeds_without_kill_flag() {
    ...
    assert!(
        matches!(response, crate::proto::Response::RespawnPane(_)),
...
async fn remain_on_exit_keeps_the_existing_window_name() {
    ...
    assert!(matches!(
        handler
            ...
            .await,
        crate::proto::Response::RenameWindow(_)
    ));
...
async fn pipe_pane_close_on_nonexistent_pipe_is_a_no_op() {
    ...
    assert!(
        matches!(response, crate::proto::Response::PipePane(_)),
...
async fn pipe_pane_empty_command_closes_existing_pipe() {
    ...
    let open = handler
        ...
        .await;
    assert!(matches!(open, crate::proto::Response::PipePane(_)));
    ...
    let close = handler
        ...
        .await;
    assert!(
        matches!(close, crate::proto::Response::PipePane(_)),
    ...
    let reopen = handler
        ...
        .await;
    assert!(
        matches!(reopen, crate::proto::Response::PipePane(_)),
...
async fn snapshot_response(
    handler: &RequestHandler,
    target: PaneTarget,
) -> crate::proto::PaneSnapshotResponse {
    match handler
        .handle(Request::PaneSnapshot(PaneSnapshotRequest { target }))
        .await
    {
        crate::proto::Response::PaneSnapshot(response) => response,
        other => panic!("expected pane-snapshot response, got {other:?}"),
    }
}

fn collect_visible_text(response: &crate::proto::PaneSnapshotResponse, row: usize) -> String {
...
fn all_visible_text(response: &crate::proto::PaneSnapshotResponse) -> String {
...
async fn pane_snapshot_revision_changes_after_clear_history() {
    ...
    let cleared = handler
        .handle(Request::ClearHistory(crate::proto::ClearHistoryRequest {
            target: target.clone(),
            reset_hyperlinks: false,
        }))
        .await;
    assert!(matches!(cleared, crate::proto::Response::ClearHistory(_)));
