...
use crate::core::{BufferStore, EnvironmentStore, OptionStore, Pane, Session, SessionStore, Window};
use crate::proto::OptionName;
use crate::proto::{PaneTarget, SessionName, TerminalSize};
...
impl<'a> RuntimeFormatContext<'a> {
    ...
    pub(crate) fn status_job_profile(&self) -> Option<crate::server::terminal::TerminalProfile> {
        ...
        crate::server::terminal::TerminalProfile::for_run_shell_with_base_environment(
            &state.environment,
            options,
            Some(session_name),
    ...
    fn pane_lifecycle(&self) -> Option<&crate::server::pane_terminals::PaneLifecycleState> {
        let pane = self.pane?;
    ...
    fn automatic_window_name(&self) -> Option<String> {
        ...
        if !crate::server::automatic_rename::window_allows_automatic_rename(
            &state.options,
            session_name,
            window_index,
            window,
    ...
    fn pane_exit_metadata(&self) -> Option<crate::server::pane_terminals::PaneExitMetadata> {
        let session = self.session?;
        let pane = self.pane?;
    ...
    fn pane_copy_mode_summary(&self) -> Option<crate::server::copy_mode::CopyModeSummary> {
        let session = self.session?;
        let pane = self.pane?;
        self.state?
    ...
    fn pane_marked(&self) -> Option<String> {
        ...
        Some(bool_string(self.state.is_some_and(|state| {
            state.pane_is_marked(&crate::proto::PaneTarget::with_window(
                session.name().clone(),
                window_index,
                pane.index(),
            ))
    ...
    fn resolved_default_shell(&self) -> Option<String> {
        Some(
            crate::server::terminal::TerminalProfile::resolved_default_shell(
...
mod tests {
    ...
    use crate::core::formats::FormatContext;
