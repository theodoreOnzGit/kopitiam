...
use crate::proto::{OptionName, PaneTarget};

use crate::server::copy_mode::ModeKeys;
use crate::server::input_keys::{decode_extended_key, ExtendedKeyDecode};
...
fn decode_escape_key(input: &[u8], backspace: Option<u8>) -> AttachedKeyDecode {
    ...
    match input.get(1).copied() {
        ...
        Some(byte) if input.len() < 3 && byte.is_ascii() && !byte.is_ascii_control() => {
            return AttachedKeyDecode::Matched {
                size: 2,
                key: u64::from(byte) | crate::core::KEYC_META | crate::core::KEYC_IMPLIED_META,
            };
        }
        Some(byte) if byte != b'\x1b' => {
            if let Some(key) = control_byte_key(byte) {
                return AttachedKeyDecode::Matched {
                    size: 2,
                    key: key | crate::core::KEYC_META | crate::core::KEYC_IMPLIED_META,
                };
            }
            if input.len() < 3 {
                return AttachedKeyDecode::Partial;
...
mod tests {
    ...
    use crate::core::{key_code_lookup_bits, key_string_lookup_string};
