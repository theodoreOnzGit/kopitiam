...
use crate::proto::request::SwitchClientExt3Request;
use crate::proto::{CommandOutput, OptionName, RmuxError};

use crate::server::handler_support::attached_client_required;
use crate::server::outer_terminal::OuterTerminal;
use crate::server::pane_io::AttachControl;
use crate::server::pane_terminals::{session_not_found, HandlerState};
use crate::server::server_access::current_owner_uid;
use crate::server::terminal::{base_process_environment, base_process_environment_display_only};
...
pub(in crate::server::handler) use list_clients::ListClientSnapshot;

pub(in crate::server::handler) const LIST_CLIENTS_TEMPLATE: &str = "#{client_name}: #{session_name} [#{client_width}x#{client_height} #{client_termname}]#{?#{==:#{client_uid},#{uid}},, [user #{?client_user,#{client_user},#{client_uid}}]}#{?client_flags, (#{client_flags}),}";
...
impl RequestHandler {
    ...
    pub(in crate::server::handler) async fn requester_can_write(&self, requester_pid: u32) -> bool {
        {
    ...
    pub(in crate::server::handler) async fn requester_uid(&self, requester_pid: u32) -> u32 {
        {
            let active_attach = self.active_attach.lock().await;
            if let Some(active) = active_attach.by_pid.get(&requester_pid) {
                return active.uid;
    ...
    pub(in crate::server::handler) async fn list_clients_snapshot(&self) -> Vec<ListClientSnapshot> {
        let options = {
            let state = self.state.lock().await;
            state.options.clone()
        };
    ...
    pub(crate) async fn refresh_attached_client_status(
        ...
        let bytes = {
            ...
            let frame = crate::server::renderer::render_status_only_with_attached_count_and_prompt(
                session.as_ref(),
                &state.options,
                attached_count,
                crate::server::renderer::StatusRenderContext {
                    prompt: prompt.as_ref(),
                    state: Some(&state),
                    key_table: key_table.as_deref(),
                    socket_path: Some(&socket_path),
                    ..crate::server::renderer::StatusRenderContext::default()
                },
            );
...
pub(in crate::server::handler) fn parse_client_flags(
...
pub(in crate::server::handler) fn command_output_from_lines(lines: &[String]) -> CommandOutput {
    if lines.is_empty() {
...
pub(in crate::server::handler) fn normalize_target_client(target_client: &str) -> &str {
    target_client.strip_suffix(':').unwrap_or(target_client)
}

#[cfg(windows)]
pub(in crate::server::handler) fn format_client_uid(_uid: u32) -> String {
    String::new()
}

#[cfg(not(windows))]
pub(in crate::server::handler) fn format_client_uid(uid: u32) -> String {
    uid.to_string()
}

#[cfg(windows)]
pub(in crate::server::handler) fn format_requester_uid(_uid: u32) -> String {
    String::new()
}

#[cfg(not(windows))]
pub(in crate::server::handler) fn format_requester_uid(uid: u32) -> String {
    uid.to_string()
}

#[cfg(windows)]
pub(in crate::server::handler) fn format_client_user(_uid: u32, user: &UserIdentity) -> String {
    match user {
...
#[cfg(not(windows))]
pub(in crate::server::handler) fn format_client_user(uid: u32, _user: &UserIdentity) -> String {
    crate::server::server_access::user_name_for_uid(uid)
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::server::handler) enum SessionSortOrder {
    Index,
    Name,
    Activity,
    Creation,
...
pub(in crate::server::handler) fn switch_target_selector_count(request: &SwitchClientExt3Request) -> usize {
...
pub(in crate::server::handler) fn clipboard_query_sequence() -> Vec<u8> {
    b"\x1b]52;c;?\x1b\\".to_vec()
}

pub(in crate::server::handler) fn parse_session_sort_order(
...
pub(in crate::server::handler) fn sort_list_clients(
...
pub(in crate::server::handler) fn format_attached_client_flags(
...
pub(in crate::server::handler) fn format_control_client_flags(
...
pub(in crate::server::handler) fn attached_client_matches_target(
    attach_pid: u32,
    target_client: &str,
) -> bool {
...
fn attached_client_tty_path(attach_pid: u32) -> Option<PathBuf> {
    crate::os::process::fd_path(attach_pid, 0)
}

pub(in crate::server::handler) fn attached_client_name(attach_pid: u32) -> String {
    attached_client_tty_path(attach_pid)
        .map(|path| path.to_string_lossy().into_owned())
        .unwrap_or_else(|| attach_pid.to_string())
}

pub(in crate::server::handler) fn session_selection_prefers_live_process(pid: u32) -> bool {
    process::is_live(pid)
}

pub(in crate::server::handler) fn client_environment_snapshot(
    requester_pid: u32,
...
pub(in crate::server::handler) fn effective_client_terminal_context(
    client_environment: Option<&HashMap<String, String>>,
    client_terminal: &crate::proto::ClientTerminalContext,
) -> crate::proto::ClientTerminalContext {
...
pub(in crate::server::handler) fn current_process_environment_snapshot() -> HashMap<String, String> {
    base_process_environment()
}

pub(in crate::server::handler) fn current_process_environment_display_snapshot() -> HashMap<String, String>
{
    base_process_environment_display_only()
}

pub(in crate::server::handler) fn seed_global_environment(
...
pub(in crate::server::handler) fn seed_global_display_environment(
    state: &mut HandlerState,
    environment: HashMap<String, String>,
) {
    for (name, value) in environment {
...
pub(in crate::server::handler) fn update_environment_from_client(
    state: &mut HandlerState,
    session_name: &crate::proto::SessionName,
...
mod tests {
    ...
    use crate::os::identity::UserIdentity;
    use crate::proto::ClientTerminalContext;
