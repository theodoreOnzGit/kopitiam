...
use crate::proto::{HookName, PaneTarget, Request, Response, ScopeSelector, Target, WindowTarget};
use tokio::sync::{broadcast, watch};
use tracing::warn;

use crate::server::hook_runtime::{
...
pub(crate) struct QueuedLifecycleEvent {
    pub(in crate::server::handler) event: LifecycleEvent,
    pub(in crate::server::handler) hook_name: HookName,
    pub(in crate::server::handler) hooks: Vec<HookDispatch>,
    pub(in crate::server::handler) formats: Vec<(String, String)>,
    pub(in crate::server::handler) current_target: Option<Target>,
}
...
impl RequestHandler {
    ...
    pub(in crate::server::handler) async fn emit(&self, event: LifecycleEvent) {
    ...
    pub(in crate::server::handler) async fn emit_without_attached_refresh(&self, event: LifecycleEvent) {
        if hooks_disabled() {
            return;
        }
        let queued = {
    ...
    pub(in crate::server::handler) fn emit_prepared(&self, event: QueuedLifecycleEvent) {
        if hooks_disabled() {
            return;
        }
    ...
    pub(in crate::server::handler) fn shutdown_wait_for(&self) {
    ...
    pub(in crate::server::handler) async fn dispatch_lifecycle_hook(&self, event: QueuedLifecycleEvent) {
    ...
    pub(in crate::server::handler) fn queue_inline_hook(
        &self,
        hook: HookName,
        scope: ScopeSelector,
    ...
    pub(in crate::server::handler) async fn run_inline_hooks(
        &self,
        requester_pid: u32,
    ...
    pub(in crate::server::handler) async fn run_request_hooks(
        &self,
        requester_pid: u32,
        request: &Request,
        response: &Response,
    ...
    pub(in crate::server::handler) async fn run_command_error_hook_for_parsed_command(
        &self,
        requester_pid: u32,
    ...
    async fn fallback_target_from_current_hook_formats(&self) -> Option<Target> {
        let formats = crate::server::hook_runtime::current_hook_formats();
        let state = self.state.lock().await;
...
pub(in crate::server::handler) fn prepare_lifecycle_event(
...
pub(in crate::server::handler) fn after_hook_format_values(
    hook: HookName,
    parsed_command: Option<&ParsedCommand>,
) -> Vec<(String, String)> {
...
fn hook_format_session_target(
    ...
    let session_name = formats
        ...
        .and_then(|(_, value)| crate::proto::SessionName::new(value.clone()).ok())?;
    first_session_pane_target(state, &session_name)
}

fn target_exists(state: &crate::server::pane_terminals::HandlerState, target: &Target) -> bool {
    match target {
