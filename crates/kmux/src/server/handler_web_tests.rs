use super::*;
use crate::core::events::SubscriptionLimits;
use crate::proto::WebShareCreatedResponse;
use crate::proto::{encode_attach_message, AttachMessage};
use crate::proto::{
...
async fn web_share_create_starts_lazy_listener() {
    ...
    assert!(matches!(
        ...
        Response::WebShare(response)
            if matches!(response.as_ref(), crate::proto::WebShareResponse::Created(_))
    ));
}
...
async fn web_share_config_starts_lazy_listener() {
    ...
    let response = handler
        .handle(Request::WebShare(Box::new(WebShareRequest::Config(
            crate::proto::WebShareConfigRequest,
        ))))
        .await;
    ...
    let crate::proto::WebShareResponse::Config(config) = *response else {
        panic!("expected web-share config response");
    };
...
async fn implicit_web_share_port_falls_back_when_default_is_busy() {
    ...
    let handler = handler_with_web_settings(
        crate::server::web::WebShareSettings::from_options_with_port_explicit(blocked_port, None, false)
            .expect("web settings"),
    );
...
async fn web_share_create_resolves_slot_target_to_stable_pane_id() {
    ...
    let created = create_share(
        ...
        share_request(WebShareScope::Pane(
            crate::proto::PaneTarget::new(session_name.clone(), 0).into(),
        )),
    )
    .await;
    assert!(matches!(
...
async fn create_share(
    ...
    let crate::proto::WebShareResponse::Created(created) = *response else {
        panic!("expected created web-share response");
    };
    created
}
...
fn handler_with_web_port(port: u16) -> RequestHandler {
    handler_with_web_settings(
        crate::server::web::WebShareSettings::from_options(port, None).expect("web settings"),
    )
}

fn handler_with_web_settings(settings: crate::server::web::WebShareSettings) -> RequestHandler {
...
async fn list_shares(handler: &RequestHandler) -> Vec<crate::proto::WebShareSummary> {
    let response = handler
    ...
    let crate::proto::WebShareResponse::List(listed) = *response else {
        panic!("expected listed web-share response");
    };
    listed.shares
}
