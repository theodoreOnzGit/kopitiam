...
use crate::proto::{HookLifecycle, HookName, RmuxError, ScopeSelector, SessionName, WindowTarget};
...
fn pane_hooks_fall_back_from_pane_to_window_to_global_window_scope() {
    ...
    let pane = crate::proto::PaneTarget::with_window(alpha.clone(), 1, 2);
...
fn removing_and_renaming_sessions_updates_window_and_pane_scopes() {
    ...
    store
        .set(
            ScopeSelector::Pane(crate::proto::PaneTarget::with_window(alpha.clone(), 2, 3)),
            HookName::PaneExited,
            "printf pane".to_owned(),
            HookLifecycle::Persistent,
        )
    ...
    assert_eq!(
        store.pane_command(
            &crate::proto::PaneTarget::with_window(beta, 2, 3),
            HookName::PaneExited,
        ),
        None
    );
...
fn swapping_windows_rekeys_window_and_pane_hooks() {
    ...
    let source_pane = crate::proto::PaneTarget::with_window(alpha.clone(), 1, 0);
    let target_pane = crate::proto::PaneTarget::with_window(alpha, 3, 0);
...
fn moving_windows_rekeys_window_and_pane_hooks() {
    ...
    let source_pane = crate::proto::PaneTarget::with_window(alpha.clone(), 2, 0);
    let target_pane = crate::proto::PaneTarget::with_window(alpha, 4, 0);
...
fn removing_a_window_also_removes_its_pane_hooks() {
    ...
    let alpha = session_name("alpha");
    let window = WindowTarget::with_window(alpha.clone(), 1);
    let pane = crate::proto::PaneTarget::with_window(alpha.clone(), 1, 0);
    let other_pane = crate::proto::PaneTarget::with_window(alpha.clone(), 2, 0);
