...
use crate::proto::{ControlModeResponse, HandshakeResponse, LayoutName, RmuxError, SdkWaitId};
...
mod tests {
    use super::*;
    use crate::proto::{OptionScopeSelector, PaneOutputSubscriptionId, SetOptionMode};
    use serde::Serialize;
    use std::mem::size_of;
    ...
    fn pr6g_boxed_response_payloads_are_bincode_transparent() {
        ...
        where
            T: Serialize,
        {
            let mut expected = crate::proto::frame_kind_for_response(&response)
                .bincode_tag()
                .to_le_bytes()
                .to_vec();
