...
use crate::pty::unsupported_op;
#[cfg(all(not(unix), not(windows)))]
use crate::pty::PtyError;
#[cfg(any(unix, windows))]
use crate::pty::{backend, PtyPair};
use crate::pty::{ProcessId, PtyMaster, Result, Signal, TerminalSize};
...
fn spawn_child(command: ChildCommand) -> Result<SpawnedPty> {
    ...
    let pre_exec = move || {
        crate::os::signals::reset_child_signal_dispositions()?;
