...
//! ---
//!
//! **Part of `kmux`, a fork of [rmux](https://github.com/helvesec/rmux).**
//!
...
//! `LICENSE-APACHE` in `crates/kmux/`). It is distributed as part of
//! KOPITIAM under **AGPL-3.0-only**. See `crates/kmux/NOTICE`.
//!
//! KOPITIAM's changes add Android/Termux support. `crate::os::runtime_dir`
//! documents every Android decision in the fork; read it before changing a
//! `cfg` gate.

// `extern crate alloc;` was hoisted to the crate root (src/lib.rs): as a nested
// module this binding no longer reaches the web_crypto submodules that do
// `use alloc::vec::Vec;`. At the crate root it enters the extern prelude crate-wide.
