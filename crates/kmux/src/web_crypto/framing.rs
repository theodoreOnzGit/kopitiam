...
use crate::web_crypto::error::Error;
use crate::web_crypto::record::{RecordOpener, RecordSealer};
...
mod tests {
    ...
    fn raw_c2s_pair() -> (RecordSealer, Opener) {
        ...
        let dh = [9u8; 32];
        let sealer = crate::web_crypto::schedule::derive(psk, &dh, &[0x42u8; 32], b"h", b"c")
            .unwrap()
            .into_client()
            .0;
        let opener = crate::web_crypto::schedule::derive(psk, &dh, &[0x42u8; 32], b"h", b"c")
            .unwrap()
            .into_server()
            .1;
