...
use crate::proto::{PaneSnapshotCell, PaneSnapshotCursor, PaneSnapshotResponse};
...
fn pane_ref_target_selector_recognizes_session_invalidation() {
    let target = PaneRef::new(crate::proto::SessionName::new("alpha").unwrap(), 3, 1);
...
fn is_already_closed_error_matches_session_not_found_for_target_session() {
    let target = PaneRef::new(crate::proto::SessionName::new("alpha").unwrap(), 0, 0);
    let error = RmuxError::protocol(crate::proto::RmuxError::SessionNotFound("alpha".to_owned()));
...
fn is_already_closed_error_does_not_match_session_not_found_for_other_session() {
    let target = PaneRef::new(crate::proto::SessionName::new("alpha").unwrap(), 0, 0);
    let error = RmuxError::protocol(crate::proto::RmuxError::SessionNotFound("beta".to_owned()));
    assert!(!is_already_closed_error(&error, &target));
}

#[test]
fn is_already_closed_error_matches_invalid_window_or_pane_target() {
    let target = PaneRef::new(crate::proto::SessionName::new("alpha").unwrap(), 5, 2);
    let pane_invalid = RmuxError::protocol(crate::proto::RmuxError::InvalidTarget {
        value: "alpha:5.2".to_owned(),
        reason: "pane index does not exist in session".to_owned(),
    });
    let window_invalid = RmuxError::protocol(crate::proto::RmuxError::InvalidTarget {
...
fn is_already_closed_error_ignores_unrelated_protocol_errors() {
    let target = PaneRef::new(crate::proto::SessionName::new("alpha").unwrap(), 0, 0);
    let error = RmuxError::protocol(crate::proto::RmuxError::Server(
...
fn is_already_closed_error_ignores_invalid_target_for_other_slot() {
    let target = PaneRef::new(crate::proto::SessionName::new("alpha").unwrap(), 5, 2);
    let foreign = RmuxError::protocol(crate::proto::RmuxError::InvalidTarget {
