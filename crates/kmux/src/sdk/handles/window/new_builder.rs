...
use crate::sdk::handles::session::{unexpected_response, Session};
use crate::sdk::transport::TransportClient;
use crate::sdk::{ProcessCommandSpec, Result, RmuxError, SessionName, Window, WindowRef};
use crate::proto::{NewWindowRequest, ProcessCommand, Request, Response};
...
fn proto_process_command(
    ...
    if process_command.is_empty() {
        return Err(RmuxError::SpawnFailed {
            message: crate::proto::PROCESS_COMMAND_EMPTY_MESSAGE.to_owned(),
        });
    }
