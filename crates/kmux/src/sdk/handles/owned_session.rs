...
use crate::sdk::{EnsureSession, Result, RmuxError, Session, SessionName};
use crate::proto::{
...
impl OwnedSessionLease {
    async fn start(session: &Session, ttl: Duration) -> Result<Self> {
        ...
        let transport = session.transport().clone();
        crate::sdk::capabilities::require(&transport, &[CAPABILITY_SDK_SESSION_LEASE]).await?;
    ...
    async fn release_confirmed(&self) -> Result<bool> {
        if self.is_lost() {
            return Err(RmuxError::from(
                crate::proto::RmuxError::owned_session_lease_lost(self.session_name.clone()),
            ));
        }
...
fn ttl_millis(ttl: Duration) -> Result<u64> {
    ...
    let millis = u64::try_from(ttl.as_millis()).map_err(|_| {
        RmuxError::protocol(crate::proto::RmuxError::Server(
...
fn validate_lease_ttl(ttl: Duration) -> Result<()> {
    ...
    if millis < crate::proto::MIN_SESSION_LEASE_TTL_MILLIS {
...
fn is_missing_session(error: &RmuxError) -> bool {
    matches!(
        error,
        RmuxError::Protocol {
            source: crate::proto::RmuxError::SessionNotFound(_),
        }
    )
}
