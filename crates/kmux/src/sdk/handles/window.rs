...
impl TryFrom<crate::proto::WindowListEntry> for ListedWindow {
    type Error = RmuxError;

    fn try_from(entry: crate::proto::WindowListEntry) -> Result<Self> {
        Ok(Self {
...
fn parse_error(message: impl Into<String>) -> RmuxError {
    RmuxError::protocol(crate::proto::RmuxError::Server(message.into()))
}

fn is_already_closed_error(error: &RmuxError, target: &WindowRef) -> bool {
    match error {
        RmuxError::Protocol {
            source: crate::proto::RmuxError::SessionNotFound(session),
        } => session == target.session_name.as_str(),
        RmuxError::Protocol {
            source: crate::proto::RmuxError::InvalidTarget { value, reason },
        } => {
