...
#[cfg(windows)]
use crate::sdk::bootstrap::deadline::StartupDeadline;
use crate::sdk::bootstrap::discovery;
#[cfg(windows)]
use crate::sdk::diagnostics::FEATURE_TRANSPORT_UNIX_SOCKET;
#[cfg(unix)]
use crate::sdk::diagnostics::FEATURE_TRANSPORT_WINDOWS_PIPE;
use crate::sdk::transport::TransportClient;
use crate::sdk::{Result, RmuxEndpoint, RmuxError};
#[cfg(windows)]
...
async fn connect_or_start_transport_for_platform(
    ...
    let outcome = crate::sdk::bootstrap::startup_unix::connect_or_start_with_timeout(
        &socket_path,
        || {
            let socket_path = socket_path.clone();
        ...
        timeout,
        crate::sdk::bootstrap::startup_unix::STARTUP_POLL_INTERVAL,
    )
    .await
...
async fn connect_or_start_transport_for_platform(
    ...
    let outcome = crate::sdk::bootstrap::startup_windows::connect_or_start_with_timeout(
        &pipe_path,
        || {
        ...
        crate::sdk::bootstrap::startup_windows::STARTUP_POLL_INTERVAL,
    )
    .await
    .map_err(startup_error)?;
...
async fn drop_windows_startup_probe_stream(
    outcome: crate::sdk::bootstrap::startup_windows::StartupOutcome,
) -> Result<()> {
...
fn spawn_hidden_daemon(endpoint: &OsStr) -> io::Result<()> {
    ...
    for (index, binary) in candidates.iter().enumerate() {
        let result = match spawn_hidden_daemon_with_binary(endpoint, binary, true) {
            Ok(()) => return Ok(()),
            Err(error) if crate::os::daemon::should_retry_hidden_daemon_without_breakaway(&error) => {
...
fn spawn_hidden_daemon_with_binary(
    ...
    #[cfg(windows)]
    let ready = crate::os::daemon::StartupReadyEvent::new()?;
    ...
    crate::os::daemon::configure_hidden_daemon_command(&mut command, allow_job_breakaway);
    let child = crate::os::daemon::spawn_hidden_daemon_command(&mut command)?;
    drop(child);
    #[cfg(windows)]
    {
...
pub(super) fn daemon_binary() -> std::ffi::OsString {
    std::env::var_os(discovery::SDK_DAEMON_BINARY_ENV)
        .unwrap_or_else(|| crate::os::host::PUBLIC_BINARY_NAME.into())
}

fn hidden_daemon_binary_candidates() -> Vec<OsString> {
    hidden_daemon_binary_candidates_from_sources(
        std::env::var_os(discovery::SDK_DAEMON_BINARY_ENV),
        resolve_executable_on_path(crate::os::host::PUBLIC_BINARY_NAME),
    )
}
...
fn hidden_daemon_binary_candidates_from_sources_with_resolved(
    ...
    push_unique_candidate(&mut candidates, OsString::from(crate::os::host::DAEMON_BINARY_NAME));
    ...
    push_unique_candidate(&mut candidates, OsString::from(crate::os::host::PUBLIC_BINARY_NAME));
    candidates
}
...
fn daemon_sibling_path(binary: &Path) -> Option<PathBuf> {
    ...
    let daemon_file_name = match binary.extension().and_then(|extension| extension.to_str()) {
        ...
        _ => crate::os::host::DAEMON_BINARY_NAME.to_owned(),
    };
