...
fn sdk_wait_request(wait_id: SdkWaitId) -> Request {
    Request::SdkWaitForOutput(SdkWaitForOutputRequest {
        owner_id: crate::proto::SdkWaitOwnerId::new(7),
        wait_id,
...
fn list_sessions_response(stdout: &[u8]) -> Response {
    Response::ListSessions(ListSessionsResponse {
        output: crate::proto::CommandOutput::from_stdout(stdout),
    })
}
...
fn session_not_found_response() -> Response {
    Response::Error(ErrorResponse {
        error: crate::proto::RmuxError::SessionNotFound("alpha".to_owned()),
    })
}

fn unsupported_capability_response() -> Response {
    Response::Error(ErrorResponse {
        error: crate::proto::RmuxError::UnsupportedCapability {
            feature: "capability.future".to_owned(),
            supported: vec![crate::proto::CAPABILITY_HANDSHAKE.to_owned()],
        },
    })
}
...
fn unsupported_wire_version_response() -> Response {
    Response::Error(ErrorResponse {
        error: crate::proto::RmuxError::UnsupportedWireVersion {
            got: crate::proto::RMUX_WIRE_VERSION + 1,
            minimum: crate::proto::RMUX_WIRE_VERSION,
            maximum: crate::proto::RMUX_WIRE_VERSION,
        },
    })
}

async fn read_request(stream: &mut tokio::io::DuplexStream) -> Request {
    let mut decoder = crate::proto::FrameDecoder::new();
...
async fn write_unknown_response_tag(stream: &mut tokio::io::DuplexStream) {
    ...
    let mut frame = vec![
        crate::proto::RMUX_FRAME_MAGIC,
        crate::proto::RMUX_WIRE_VERSION as u8,
    ];
...
fn assert_transport_kind(result: crate::sdk::Result<Response>, expected: io::ErrorKind) {
...
fn assert_transport_message(
    result: crate::sdk::Result<Response>,
...
fn assert_unsupported_feature(result: crate::sdk::Result<Response>, expected: &str) {
    match result.expect_err("request must fail") {
...
fn spawn_request(
    ...
    request: Request,
) -> JoinHandle<crate::sdk::Result<Response>> {
    let client = client.clone();
    tokio::spawn(async move { client.request(request).await })
}

async fn join_request(handle: JoinHandle<crate::sdk::Result<Response>>) -> crate::sdk::Result<Response> {
...
async fn error_response_completes_current_fifo_slot_as_protocol_error() {
    ...
    match join_request(request).await.expect_err("request must fail") {
        RmuxError::Protocol { source } => {
            assert_eq!(
                source,
                crate::proto::RmuxError::SessionNotFound("alpha".to_owned())
            );
        }
...
async fn unsupported_capability_response_maps_to_stable_sdk_feature() {
    ...
    match join_request(request).await.expect_err("request must fail") {
        RmuxError::Unsupported { feature, hint } => {
            ...
            assert!(hint.contains(crate::proto::CAPABILITY_HANDSHAKE));
        }
        error => panic!("expected unsupported error, got {error:?}"),
    }
}
...
async fn unsupported_wire_version_response_maps_to_stable_sdk_feature() {
    ...
    match join_request(request).await.expect_err("request must fail") {
        RmuxError::Unsupported { feature, .. } => {
            assert_eq!(feature, crate::sdk::FEATURE_PROTOCOL_WIRE_VERSION);
        }
        error => panic!("expected unsupported error, got {error:?}"),
    }
}
...
async fn handshake_response_decode_mismatch_maps_to_stable_sdk_feature() {
    ...
    assert_unsupported_feature(
        ...
        crate::sdk::FEATURE_PROTOCOL_CAPABILITIES,
    );
}
...
async fn capability_require_caches_handshake_per_transport() {
    ...
    let first_require = {
        ...
        tokio::spawn(async move {
            crate::sdk::capabilities::require(&client, &[CAPABILITY_SDK_PANE_BY_ID]).await
        })
    };
    ...
    write_response(
        ...
        &Response::Handshake(HandshakeResponse {
            wire_version: crate::proto::RMUX_WIRE_VERSION,
            capabilities: vec![
    ...
    crate::sdk::capabilities::require(&client, &[CAPABILITY_SDK_PANE_BY_ID])
        .await
