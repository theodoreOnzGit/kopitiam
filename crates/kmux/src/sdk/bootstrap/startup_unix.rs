...
use crate::os::identity::real_user_id;
...
async fn connect_socket_path(socket_path: &Path) -> io::Result<UnixStream> {
    ...
    let stream = task::spawn_blocking(move || {
        let endpoint = crate::ipc::resolve_endpoint(None, Some(socket_path.as_path()))?;
        crate::ipc::connect_blocking(&endpoint, CONNECT_PROBE_TIMEOUT)
    })
    .await
