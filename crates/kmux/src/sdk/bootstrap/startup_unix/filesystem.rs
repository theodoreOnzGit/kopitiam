...
fn private_startup_lock_root() -> Result<PathBuf, StartupError> {
    let env = crate::os::runtime_dir::RuntimeDirEnv::from_process_env();
    crate::os::runtime_dir::resolve_runtime_dir(&env, None).map_err(|error| {
        StartupError::Filesystem {
            operation: "resolve private startup lock root",
            path: PathBuf::from(crate::os::runtime_dir::FHS_FALLBACK_ROOT),
            source: error,
        }
    })
}
