...
use crate::sdk::{Pane, PaneId, PaneRef, Result, RmuxError};
use crate::proto::{PaneBroadcastInputRequest, Request, Response, CAPABILITY_SDK_PANE_BROADCAST};
...
async fn broadcast_daemon_side(panes: &[Pane], input: Input<'_>) -> Result<BroadcastResult> {
    crate::sdk::capabilities::require(panes[0].transport(), &[CAPABILITY_SDK_PANE_BROADCAST]).await?;
...
fn fallback_target_from_ref(target: crate::proto::PaneTargetRef) -> PaneRef {
    match target {
        crate::proto::PaneTargetRef::Slot(target) => target.into(),
        crate::proto::PaneTargetRef::Id { session_name, .. } => PaneRef::new(session_name, 0, 0),
    }
}

fn is_daemon_broadcast_unavailable(error: &RmuxError) -> bool {
    if crate::sdk::capabilities::is_unavailable(error, CAPABILITY_SDK_PANE_BROADCAST) {
        return true;
    }
...
mod tests {
    ...
    use crate::sdk::{Pane, PaneId, PaneRef, RmuxEndpoint, SessionName};
    use crate::proto::{
