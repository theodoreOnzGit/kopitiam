...
fn is_subscription_gone_matches_known_server_strings() {
    let gone = RmuxError::protocol(crate::proto::RmuxError::Server(
        "subscription not found".to_owned(),
    ));
    let receiver_gone = RmuxError::protocol(crate::proto::RmuxError::Server(
        "subscription receiver not found".to_owned(),
    ));
    let other = RmuxError::protocol(crate::proto::RmuxError::Server(
...
fn is_subscription_gone_does_not_match_ownership_or_invalid_target_errors() {
    ...
    // ending the stream.
    let owned_elsewhere = RmuxError::protocol(crate::proto::RmuxError::Server(
        "subscription is not owned by this connection".to_owned(),
    ));
    let invalid_target = RmuxError::protocol(crate::proto::RmuxError::InvalidTarget {
        value: "alpha:0.0".to_owned(),
        reason: "pane index does not exist in session".to_owned(),
    });
    let session_not_found =
        RmuxError::protocol(crate::proto::RmuxError::SessionNotFound("alpha".to_owned()));
