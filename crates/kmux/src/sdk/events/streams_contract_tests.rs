...
//!   exposing [`crate::proto::PaneOutputSubscriptionId`] to the SDK's
...
use crate::sdk::{PaneId, PaneRef, Result};
use crate::proto::{
    ...
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    PaneOutputStream::open(
        transport,
        crate::proto::PaneTargetRef::slot(target.into()),
        start,
    )
    .await
}
...
async fn drive_cursor_with_subscription_gone(stream: &mut DuplexStream) {
    ...
    write_response(
        ...
        &Response::Error(ErrorResponse {
            error: crate::proto::RmuxError::Server("subscription not found".to_owned()),
        }),
    )
    .await;
}
