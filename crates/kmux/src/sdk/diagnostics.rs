...
pub const FEATURE_PROTOCOL_CAPABILITIES: &str = crate::proto::CAPABILITY_HANDSHAKE;
...
pub const FEATURE_DAEMON_SHUTDOWN: &str = crate::proto::CAPABILITY_DAEMON_SHUTDOWN;
...
#[must_use]
pub fn unsupported_feature_id(error: &crate::proto::RmuxError) -> Option<String> {
    match error {
        crate::proto::RmuxError::UnsupportedWireVersion { .. } => {
            Some(FEATURE_PROTOCOL_WIRE_VERSION.to_owned())
        }
        crate::proto::RmuxError::UnsupportedCapability { feature, .. } => Some(feature.clone()),
        crate::proto::RmuxError::UnknownCommand(command) => Some(command_feature_id(command)),
        _ => None,
    }
}
...
#[must_use]
pub fn protocol_diagnostic(error: &crate::proto::RmuxError) -> Diagnostic {
    match error {
        crate::proto::RmuxError::UnsupportedWireVersion {
            got,
            minimum,
...
mod tests {
    ...
    fn protocol_unsupported_errors_map_to_stable_feature_ids() {
        assert_eq!(
            unsupported_feature_id(&crate::proto::RmuxError::UnsupportedWireVersion {
                got: 2,
                minimum: 1,
                maximum: 1,
            })
        ...
        assert_eq!(
            unsupported_feature_id(&crate::proto::RmuxError::UnsupportedCapability {
                feature: "capability.future".to_owned(),
                supported: vec![crate::proto::CAPABILITY_DETACHED_RPC.to_owned()],
            })
            .as_deref(),
            Some("capability.future")
        );
        assert_eq!(
            unsupported_feature_id(&crate::proto::RmuxError::UnknownCommand(
    ...
    fn protocol_diagnostic_preserves_unsupported_capability_hint() {
        let diagnostic = protocol_diagnostic(&crate::proto::RmuxError::UnsupportedCapability {
            feature: "capability.future".to_owned(),
        ...
        assert!(diagnostic
            .hint()
            .expect("hint")
            .contains(crate::proto::CAPABILITY_HANDSHAKE));
    }
}
