...
use crate::sdk::handles::{session, Rmux, Session};
use crate::sdk::transport::TransportClient;
use crate::sdk::{ProcessCommandSpec, ProcessSpec, Result, RmuxError, SessionName, TerminalSizeSpec};
use redaction::redact_environment_error;
use crate::proto::{NewSessionExtRequest, Request, Response};
...
impl EnsureSession {
    ...
    ///
    /// `Duration::MAX` requests no timeout, matching [`RmuxBuilder`](crate::sdk::RmuxBuilder)
    /// timeout semantics.
    #[must_use]
    ...
    pub(crate) fn to_new_session_request(&self, attach_if_exists: bool) -> NewSessionExtRequest {
        NewSessionExtRequest {
            ...
            process_command: self
                ...
                .process_command
                .clone()
                .map(crate::proto::ProcessCommand::from),
...
async fn create_session(
    ...
    crate::sdk::capabilities::require_process_command_if_present(
        transport,
...
async fn reuse_session(
    ...
    let Some(session_name) = builder.session_name.clone() else {
        return Err(RmuxError::protocol(crate::proto::RmuxError::Server(
        ...
        .map_err(|error| redact_builder_environment_error(error, builder))?
    {
        ...
        Err(RmuxError::protocol(crate::proto::RmuxError::SessionNotFound(
...
fn is_duplicate_session_error(error: &RmuxError) -> bool {
    matches!(
        error,
        RmuxError::Protocol {
            source: crate::proto::RmuxError::DuplicateSession(_),
        }
    )
}
