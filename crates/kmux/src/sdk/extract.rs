...
use crate::sdk::{Pane, PaneExitState, PaneOutputChunk, PaneOutputStart, PaneSnapshot, Result};
...
pub(crate) async fn collect_output_until_exit_starting_at(
    ...
    let timeout = crate::sdk::wait::resolved_wait_timeout(pane.configured_default_timeout());
    crate::sdk::wait::with_wait_timeout(
...
async fn collect_output_until_exit_without_timeout(
    ...
    let mut stream = match pane.output_stream_starting_at(start).await {
        Ok(stream) => stream,
        Err(error) if crate::sdk::handles::is_already_closed_pane_error(&error, pane.target()) => {
    ...
    if let crate::sdk::wait::PaneExitObservation::Exited(exit_state) =
        crate::sdk::wait::pane_exit_observation(pane).await?
    {
    ...
    loop {
        ...
        if !saw_ready_output.saw_output {
            tokio::time::sleep(crate::sdk::wait::TEXT_POLL_INTERVAL).await;
        }
    }
}

async fn drain_until_output_eof_or_close(
    stream: &mut crate::sdk::PaneOutputStream,
...
async fn exit_state_after_stream_close(pane: &Pane) -> Result<Option<PaneExitState>> {
    loop {
        match crate::sdk::wait::pane_exit_observation(pane).await? {
            crate::sdk::wait::PaneExitObservation::Running => {
                tokio::time::sleep(crate::sdk::wait::TEXT_POLL_INTERVAL).await;
            }
            crate::sdk::wait::PaneExitObservation::Exited(exit_state) => return Ok(exit_state),
        }
    }
}
...
mod tests {
    use super::*;
    use crate::sdk::{PaneCell, PaneCursor, PaneGlyph};
