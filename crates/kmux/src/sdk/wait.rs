...
use crate::sdk::handles::{connect_transport_to_endpoint, Pane};
use crate::sdk::transport::{DropGuard, PendingResponse, TransportClient};
use crate::sdk::{Result, RmuxError};
...
///
/// Values are returned by [`Pane::wait_for_next`](crate::sdk::Pane::wait_for_next)
/// and [`Pane::wait_for_text_next`](crate::sdk::Pane::wait_for_text_next) after the
...
pub(crate) async fn wait_exit(pane: &Pane) -> Result<Option<crate::sdk::PaneExitState>> {
    let timeout = resolved_wait_timeout(pane.configured_default_timeout());
    with_wait_timeout(
...
async fn arm_sdk_wait(
    ...
    crate::sdk::capabilities::require_with_handshake(
        &wait_client,
...
async fn sdk_wait_request_for_pane(
    pane: &Pane,
    owner_id: crate::proto::SdkWaitOwnerId,
    wait_id: SdkWaitId,
    bytes: Vec<u8>,
) -> Result<Request> {
    if pane.is_stable_id() {
        crate::sdk::capabilities::require(pane.transport(), &[CAPABILITY_SDK_PANE_BY_ID]).await?;
...
async fn wait_exit_without_timeout(pane: &Pane) -> Result<Option<crate::sdk::PaneExitState>> {
    loop {
...
pub(crate) async fn pane_exit_observation(pane: &Pane) -> Result<PaneExitObservation> {
    ...
    if matches!(pane.process, crate::sdk::PaneProcessState::Exited) || pane.exit_state.is_some() {
...
pub(crate) enum PaneExitObservation {
    Running,
    Exited(Option<crate::sdk::PaneExitState>),
}
...
pub(crate) fn resolved_wait_timeout(default_timeout: Option<Duration>) -> Option<Duration> {
    crate::sdk::bootstrap::discovery::resolve_timeout(None, default_timeout)
}
...
fn sdk_wait_response_to_result(response: Response, expected_wait_id: SdkWaitId) -> Result<()> {
    match response {
        ...
        response => Err(crate::sdk::handles::session::unexpected_response(
            "sdk-wait-output",
            response,
        )),
    }
...
mod tests {
    ...
    use crate::proto::{encode_frame, CancelSdkWaitResponse, FrameDecoder, SdkWaitForOutputResponse};
