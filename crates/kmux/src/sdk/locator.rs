...
use crate::sdk::{Pane, PaneSnapshot, PaneTextMatch, Result, RmuxError, WaitTimeoutError};
...
/// Text query accepted by [`Pane::get_by_text`](crate::sdk::Pane::get_by_text).
...
impl Locator {
    ...
    fn new(pane: Pane, query: LocatorQuery) -> Self {
        Self {
            ...
            timeout: None,
            poll_interval: crate::sdk::wait::TEXT_POLL_INTERVAL,
            invalid_reason: None,
        }
    }
    ...
    pub(crate) async fn resolve_strict_with_wait(&self) -> Result<(PaneSnapshot, LocatorMatch)> {
        let timeout = self
            .timeout
            .or_else(|| crate::sdk::wait::resolved_wait_timeout(self.pane.configured_default_timeout()));
    ...
    fn is_plain_combinable(&self) -> bool {
        self.selection == LocatorSelection::Strict
            ...
            && self.poll_interval == crate::sdk::wait::TEXT_POLL_INTERVAL
...
async fn wait_for_assertion(locator: Locator, kind: LocatorAssertionKind) -> Result<PaneSnapshot> {
    ...
    let timeout = locator
        .timeout
        .or_else(|| crate::sdk::wait::resolved_wait_timeout(locator.pane.configured_default_timeout()));
    let deadline = timeout.map(|timeout| Instant::now() + timeout);
    loop {
