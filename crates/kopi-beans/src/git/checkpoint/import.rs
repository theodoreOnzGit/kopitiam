...
use crate::git::core::crdt::Crdt;
use crate::git::core::limits::LimitViolation;
use crate::git::core::state::LabelState;
use crate::git::core::wire_bead::{WireDepStoreV1, WireLabelStateV1, WireStamp, WireTombstoneV1};
use crate::git::core::{
...
pub enum CheckpointImportError {
    ...
    StoreIdMismatch {
        meta: crate::git::core::StoreId,
        manifest: crate::git::core::StoreId,
    },
    #[error("checkpoint store epoch mismatch (meta {meta}, manifest {manifest})")]
    StoreEpochMismatch {
        meta: crate::git::core::StoreEpoch,
        manifest: crate::git::core::StoreEpoch,
    },
        ...
        let stats = match rel_path.kind {
            CheckpointFileKind::State => parse_jsonl_file::<BeadSnapshotWireV1, _>(
                ...
                |line, wire| {
                    ...
                    let bead = crate::git::core::Bead::from(wire);
                    let state = state_for_namespace(&mut state, &ns);
                    state.insert_live(bead);

                    for note in notes {
                        let note = crate::git::core::Note::from(note);
...
impl StampFromWire {
    fn stamp(stamp: WireStamp, actor: &crate::git::core::ActorId) -> crate::git::core::Stamp {
        crate::git::core::Stamp::new(WriteStamp::from(stamp), actor.clone())
    }
}
...
mod tests {
    ...
    use crate::git::checkpoint::{CheckpointFileKind, CheckpointShardPath, ManifestFile, shard_name};
    use crate::git::core::wire_bead::WireClaimSnapshot;
    use crate::git::core::{
...
"#;
    ...
    fn merge_store_states_is_commutative() {
        ...
        let bead = crate::git::core::Bead::from(sample_wire_bead_full("bd-abc"));
        state.insert_live(bead);
    ...
    fn store_state_from_legacy_places_state_in_core_namespace() {
        let mut state = CanonicalState::default();
        let bead = crate::git::core::Bead::from(sample_wire_bead_full("bd-legacy"));
        state.insert_live(bead);
