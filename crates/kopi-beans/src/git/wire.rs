...
impl WireBeadFullCompat {
    fn from_wire(wire: BeadSnapshotWireV1) -> Self {
        ...
        let assignee_at = match &wire.claim {
            crate::git::core::WireClaimSnapshot::Claimed(_) => Some(WireStamp::from(&claim_stamp.at)),
            crate::git::core::WireClaimSnapshot::Unclaimed(_) => None,
        };
        Self {
            wire,
            closed_at,
...
pub fn serialize_meta(
    ...
    let meta = WireMeta {
        format_version: crate::git::core::FormatVersion::CURRENT.get(),
...
fn legacy_dep_dot_for_key(key: &DepKey) -> Dot {
    crate::git::core::wire_bead::legacy_hash_dot(b"legacy-deps-v0", &key.collision_bytes())
}
...
impl StoreMeta {
    ...
    pub fn format_version(&self) -> Option<u32> {
        match self {
            StoreMeta::Legacy => None,
            StoreMeta::V1 { .. } => Some(1),
            StoreMeta::V2 { .. } => Some(crate::git::core::FormatVersion::CURRENT.get()),
        }
    }
}
...
impl SupportedStoreMeta {
    ...
    pub fn parse(bytes: &[u8]) -> Result<Self, WireError> {
        ...
        let checksums = match meta.format_version {
            ...
            version if version == crate::git::core::FormatVersion::CURRENT.get() => {
                match (
                    meta.state_sha256,
                    meta.tombstones_sha256,
                    meta.deps_sha256,
        ...
        Ok(Self {
            meta: match meta.format_version {
                ...
                version if version == crate::git::core::FormatVersion::CURRENT.get() => StoreMeta::V2 {
                    root_slug,
                    last_write_stamp,
                    checksums,
                },
...
fn map_snapshot_error(file: &str, err: crate::git::core::SnapshotCodecError) -> WireError {
