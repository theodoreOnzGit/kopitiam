...
use crate::surface::ipc::{EmptyPayload, Request, send_request};
use crate::surface::store_admin::{
...
use crate::git::{SyncError, SyncProcess, sync, wire};
...
impl CliHostBackend for BeadsRsCliBackend {
    ...
    fn run_store_fsck(&self, request: StoreFsckRequest) -> Result<crate::api::AdminFsckOutput> {
    ...
    ) -> Result<crate::api::AdminStoreUnlockOutput> {
...
fn map_deps_format(format: crate::git::wire::DepsFormat) -> DepsFormat {
    match format {
        crate::git::wire::DepsFormat::OrSetV1 => DepsFormat::OrSetV1,
        crate::git::wire::DepsFormat::LegacyEdges => DepsFormat::LegacyEdges,
    }
}
...
fn detect_store_migration_state(repo: &Repository) -> Result<MigrateDetectOutcome> {
    ...
    let live_remote_preview = match sync::open_remote_store_preview(repo) {
        Ok(preview) => preview,
        Err(crate::git::SyncError::Fetch(err)) if local_oid.is_some() => {
            tracing::warn!(
                error = ?err,
...
fn resolve_ref_oid(repo: &Repository, refname: &str) -> Result<Option<Oid>> {
    match repo.refname_to_id(refname) {
        ...
        Err(err) => Err(crate::git::SyncError::Git(err).into()),
    }
}
...
fn read_meta_probe(repo: &Repository, tree: &Tree<'_>) -> Result<Option<(u32, bool)>> {
    ...
    let parsed = wire::parse_supported_meta(&bytes).map_err(crate::git::SyncError::from)?;
...
fn read_blob_from_tree(
    ...
    let blob = obj
        .peel_to_blob()
        .map_err(|_| crate::git::SyncError::NotABlob(name))?;
