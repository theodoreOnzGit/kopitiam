...
use crate::cli_surface::cli::{CliHost, RuntimeBuildArgs};
use crate::cli_surface::runtime::{CliRuntimeCtx, validate_actor_id as cli_validate_actor_id};
use crate::cli_surface::validation::{normalize_optional_client_request_id, normalize_optional_namespace};
use crate::core::{ActorId, Applied, DurabilityClass, NamespaceId, Watermarks};

pub use crate::cli_surface::cli::{Cli, Command, command_name, parse_from};
pub use crate::cli_surface::commands::daemon::DaemonCmd;
...
pub fn run(cli: Cli) -> Result<()> {
    ...
    crate::cli_surface::cli::run(cli, &host, &backend)
}
...
fn resolve_repo(repo: Option<PathBuf>) -> Result<PathBuf> {
    ...
    let abs = if p.is_absolute() {
        ...
        let cwd = std::env::current_dir().map_err(|err| {
            Error::Ipc(crate::surface::IpcError::DaemonUnavailable(format!(
...
mod tests {
    ...
    use crate::cli_surface::validation::{
