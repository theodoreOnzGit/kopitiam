use super::*;
use crate::daemon::runtime::core::StoreSessionToken;

impl Daemon {
    pub(in crate::daemon::runtime) fn start_sync_with_gate(
        &mut self,
        gate: crate::daemon::scheduler::SyncGateToken,
        git_tx: &Sender<GitOp>,
    ) -> bool {
    ...
    pub(in crate::daemon::runtime) fn force_start_sync(
        &mut self,
        remote: &RemoteUrl,
        git_tx: &Sender<GitOp>,
    ) -> bool {
    ...
    pub(in crate::daemon::runtime) fn ensure_repo_fresh(
        &mut self,
        repo: &Path,
        git_tx: &Sender<GitOp>,
    ...
    pub(in crate::daemon::runtime) fn complete_refresh(
        &mut self,
        ...
        match result {
            Ok(fresh) => {
                ...
                repo_state.last_fetch_error =
                    fresh
                        .fetch_error
                        .map(|message| crate::daemon::git_lane::FetchErrorRecord {
                            message,
                            wall_ms: now_wall_ms,
                        });
                repo_state.last_divergence =
                    fresh
                        .divergence
                        .map(|divergence| crate::daemon::git_lane::DivergenceRecord {
                ...
                repo_state.last_force_push =
                    fresh
                        .force_push
                        .map(|force_push| crate::daemon::git_lane::ForcePushRecord {
    ...
    pub(in crate::daemon::runtime) fn force_reload(
        &mut self,
        repo: &Path,
        git_tx: &Sender<GitOp>,
    ) -> Result<LoadedStore<'_>, OpError> {
    ...
    /// - Repo is dirty
    /// - Not already syncing
    pub(in crate::daemon::runtime) fn maybe_start_sync(
        &mut self,
        remote: &RemoteUrl,
        git_tx: &Sender<GitOp>,
    ) -> bool {
    ...
    pub(in crate::daemon::runtime) fn complete_sync(
        &mut self,
        session: StoreSessionToken,
        remote: &RemoteUrl,
    ...
    pub(in crate::daemon::runtime) fn next_sync_deadline(&mut self) -> Option<Instant> {
        self.scheduler_mut().next_deadline()
    }

    pub(in crate::daemon::runtime) fn next_checkpoint_deadline(&mut self) -> Option<Instant> {
