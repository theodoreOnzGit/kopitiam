...
use crate::daemon::git_lane::GitLaneState;
...
impl Daemon {
    ...
    /// Get a single bead.
    pub(in crate::daemon::runtime) fn query_show(
        &mut self,
        repo: &Path,
        id: &BeadId,
        read: ReadConsistency,
    ...
    pub(in crate::daemon::runtime) fn query_show_multiple(
        &mut self,
        repo: &Path,
        ids: &[BeadId],
        read: ReadConsistency,
    ...
    pub(in crate::daemon::runtime) fn query_show_details(
        &mut self,
        repo: &Path,
        id: &BeadId,
        read: ReadConsistency,
    ...
    pub(in crate::daemon::runtime) fn query_list(
        &mut self,
        repo: &Path,
        filters: &Filters,
        read: ReadConsistency,
    ...
    pub(in crate::daemon::runtime) fn query_tracker_list(
        &mut self,
        repo: &Path,
    ...
    pub(in crate::daemon::runtime) fn query_ready(
        &mut self,
        repo: &Path,
        limit: Option<usize>,
        read: ReadConsistency,
    ...
    pub(in crate::daemon::runtime) fn query_dep_tree(
        &mut self,
        repo: &Path,
        id: &BeadId,
        read: ReadConsistency,
    ...
    pub(in crate::daemon::runtime) fn query_deps(
        &mut self,
        repo: &Path,
        id: &BeadId,
        read: ReadConsistency,
    ...
    pub(in crate::daemon::runtime) fn query_notes(
        &mut self,
        repo: &Path,
        id: &BeadId,
        read: ReadConsistency,
    ...
    pub(in crate::daemon::runtime) fn query_status(
        &mut self,
        repo: &Path,
        read: ReadConsistency,
        git_tx: &Sender<GitOp>,
    ...
    /// Get blocked issues.
    pub(in crate::daemon::runtime) fn query_blocked(
        &mut self,
        repo: &Path,
        read: ReadConsistency,
        git_tx: &Sender<GitOp>,
    ...
    pub(in crate::daemon::runtime) fn query_stale(
        &mut self,
        repo: &Path,
        days: u32,
    ...
    pub(in crate::daemon::runtime) fn query_count(
        &mut self,
        repo: &Path,
        filters: &Filters,
        group_by: Option<&str>,
                ...
                match group_by {
                    ...
                    "assignee" => {
                        let group = match &bead.fields.claim.value {
                            crate::daemon::core::Claim::Claimed { assignee, .. } => {
                                assignee.as_str().to_string()
                            }
                            crate::daemon::core::Claim::Unclaimed => "(unassigned)".to_string(),
                        };
                        *counts.entry(group).or_insert(0) += 1;
                    }
                    "label" => {
    ...
    pub(in crate::daemon::runtime) fn query_deleted(
        &mut self,
        repo: &Path,
        since_ms: Option<u64>,
        id: Option<&BeadId>,
    ...
    pub(in crate::daemon::runtime) fn query_epic_status(
        &mut self,
        repo: &Path,
        eligible_only: bool,
        read: ReadConsistency,
    ...
    /// Validate state.
    pub(in crate::daemon::runtime) fn query_validate(
        &mut self,
        repo: &Path,
        read: ReadConsistency,
        git_tx: &Sender<GitOp>,
    ...
    /// Dependency cycles.
    pub(in crate::daemon::runtime) fn query_dep_cycles(
        &mut self,
        repo: &Path,
        read: ReadConsistency,
        git_tx: &Sender<GitOp>,
...
mod tests {
    ...
    fn dep_cycles_from_state_reports_cycle() {
        ...
        let ab = DepKey::new_local(
            &NamespaceId::core(),
            crate::daemon::core::BeadId::parse(a).unwrap(),
            crate::daemon::core::BeadId::parse(b).unwrap(),
            DepKind::Blocks,
        )
        .expect("dep key");
