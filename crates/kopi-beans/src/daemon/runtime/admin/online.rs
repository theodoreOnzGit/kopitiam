...
impl Daemon {
    fn validated_replication_roster_present(&self, store_id: StoreId) -> Result<bool, OpError> {
        crate::daemon::runtime::store::runtime::load_replica_roster(self.layout(), store_id)
    ...
    pub(crate) fn admin_metrics(
        ...
        drop(proof);
        let snapshot = crate::daemon::runtime::metrics::snapshot();
    ...
    pub(crate) fn admin_doctor(
        ...
        if report.summary.safe_to_accept_writes {
            crate::daemon::runtime::metrics::scrub_ok();
        } else {
            crate::daemon::runtime::metrics::scrub_err();
        }
        crate::daemon::runtime::metrics::scrub_records_checked(report.stats.records_checked);
    ...
    pub(crate) fn admin_fingerprint(
        ...
        let now_ms = crate::daemon::WallClock::now().0;
    ...
    pub(crate) fn admin_reload_limits(&mut self, repo: &Path, git_tx: &Sender<GitOp>) -> Response {
        ...
        let config = match crate::daemon::config::load_for_repo(Some(repo)) {
            Ok(config) => config,
            Err(err) => {
                return Response::err_from(OpError::ValidationFailed {
                    field: "limits".into(),
