...
use crate::daemon::core::error::details::WalTailTruncatedDetails;
...
pub enum StoreRuntimeError {
    ...
    NamespacePoliciesParse {
        path: Box<std::path::PathBuf>,
        #[source]
        source: crate::daemon::core::NamespacePoliciesError,
    },
...
pub(crate) fn load_namespace_policies(
    ...
    let path = crate::daemon::daemon_layout_from_paths().namespaces_path(&store_id);
    let raw = match read_secure_store_file(&path) {
        Ok(Some(raw)) => raw,
        Ok(None) => return Ok(defaults.clone()),
        Err(StoreConfigFileError::Symlink { path }) => {
...
mod tests {
    ...
    use crate::daemon::core::bead::{BeadCore, BeadFields};
    use crate::daemon::core::composite::Claim;
    use crate::daemon::core::crdt::Lww;
    use crate::daemon::core::domain::{BeadType, DepKind, Priority};
    use crate::daemon::core::identity::BeadId;
    ...
    fn store_runtime_resets_older_store_format_and_rebuilds_cleanly() {
        ...
        let wal_sentinel = paths::store_dir(store_id)
            .join("wal")
            .join(crate::daemon::core::NamespaceId::core().as_str())
    ...
    fn checkpoint_dirty_shards_roundtrip() {
        ...
        let bead = crate::daemon::core::Bead::new(core, fields);
...
"#;
    ...
    fn namespace_policies_enforce_permissions() {
        ...
        let defaults = crate::daemon::config::Config::default()
            .namespace_defaults
            .namespaces;
