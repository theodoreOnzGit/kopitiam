...
use crate::daemon::core::{ProtocolErrorCode, ReplicateMode, StoreId};
use crate::daemon::runtime::ops::OpError;
...
use crate::daemon::runtime::store::runtime::load_replica_roster;
use crate::daemon::runtime::wal::WalIndex;

const REPL_RUNTIME_RETRY_AFTER_MS: u64 = 100;

struct ReplicationRuntimeInputs {
    runtime_version: crate::daemon::runtime::store::runtime::ReplicationRuntimeVersion,
...
impl Daemon {
    pub(in crate::daemon::runtime) fn replication_config(&self) -> &crate::daemon::config::ReplicationConfig {
        &self.replication
    }

    pub(in crate::daemon::runtime) fn set_replication_config(
        &mut self,
        config: crate::daemon::config::ReplicationConfig,
    ) {
    ...
    fn start_replication_server(
        ...
        session_store: SharedSessionStore<ReplSessionStore>,
        server_config: ReplicationServerConfig,
    ) -> Option<crate::daemon::runtime::repl::ReplicationServerHandle> {
    ...
    pub(crate) fn reload_replication_config(&mut self, repo_path: &Path) -> Result<(), OpError> {
        let config = crate::daemon::config::load_for_repo(Some(repo_path)).map_err(|e| {
            OpError::ValidationFailed {
                field: "replication".into(),
                reason: format!("failed to reload config: {e}"),
            }
        })?;
        let runtime = crate::daemon::config::daemon_runtime_from_config(&config);
