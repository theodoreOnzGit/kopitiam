...
pub(super) fn apply_checkpoint_watermarks(
    ...
    for (namespace, origin_map) in origins {
        for (origin, _) in origin_map {
            ...
            let watermarks = crate::daemon::core::WatermarkPair::new(applied, durable)
...
/// ```compile_fail,E0616
/// use crate::daemon::__doctest::core::PendingReplayApply;
///
    ...
    for row in rows {
        ...
        while from_seq_excl.get() < row.applied().seq().get() {
            ...
            for item in items {
                ...
                let dirty_paths =
                    crate::daemon::runtime::store::runtime::checkpoint_dirty_paths_for_outcome(
                        &namespace,
                        state_for_namespace,
                        &outcome,
                    );
...
pub(super) fn replication_listen_addr(config: &crate::daemon::config::ReplicationConfig) -> String {
...
pub(super) fn replication_backoff(config: &crate::daemon::config::ReplicationConfig) -> BackoffPolicy {
