...
use crate::daemon::core::limits::LimitViolation;
...
use crate::daemon::runtime::wal::record::RECORD_HEADER_BASE_LEN;
use crate::surface::ops::Patch;
...
/// ```compile_fail,E0616
/// use crate::daemon::__doctest::mutation_engine::PlannedMutation;
///
...
pub struct SequencedEvent {
    ...
    pub event_bytes: EventBytes<crate::daemon::core::Canonical>,
    pub request_sha256: [u8; 32],
    pub client_request_id: Option<ClientRequestId>,
}
...
impl MutationEngine {
    ...
    fn enforce_record_size(
        &self,
        event_bytes: &EventBytes<crate::daemon::core::Canonical>,
    ...
    fn plan_claim(
        ...
        if let crate::daemon::core::Claim::Claimed {
            assignee,
            expires: Some(exp),
    ...
    fn plan_unclaim(
        ...
        if let crate::daemon::core::Claim::Claimed { assignee, .. } = &bead.fields.claim.value {
    ...
    fn plan_extend_claim(
        ...
        let assignee = match &bead.fields.claim.value {
            ...
            crate::daemon::core::Claim::Unclaimed => {
                return Err(OpError::NotClaimedByYou);
            }
        };
...
fn parse_create_dep_specs(
    ...
    for raw in deps {
        ...
        let (kind, id_raw) = if let Some((kind_raw, id_raw)) = trimmed.split_once(':') {
            (
                crate::daemon::core::ValidatedDepKind::parse(kind_raw)
                    .map_err(|err| OpError::ValidationFailed {
...
fn txn_id_for_stamp(store: &StoreIdentity, stamp: &crate::daemon::core::Stamp) -> TxnId {
...
fn generate_unique_id(
    ...
    description: &str,
    actor: &ActorId,
    stamp: &crate::daemon::core::WriteStamp,
    remote: &RemoteUrl,
) -> Result<BeadId, OpError> {
