...
use crate::daemon::config::PathsConfig;
use crate::daemon::core::{NamespaceId, StoreId};
use crate::bootstrap::paths as moved;
...
pub fn init_from_config(config: &PathsConfig) {
    ...
    *guard = config.clone();
    ...
    crate::git::init_data_dir_override(config.data_dir.clone());
    crate::surface::ipc::set_runtime_dir_override(config.runtime_dir.clone());
}
...
pub struct DataDirOverride {
    prev: Option<PathBuf>,
    _git_override: crate::git::test_support::DataDirOverride,
}

impl DataDirOverride {
    pub fn new(path: Option<PathBuf>) -> Self {
        let git_override = crate::git::test_support::override_data_dir_for_tests(path.clone());
