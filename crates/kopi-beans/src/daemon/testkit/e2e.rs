...
use crate::daemon::testkit::wal::{
    EventWal, MemoryWalIndex, ReplicaLivenessRow, SegmentConfig, SegmentSyncMode, WalIndexError,
    rebuild_index,
};
use crate::daemon::testkit::{TestGitRx, TestGitTx, handle_request_for_tests, new_test_git_channel};
use crate::daemon_core::repl::frame::{FrameLimitState, FrameReader, encode_frame};
use crate::daemon_core::repl::proto::{
...
impl TestNode {
    ...
    pub fn restart(&self) {
        ...
        *daemon.clock_mut() = Clock::with_time_source_and_max_forward_drift(
...

        {
            ...
            let store_dir = crate::daemon::daemon_layout_from_paths().store_dir(&store_id);
    ...
    pub fn read_wal_range(
        ...
        self.with_daemon(|daemon| {
            ...
            let reader = crate::daemon::testkit::repl::WalRangeReader::new(
                paths::store_dir(store_id),
            ...
            match reader.read_range(
                ...
                Ok(frames) => frames
                    ...
                    .collect(),
                Err(crate::daemon::testkit::repl::WalRangeError::MissingRange { .. }) => Vec::new(),
...
impl SessionStore for TestSessionStore {
    ...
    fn ingest_remote_batch(
        &mut self,
        batch: &crate::daemon::testkit::repl::ContiguousBatch,
        now_ms: u64,
    ...
    fn update_replica_liveness(
        ...
        role: crate::daemon::testkit::wal::ReplicaDurabilityRole,
        ...
        self.node.with_daemon(|daemon| {
            ...
            txn.upsert_replica_liveness(&crate::daemon::testkit::wal::ReplicaLivenessRow {
                replica_id,
                last_seen_ms,
                last_handshake_ms,
                role,
        ...
        if matches!(
            ...
            if want_map.is_empty() {
                ...
                if resend_due || changed {
                    ...
                    self.transport
                        .b
                        .send_message(&crate::daemon::testkit::repl::ReplMessage::Want(want));
        ...
        match action {
            ...
            SessionAction::PeerWant(want) => {
                ...
                if !events.is_empty() {
                    ...
                    transport
                        .send_message(&crate::daemon::testkit::repl::ReplMessage::Events(message));
                }
            }
...
fn ensure_tmp_root() -> PathBuf {
    crate::daemon::test_utils::ensure_relative_tmp_root()
}
...
impl ChannelEndpoint {
    pub fn send_message(&self, message: &crate::daemon::testkit::repl::ReplMessage) {
...
fn encode_message_frame(
    message: crate::daemon::testkit::repl::ReplMessage,
    max_frame_bytes: usize,
) -> Vec<u8> {
