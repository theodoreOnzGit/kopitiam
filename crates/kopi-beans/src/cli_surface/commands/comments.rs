...
use crate::api::{Note, QueryResult};
use crate::surface::ipc::{AddNotePayload, IdPayload, Request, ResponsePayload};
...
pub fn handle_comment_add(ctx: &CliRuntimeCtx, args: CommentAddArgs) -> CommandResult<()> {
    let content = if !args.content.is_empty() {
        ...
        std::io::stdin()
            ...
            .map_err(|source| crate::surface::ipc::IpcError::Transport { source })?;
        s.trim().to_string()
    };
