...
use crate::cli_surface::commands::common::{dep_edges_need_namespace, fmt_dep_endpoint};
use crate::cli_surface::parsers::parse_dep_kind;
use crate::cli_surface::render::{dep_record_json_value, dependency_summary_json_value_for_edge, print_json};
use crate::cli_surface::runtime::{CliRuntimeCtx, send};
use crate::cli_surface::validation::{normalize_bead_ref_for, validation_error};
use crate::api::{DepCycles, DepEdge};
use crate::core::{BeadId, BeadRef, DepKind, NamespaceId};
use crate::surface::Filters;
use crate::surface::ipc::{
...
fn handle_dep_list(ctx: &CliRuntimeCtx, args: DepListArgs) -> CommandResult<()> {
    ...
    crate::cli_surface::render::print_line(&render_deps(&incoming, &outgoing))?;
    Ok(())
}
...
fn fetch_dep_edges(
    ...
    match send(&req)? {
        ResponsePayload::Query(crate::api::QueryResult::Deps { incoming, outgoing }) => {
            Ok((incoming, outgoing))
        }
        other => Err(crate::surface::ipc::IpcError::DaemonUnavailable(format!(
...
fn fetch_summary_map(
    ctx: &CliRuntimeCtx,
    refs: Vec<BeadRef>,
) -> CommandResult<HashMap<BeadRef, crate::api::IssueSummary>> {
    if refs.is_empty() {
    ...
    for (namespace, ids) in by_namespace {
        ...
        match send(&req)? {
            ResponsePayload::Query(crate::api::QueryResult::Issues(issues)) => {
                for issue in issues {
            ...
            other => {
                return Err(crate::surface::ipc::IpcError::DaemonUnavailable(format!(
...
pub fn render_dep_tree(root: &crate::core::BeadRef, edges: &[DepEdge]) -> String {
    let force_namespace =
