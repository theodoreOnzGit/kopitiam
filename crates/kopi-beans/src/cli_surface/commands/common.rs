...
use crate::cli_surface::validation::validation_error;
use crate::api::{Issue, QueryResult};
use crate::core::{BeadId, IssueStatus, NamespaceId};
use crate::surface::ipc::{IdPayload, IpcError, Request, ResponsePayload};
...
pub(crate) fn dep_edges_need_namespace<'a>(
    mut edges: impl Iterator<Item = &'a crate::api::DepEdge>,
) -> bool {
...
pub(crate) fn fmt_metric_labels(labels: &[crate::api::AdminMetricLabel]) -> String {
    if labels.is_empty() {
        return String::new();
    }
    let mut out = String::from(" {");
