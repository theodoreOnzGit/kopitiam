...
use crate::cli_surface::parsers::{parse_bead_type, parse_priority};
...
use crate::cli_surface::runtime::{CliRuntimeCtx, send, send_raw};
use crate::api::{Issue, QueryResult};
use crate::core::{BeadType, Priority};
use crate::surface::OpResult;
use crate::surface::ipc::{CreatePayload, Request, Response, ResponsePayload};
...
fn handle_from_markdown_file(ctx: &CliRuntimeCtx, path: &std::path::Path) -> CommandResult<()> {
    ...
    if ctx.json {
        print_json(&crate::cli_surface::render::issue_array_json_value(&created))?;
        return Ok(());
    }
...
fn parse_markdown_file(path: &std::path::Path) -> CommandResult<Vec<MarkdownIssue>> {
    ...
    let file = std::fs::File::open(path)
        .map_err(|source| crate::surface::IpcError::Transport { source })?;
    ...
    for line in reader.lines() {
        let line = line.map_err(|source| crate::surface::IpcError::Transport { source })?;
...
mod tests {
    use super::*;
    use crate::cli_surface::commands::CommandError;
