...
use crate::surface::ipc::{Request, ResponsePayload, StalePayload};
use clap::Args;
...
pub struct StaleArgs {
    ...
    #[arg(short = 's', long, value_parser = crate::cli_surface::parsers::parse_status)]
...
pub fn handle(ctx: &CliRuntimeCtx, args: StaleArgs) -> CommandResult<()> {
    ...
    if let ResponsePayload::Query(QueryResult::Stale(issues)) = ok {
        crate::cli_surface::render::print_line(&render_stale(&issues, args.days))?;
    } else {
        print_ok(&ok, false)?;
    }
...
pub fn render_stale(issues: &[crate::api::IssueSummary], threshold_days: u32) -> String {
    if issues.is_empty() {
