...
use crate::core::{BeadRef, WallClock, WriteStamp};
use crate::surface::OpResult;
use crate::surface::ipc::ResponsePayload;
use serde::Serialize;
...
pub fn print_line(line: &str) -> crate::cli_surface::Result<()> {
    use std::io::Write;
...
pub fn print_json<T: Serialize>(value: &T) -> crate::cli_surface::Result<()> {
    use std::io::Write;
    let mut stdout = std::io::stdout().lock();
    serde_json::to_writer_pretty(&mut stdout, value)
        .map_err(|source| crate::surface::ipc::IpcError::PayloadEncode { source })?;
...
pub fn print_cli_json(payload: &ResponsePayload) -> crate::cli_surface::Result<()> {
