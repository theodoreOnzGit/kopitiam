// Only the `nl_langinfo` codeset path needs `CStr`, and Android does not have
// that path -- see `LibcLocaleBackend::codeset`.
...
impl LocaleBackend for LibcLocaleBackend {
    ...
    /// Android's codeset is UTF-8. Always.
    ///
    /// Two independent things force this branch:
    ///
    /// 1. **The `libc` crate does not bind `nl_langinfo`/`CODESET` for
    ///    `target_os = "android"`.** Bionic has had the function since API 26,
    ///    but there is no Rust binding to call it through.
    /// 2. **Bionic has no non-UTF-8 codeset to report even if we could ask.**
    ///    Bionic's locale support is deliberately vestigial: it accepts only
    ///    `C`, `C.UTF-8`, `en_US.UTF-8` and `POSIX`, and every one of them is
    ///    UTF-8. Android has been UTF-8-only since its first release.
    ///
    /// So this is not a stub that papers over a gap — it is the correct answer,
    /// and the caller's "is the locale UTF-8?" check is trivially satisfiable on
    /// this platform rather than unanswerable. Reporting `None` here would make
    /// kmux refuse to start on every Android device for no reason.
