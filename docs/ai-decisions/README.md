...
| [AID-0044](AID-0044-tui-is-rust-driven-constrained-oracle-human-is-judgment-rung.md) | The Termux TUI + agent loop is **Rust-driven with the 0.5B as a constrained oracle**, and the **human is the judgment rung** (first-class choice-card decision loop, the `AskUserQuestion` discipline) — **NOT** a model-driven agentic loop; a 0.5B can't drive reliable multi-step function-calling, so Rust owns control flow + grammar-constrained decoding (AID-0045) + generation→selection + deterministic verification + Rust-gated tool exec, and judgment moves to the human present; partially resolves design §8.2 (frame the fork for the human) | Pending review | `kopitiam-02p` |
| [AID-0045](AID-0045-grammar-constrained-decoding-logit-masking.md) | **Grammar-constrained decoding** in `kopitiam-runtime` — mask disallowed tokens' logits to `-inf` **before** sampling (front of `sampling.rs`, before temperature/top-k), so the 0.5B **physically cannot** emit invalid JSON, non-existent tool names, or malformed paths; guarantees *form, not sense*; cheap vector op; **the keystone** that makes the small model tool-capable; watch token-vs-grammar (BPE) boundary alignment | Pending review | `kopitiam-190` |
| [AID-0046](AID-0046-portable-capability-tiered-model-ladder.md) | **Portable capability-tiered model ladder** — one binary, rungs auto-discovered by the AID-0043 probe (which gains a **THIRD client**: model-tier selection); the 0.5B is a **PERMANENT fast-reflex tier** (routing / structured-emission / drafting), not a fallback; compose by **cascade/escalation first** (reuses AID-0040 verbatim), **speculative decoding later**; determinism is device-relative and that's honest | Pending review | `kopitiam-nhu` |
| [AID-0047](AID-0047-human-decisions-persist-as-deterministic-rules.md) | **Human decisions are persisted as deterministic rules** — the offline **decision flywheel**: each human choice (decision + rationale) hardens into a rule in `kopitiam-index`, and the next matching fork is applied **without asking**, so the tool asks *less* over time; Rust-owned dispatch, not model memory; rules are defaults-until-changed (listable + revocable) | Pending review | `kopitiam-h0l` |
| [AID-0048](AID-0048-kvim-tab-pages.md) | **kvim tab pages** — a tab IS a whole `WindowTree`; `App` owns an ordered `TabPages` collection above the window layer, but the **active tab's live tree stays in `App.windows`** (park/swap the others, O(1), no clone) so all ~50 window/`<C-w>` call sites stay untouched; tab commands ride the same "editor recognises, UI performs" seam as `:sp` via a new `core::TabCommand`; one editor cursor, a tab is a viewport (extends AID-0020) | Pending review | `kopitiam-ygk` |
