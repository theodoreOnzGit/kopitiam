...
| [AID-0037](AID-0037-huggingface-fetching-is-default-on.md) | HuggingFace fetching (`hf::HfFetcher` + `HfModel`/`Revision`) is a default-on capability riding the existing `net` gate (already `default`), not a separate opt-in flag and not an ungated stack — `--no-default-features` still gives the offline BYO build; catalog population deferred to the maintainer | Pending review | `kopitiam-9l4` |
