...
| 2026-07-28 | [Silent wrongness, and why only a reference oracle finds it](2026-07-28-silent-wrongness-and-the-reference-oracle.md) | Three bugs that ran, looked plausible and passed every test — a wrong RoPE convention (`bd-b9x`), a dequantized tied embedding (`bd-8f8`), an over-strict vocab check (AID-0054). What found them was diffing intermediate tensors against a locally built `llama.cpp`, not inspection. Also: a test that asserted the bug, optimising against a broken baseline, and why measuring before wiring stopped a 2–4x slowdown |
| 2026-08-25 | [The guard you drop when porting is the one you never had a fixture for](2026-08-25-dropped-guards-in-a-port.md) | GitHub #70: `resolve()` gave a different answer depending on whether the page had been rasterized first. A port of MuPDF's `pdf_load_obj_stm` had dropped the check that the xref still sources an object from *this* object stream, so an incremental update's superseded copy poisoned the cache. Why the dropped branch was unfalsifiable against a single-generation test corpus |
